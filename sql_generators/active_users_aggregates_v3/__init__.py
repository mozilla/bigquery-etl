...
def generate(target_project, output_dir, use_cloud_function):
    ...
    for browser in Browsers:
        ...
        # Write view SQL.
        # focus_android's view is generated by the active_users_aggregates_v4
        # generator (pointing to the v4 table). Skip it here so v3 doesn't also
        # emit the view pointing to the v3 table.
        if browser.name != "focus_android":
            write_sql(
                output_dir=output_dir,
                full_table_id=f"{target_project}.{browser.name}.{BASE_NAME}",
