...
class TestGenerateDocs:
    ...
    def test_generate_docs(self, tmp_path):
        generate_docs(tmp_path, TEST_DIR / "data" / "test_docs")

        with open(TEST_DIR / "data" / "test_docs" / "mkdocs.yml") as expected_mkdocs:
            with open(tmp_path / "mkdocs.yml") as mkdocs:
                assert expected_mkdocs.read() == mkdocs.read()
