...
old_events_staging AS (
  SELECT
    a.client_id AS telemetry_client_id,
    MIN(COALESCE(a.submission_date, c.submission_date)) AS firefox_first_run_date,
    MIN(
      CASE
    ...
    LEFT JOIN 
    `moz-fx-data-shared-prod.firefox_desktop_derived.metrics_clients_daily_v1` c 
    ON a.client_id = c.client_id
    AND a.submission_date = c.submission_date
  WHERE
    a.submission_date <= current_date
    AND c.submission_date <= current_date
  GROUP BY
    a.client_id
),
...
old_events_stg_1 AS (
  --by client ID, get first run date
    SELECT
    a.client_id AS telemetry_client_id,
    MIN(a.submission_date) AS firefox_first_run_date,
    MIN(
      CASE
        WHEN IFNULL(a.search_count_all, 0) > 0 --NEED TO ADD THIS TO the table before continuing with this PR
          THEN a.submission_date
        ELSE NULL
      END
    ) AS firefox_first_search_date,
...
old_events_stg_2 AS (
  --by client ID, get first ad click date
  SELECT 
    client_id,
    MIN(
    CASE
