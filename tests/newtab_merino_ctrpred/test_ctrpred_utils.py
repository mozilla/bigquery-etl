...
def test_replace_ctrpred_treatment_rows_preserves_other_rows():
    ...
    assert artifact[0]["click_count"] == 12
    assert artifact[0]["impression_count"] == 250
    assert isinstance(artifact[0]["click_count"], int)
    assert isinstance(artifact[0]["impression_count"], int)
