...
-- resolved_date (mirrors Zendesk Explore). TTFR metrics (replied_tickets,
-- first_reply_business_minutes_sum) include unresolved tickets so TTFR reflects
-- every replied ticket; rows for unresolved tickets have resolved_date IS NULL.
-- full_resolution_business_minutes_sum is effectively resolved-only because
-- full_resolution_time_business_minutes is NULL for unresolved tickets in the
-- base table.
--
...
SELECT
  ...
  COUNTIF(is_resolved AND rating_category IN ('good', 'bad')) AS rated_surveys,
