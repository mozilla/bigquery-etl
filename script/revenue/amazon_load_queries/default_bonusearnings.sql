SELECT
  ...
  CASE
    ...
    ELSE field_0
  END AS category,
  field_1 AS tracking_id,
  SAFE_CAST(field_2 AS DATE) AS date,
  SAFE_CAST(field_3 AS FLOAT64) AS total_revenue_{{ currency }},
  SAFE_CAST(field_4 AS FLOAT64) AS commission_income_{{ currency }},
  SAFE_CAST(field_5 AS FLOAT64) AS bonus_{{ currency }},
FROM
  {{ table_id }}
WHERE
  {date_filter}
