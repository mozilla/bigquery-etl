...
*/
CREATE OR REPLACE FUNCTION udf.bits28_active_in_range(bits INT64, start_offset INT64, n_bits INT64)
RETURNS BOOLEAN AS (
  CASE
  WHEN
    start_offset > 0
  ...
  ELSE
    BIT_COUNT(udf.bits28_range(bits, start_offset, n_bits)) > 0
  END
);

-- Tests
SELECT
  assert_true(udf.bits28_active_in_range(1 << 10, -13, 7)),
  assert_false(udf.bits28_active_in_range(1 << 10, -6, 7)),
  assert_true(udf.bits28_active_in_range(1, 0, 1)),
  assert_false(udf.bits28_active_in_range(0, 0, 1));
