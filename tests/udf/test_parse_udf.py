...
class TestParseUdf:
    ...
    def test_raw_udf_from_file(self):
        ...
        result = parse_udf.RawUdf.from_file(udf_dir / "bitmask_lowest_28" / "udf.sql")
        assert result.name == "udf.bitmask_lowest_28"
  ...
  0x0FFFFFFF
        ...
        result = parse_udf.RawUdf.from_file(
            udf_dir / "shift_28_bits_one_day" / "udf.sql"
        )
        assert len(result.tests) == 1
        assert (
            "assert_equals(0, udf.shift_28_bits_one_day(1 << 27));" in result.tests[0]
        )
        assert result.dependencies == ["udf.bitmask_lowest_28"]
    ...
    def test_parse_udf(self):
        ...
        assert result.name == "udf.shift_28_bits_one_day"
    ...
    def test_read_udf_dirs(self):
        ...
        assert "udf.shift_28_bits_one_day" in raw_udfs
        assert "udf.safe_crc32_uuid" in raw_udfs
        assert "udf.safe_sample_id" in raw_udfs
        assert "udf.shift_28_bits_one_day"
        assert raw_udfs["udf.shift_28_bits_one_day"].name == "udf.shift_28_bits_one_day"
        assert type(raw_udfs["udf.shift_28_bits_one_day"]) == parse_udf.RawUdf
    ...
    def test_parse_udf_dirs(self):
        ...
        bitmask_lowest_28 = [
            u for u in parsed_udfs if u.name == "udf.bitmask_lowest_28"
        ][0]
        assert type(bitmask_lowest_28) == parse_udf.ParsedUdf

        shift_28_bits_one_day = [
            u for u in parsed_udfs if u.name == "udf.shift_28_bits_one_day"
        ][0]
        assert (
    ...
    def test_accumulate_dependencies(self):
        ...
        result = parse_udf.accumulate_dependencies(
            [], raw_udfs, "udf.shift_28_bits_one_day"
        )
        assert "udf.shift_28_bits_one_day" in result
        assert "udf.bitmask_lowest_28" in result

        result = parse_udf.accumulate_dependencies(
            [], raw_udfs, "udf.bitmask_lowest_28"
        )
        assert "udf.bitmask_lowest_28" in result
    ...
    def test_udf_usage_definitions(self):
        ...
        text = "SELECT udf.bitmask_lowest_28(0), udf.safe_sample_id('')"
    ...
    def test_persistent_udf_as_temp(self):
        ...
        assert "CREATE TEMP FUNCTION udf_bitmask_lowest_28" not in raw_udf
        result = parse_udf.persistent_udf_as_temp(raw_udf, raw_udfs)
        assert "CREATE TEMP FUNCTION udf_shift_28_bits_one_day" in result
        assert "CREATE TEMP FUNCTION udf_bitmask_lowest_28" in result

        text = "SELECT udf.bitmask_lowest_28(23), mozfun.hist.range('{}')"
    ...
    def test_udf_tests_sql(self):
        ...
        raw_udf = parse_udf.RawUdf.from_file(udf_dir / "bitmask_lowest_28" / "udf.sql")
