...
See also: moz-fx-data-shared-prod.udf.extract_histogram_sum.
*/
CREATE OR REPLACE FUNCTION hist.extract_histogram_sum(input STRING)
RETURNS INT64 AS (
  SAFE_CAST(
    COALESCE(
...
extracted AS (
  SELECT
    hist.extract_histogram_sum(h) AS hsum
  FROM
    histogram
  CROSS JOIN
...
--
SELECT
  assert.null(hist.extract_histogram_sum('foo'));
