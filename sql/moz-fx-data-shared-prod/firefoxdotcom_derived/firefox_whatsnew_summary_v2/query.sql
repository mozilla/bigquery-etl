...
all_events_with_page_location_on_WNP AS (
  ...
  --the last source before starting their session
    ...
    page_location,
    TRIM(
      SPLIT(REGEXP_REPLACE(page_location, '^https://www.firefox.com', ''), '/')[SAFE_OFFSET(1)],
      '/'
    ) AS page_location_locale,
    SPLIT(REGEXP_REPLACE(page_location, '^https://www.firefox.com', ''), '/')[
      SAFE_OFFSET(3)
    ] AS page_level_2,
...
SELECT
  ...
  -- There are a few cases where incredibly long value is injected for version,
  -- Any value longer than 18 characters is not a valid version and can be safely dropped.
  mozfun.norm.browser_version_info(
    IF(LENGTH(wnp.page_level_2) >= 18, NULL, wnp.page_level_2)
  ) AS page_level_2_version_info,
  mozfun.norm.browser_version_info(
    IF(LENGTH(wnp.oldversion) >= 18, NULL, wnp.oldversion)
  ) AS old_version_version_info,
  mozfun.norm.browser_version_info(
    IF(LENGTH(wnp.newversion) >= 18, NULL, wnp.newversion)
  ) AS new_version_version_info,
  CASE
