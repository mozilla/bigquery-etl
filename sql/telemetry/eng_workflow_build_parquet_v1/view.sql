CREATE OR REPLACE VIEW
  `moz-fx-data-shared-prod.telemetry.eng_workflow_build_parquet_v1` AS
SELECT
  DATE(submission_timestamp) AS submission_date,
  DATE(submission_timestamp) AS submission_date_s3,
  STRUCT( document_id,
    UNIX_MICROS(submission_timestamp) * 1000 AS `timestamp`,
    metadata.header.date AS `date` ) AS metadata,
  argv,
  build_opts,
  client_id,
  command,
  ...
  STRUCT( system.os,
    ...
    system.virtual_machine) AS system,
  FORMAT_TIMESTAMP('%FT%X', time) AS time,
  exception,
  ARRAY(
  SELECT
    AS STRUCT u.* replace (CAST(count AS int64) AS count)
  FROM
    UNNEST(file_types_changed) AS u) AS file_types_changed,
  build_attrs
FROM
