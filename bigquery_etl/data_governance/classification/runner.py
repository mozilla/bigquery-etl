...
def _sanitized_samples(column: dict[str, Any]) -> list[str]:
    """Return the sample values the model was shown.

    Follows the same rule as the prompt's profile block, cap included. A column
    can carry up to 50 stored values while only MAX_SAMPLE_VALUES are rendered,
    so recording all of them would claim the model saw values it never saw.
    """
    ...
    return {
        ...
        **answer,
        ...
        "evidence": {
            ...
            "sanitized_samples": _sanitized_samples(column),
        },
    }
