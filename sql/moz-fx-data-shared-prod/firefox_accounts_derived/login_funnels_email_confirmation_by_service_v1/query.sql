...
WITH login_success_with_email_by_service_login_view AS (
  SELECT
...
merged_funnels AS (
  SELECT
    COALESCE(login_success_with_email_by_service_login_view_aggregated.service) AS service,
    submission_date,
    funnel,
    COALESCE(login_success_with_email_by_service_login_view_aggregated.aggregated) AS login_view,
    COALESCE(
