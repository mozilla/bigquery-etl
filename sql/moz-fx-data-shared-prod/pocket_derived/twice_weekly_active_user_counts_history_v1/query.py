...
"""Load Pocket TWAU data from GCS."""
import uuid
...
parser.add_argument("--source-prefix", default="twice_weekly_active_user_counts")
parser.add_argument("--destination_dataset", default="pocket_derived")
parser.add_argument(
    "--destination_table", default="twice_weekly_active_user_counts_history_v1"
)


def main():
    """Load data from Parquet to temp table and then query to destination."""
    ...
    client.query(
        ...
        CAST(yearly_cumulative_user_count AS INT64) AS yearly_cumulative_user_count,
        FROM {tmp_table}
