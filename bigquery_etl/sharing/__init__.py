...
`_shared` views that declare an `external_sharing` block in their metadata are
shared with external partners through a BigQuery Sharing data exchange. These
views are published to the user-facing project (mozdata), and the exchange,
listing and subscriber grants are all set up on that copy.
...
def ensure_listing(
    ...
    """Ensure a listing for the given table exists and return its name."""
    from google.cloud import bigquery_analyticshub_v1 as analyticshub
    ...
    table_ref = client.table_path(source_project, source_dataset, source_table)
    bigquery_dataset = analyticshub.Listing.BigQueryDatasetSource(
        dataset=dataset_ref,
        selected_resources=[
            analyticshub.Listing.BigQueryDatasetSource.SelectedResource(table=table_ref)
        ],
    )
...
def publish_table_sharing(
    client, bq_client, metadata, project, dataset, table, dry_run=False
):
    """Reconcile the exchange, listing and subscriber grants for one table.

    The exchange location is derived from the shared dataset's location, since
    BigQuery Sharing requires the exchange to be co-located with the data.
    """
    sharing = metadata.external_sharing
    exchange_id = _sanitize_id(sharing.exchange)
    listing_id = _sanitize_id(f"{dataset}_{table}")
    location = dataset_location(bq_client, project, dataset)

    click.echo(
        f"Deploying external sharing for {project}.{dataset}.{table} in {location}"
    )
    ...
    # Subscribers are granted on the listing so access is scoped per table.
