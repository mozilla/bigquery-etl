...
FROM
  `moz-fx-data-shared-prod.focus_android.usage_reporting_clients_last_seen` AS last_seen
LEFT JOIN
  `moz-fx-data-shared-prod.focus_android.usage_reporting_clients_daily` AS daily
  USING (submission_date, usage_profile_id, normalized_app_id, normalized_channel)
LEFT JOIN
  `moz-fx-data-shared-prod.focus_android.usage_reporting_clients_first_seen` AS first_seen
