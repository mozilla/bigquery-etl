-- Each CTE is broken down into two sections that are unioned
-- as we know only mozilla.org web_referrer has campaigns linked to it.
-- This allows us to combine the campaign and web_referrer datasets together.
CREATE MATERIALIZED VIEW `moz-fx-data-shared-prod.firefox_ios.app_store_funnel_web_referrer`
PARTITION BY
  `date`
CLUSTER BY
  app_id,
...
OPTIONS
  (
    ...
    max_staleness = INTERVAL 12 HOUR
  )
AS
