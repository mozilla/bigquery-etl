"""Pull from BigEye API from User Service and upload to Big Query."""
...
API_URL = "https://app.bigeye.com/api/v1/users"
...
def process_response(response_data: Dict[str, Any]) -> pd.DataFrame:
    ...
    df = pd.json_normalize(
        response_data["users"],
        meta=["id", "name", "email", "idpGroups", "lastLoginAt", "pictureUrl"],
        record_path="groups",
        meta_prefix="user_",
        errors="ignore",
    )
...
def main() -> None:
    ...
    parser.add_argument("--table", default="user_service_v1")
