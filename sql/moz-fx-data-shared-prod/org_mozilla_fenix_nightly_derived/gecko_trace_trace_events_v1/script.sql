...
MERGE INTO
  `moz-fx-data-shared-prod.org_mozilla_fenix_nightly_derived.gecko_trace_trace_events_v1` AS T
  USING (
    WITH RECURSIVE
...
-- Included by traces_v1 and trace_events_v1 scripts.
    ...
    new_trace_events AS (
      ...
      JOIN
        `moz-fx-data-shared-prod.org_mozilla_fenix_nightly_derived.gecko_trace_events_v1` ev
...
THEN
  INSERT
    (submission_date, stable_trace_id, stable_event_id, hit_count, event_position)
  VALUES
    (S.submission_date, S.stable_trace_id, S.stable_event_id, S.hit_count, S.event_position);
