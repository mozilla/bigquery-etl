...
SELECT
  ...
  cls.attribution_experiment AS attribution_experiment,
  au.attribution_medium,
  au.attribution_source,
  cls.attribution_variation AS attribution_variation,
  au.city,
  au.country,
  cls.device_model,
  au.distribution_id,
  ...
  bcls.normalized_channel,
  au.os AS normalized_os,
  bcls.normalized_os_version,
  COALESCE(
    SAFE_CAST(NULLIF(SPLIT(bcls.normalized_os_version, ".")[SAFE_OFFSET(0)], "") AS INTEGER),
    0
  ) AS os_version_major,
  COALESCE(
    SAFE_CAST(NULLIF(SPLIT(bcls.normalized_os_version, ".")[SAFE_OFFSET(1)], "") AS INTEGER),
    0
  ) AS os_version_minor,
...
LEFT JOIN
  `moz-fx-data-shared-prod.firefox_desktop_derived.clients_last_seen_joined_v1` cls
  USING (client_id, submission_date)
LEFT JOIN
  `moz-fx-data-shared-prod.firefox_desktop.baseline_clients_last_seen` bcls
  USING (client_id, submission_date)
WHERE
