{% for funnel_name, funnel in funnels %}
...
{{ funnel_name }}_{{ step_name }} AS (
  SELECT
    ...
    {{ steps[step_name].column }} AS column
  FROM 
    {{ data_sources[steps[step_name]["data_source"]]["select_expression"] }}
...
{{ funnel_name }}_{{ step_name }}_aggregated AS (
  SELECT
    ...
    {{ steps[step_name]["aggregation"] }}(column) AS aggregated
  FROM
