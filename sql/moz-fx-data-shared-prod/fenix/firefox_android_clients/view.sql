...
SELECT
  CASE
    WHEN first_seen_date IS NULL
      AND metadata.min_first_session_ping_submission_date IS NULL
      THEN metadata.min_metrics_ping_submission_date
    WHEN first_seen_date IS NULL
      AND metadata.min_first_session_ping_submission_date IS NOT NULL
      THEN metadata.min_first_session_ping_submission_date
