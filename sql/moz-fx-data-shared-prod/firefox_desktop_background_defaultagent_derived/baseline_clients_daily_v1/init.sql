...
CREATE TABLE IF NOT EXISTS
  `firefox_desktop_background_defaultagent_derived.baseline_clients_daily_v1`
PARTITION BY
  submission_date
CLUSTER BY
  normalized_channel,
...
joined AS (
  ...
  FROM
    windowed AS cd
  LEFT JOIN
    `firefox_desktop_background_defaultagent_derived.baseline_clients_first_seen_v1` AS cfs
  USING
    (client_id)
  WHERE
    _n = 1
