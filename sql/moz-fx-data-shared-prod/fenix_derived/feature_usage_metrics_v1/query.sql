...
(SELECT
    ...
    --Privacy
    COALESCE(SUM(CASE WHEN metrics.string.preferences_enhanced_tracking_protection = 'standard' THEN 1 ELSE 0 END), 0) AS etp_standard,
    COALESCE(SUM(CASE WHEN metrics.string.preferences_enhanced_tracking_protection = 'strict' THEN 1 ELSE 0 END), 0) AS etp_strict,
    COALESCE(SUM(CASE WHEN metrics.string.preferences_enhanced_tracking_protection = 'custom' THEN 1 ELSE 0 END), 0) AS etp_custom,
    COALESCE(SUM(CASE WHEN metrics.string.preferences_enhanced_tracking_protection NOT IN ('custom', 'standard', 'strict') THEN 1 ELSE 0 END), 0) AS etp_disabled,
    ...
    --Awesomebar Location
    COALESCE(SUM(CASE WHEN metrics.string.preferences_toolbar_position_setting in ('top', 'fixed_top') THEN 1 ELSE 0 END), 0) AS awesomebar_top,
    COALESCE(SUM(CASE WHEN metrics.string.preferences_toolbar_position_setting = 'bottom' THEN 1 ELSE 0 END), 0) AS awesomebar_bottom,
    COALESCE(SUM(CASE WHEN metrics.string.preferences_toolbar_position_setting not in ('top', 'fixed_top', 'bottom') THEN 1 ELSE 0 END), 0) AS awesomebar_null,
...
(SELECT
    ...
    COUNT(DISTINCT CASE WHEN currently_stored_logins > 0 THEN client_id END) AS currently_stored_logins_users,
    COALESCE(SUM(currently_stored_logins), 0) AS currently_stored_logins,
    
    /*Credit Card*/
    --credit card deleted
    COUNT(DISTINCT CASE WHEN credit_cards_deleted > 0 THEN client_id END) AS credit_cards_deleted_users,
    COALESCE(SUM(credit_cards_deleted), 0) AS credit_cards_deleted,
    --credit card currently stored
    COUNT(DISTINCT CASE WHEN currently_stored_credit_cards > 0 THEN client_id END) AS currently_stored_credit_cards_users,
    ...
    COUNT(DISTINCT CASE WHEN currently_stored_addresses > 0 THEN client_id END) AS currently_stored_addresses_users,
    ...
    COUNT(DISTINCT CASE WHEN metrics_desktop_bookmarks_count > 0 THEN client_id END) AS metrics_desktop_bookmarks_count_users,
    COALESCE(SUM(metrics_desktop_bookmarks_count), 0) AS metrics_desktop_bookmarks_count,
    --metrics_mobile_bookmarks_count
    COUNT(DISTINCT CASE WHEN metrics_mobile_bookmarks_count > 0 THEN client_id END) AS metrics_mobile_bookmarks_count_users,
    COALESCE(SUM(metrics_mobile_bookmarks_count), 0) AS metrics_mobile_bookmarks_count,
    --metrics_has_desktop_bookmarks
    COUNT(DISTINCT CASE WHEN metrics_has_desktop_bookmarks > 0 THEN client_id END) AS metrics_has_desktop_bookmarks_users,
    COALESCE(SUM(metrics_has_desktop_bookmarks), 0) AS metrics_has_desktop_bookmarks,
    --metrics_has_mobile_bookmarks
    COUNT(DISTINCT CASE WHEN metrics_has_mobile_bookmarks > 0 THEN client_id END) AS metrics_has_mobile_bookmarks_users,
    ...
    COUNT(DISTINCT CASE WHEN metrics_private_tabs_open_count > 0 THEN client_id END) AS metrics_private_tabs_open_count_users,
    COALESCE(SUM(metrics_private_tabs_open_count), 0) AS metrics_private_tabs_open_count,
    --metrics_tabs_open_count
    COUNT(DISTINCT CASE WHEN metrics_tabs_open_count > 0 THEN client_id END) AS metrics_tabs_open_count_users,
    COALESCE(SUM(metrics_tabs_open_count), 0) AS metrics_tabs_open_count,

    /*Default Browser*/
    --metrics_default_browser
    COUNT(DISTINCT CASE WHEN metrics_default_browser > 0 THEN client_id END) AS metrics_default_browser_users,
    ...
    COUNT(DISTINCT CASE WHEN metrics_notifications_allowed > 0 THEN client_id END) AS metrics_notifications_allowed_users,
    COALESCE(SUM(metrics_notifications_allowed), 0) AS metrics_notifications_allowed,
    --events_marketing_notification_allowed
    COUNT(DISTINCT CASE WHEN events_marketing_notification_allowed > 0 THEN client_id END) AS events_marketing_notification_allowed_users,
    COALESCE(SUM(events_marketing_notification_allowed), 0) AS events_marketing_notification_allowed,

    /*Customize Homepage*/
    --customize_home_contile
    COUNT(DISTINCT CASE WHEN customize_home_contile > 0 THEN client_id END) AS customize_home_contile_users,
    COALESCE(SUM(customize_home_contile), 0) AS customize_home_contile,
    --customize_home_jump_back_in_users
    COUNT(DISTINCT CASE WHEN customize_home_jump_back_in > 0 THEN client_id END) AS customize_home_jump_back_in_users,
    COALESCE(SUM(customize_home_jump_back_in), 0) AS customize_home_jump_back_in,
    --customize_home_most_visited_sites_users
    COUNT(DISTINCT CASE WHEN customize_home_most_visited_sites > 0 THEN client_id END) AS customize_home_most_visited_sites_users,
    COALESCE(SUM(customize_home_most_visited_sites), 0) AS customize_home_most_visited_sites,
    --customize_home_pocket_users
    COUNT(DISTINCT CASE WHEN customize_home_pocket > 0 THEN client_id END) AS customize_home_pocket_users,
    COALESCE(SUM(customize_home_pocket), 0) AS customize_home_pocket,
    --customize_home_recently_saved_users
    COUNT(DISTINCT CASE WHEN customize_home_recently_saved > 0 THEN client_id END) AS customize_home_recently_saved_users,
    COALESCE(SUM(customize_home_recently_saved), 0) AS customize_home_recently_saved,
    --customize_home_recently_visited_users
    COUNT(DISTINCT CASE WHEN customize_home_recently_visited > 0 THEN client_id END) AS customize_home_recently_visited_users,
