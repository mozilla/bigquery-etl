WITH _previous AS (
  ...
  FROM
    `moz-fx-data-shared-prod.thunderbird_crashreporter_derived.metrics_clients_last_seen_v1`
  WHERE
...
_current AS (
  SELECT
    *
  FROM
    `moz-fx-data-shared-prod.thunderbird_crashreporter.metrics_clients_daily` AS m
  WHERE
