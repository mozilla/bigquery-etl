...
from ..metadata.parse_metadata import DATASET_METADATA_FILE, DatasetMetadata
    ...
    for dep_file in view_dependency_files:
        ...
        if dep_file.name == VIEW_FILE:
            ...
            for dependency in view.table_references:
                ...
                # if no local file exists, check if we should create a stub
                if not file_exists_for_dependency:
                    # skip creating stubs for: live tables, stable tables, syndication datasets
                    should_skip = False

                    if any(dataset.endswith(suffix) for suffix in ("_live", "_stable")):
                        should_skip = True
                    else:
                        # check if dataset is used for syndication
                        dataset_metadata_path = (
                            Path(sql_dir) / project / dataset / DATASET_METADATA_FILE
                        )
                        if dataset_metadata_path.exists():
                            dataset_metadata = DatasetMetadata.from_file(
                                str(dataset_metadata_path)
                            )
                            should_skip = (
                                dataset_metadata.dataset_base_acl == "syndicate"
                            )

                    # only create stub if we shouldn't skip
                    if not should_skip:
                        ...
                        if not path.exists():
                            ...
                            if "*" not in name and name != "INFORMATION_SCHEMA":
                                ...
                                # create query.py stub since we have a schema
