...
def generate_query_with_shredder_mitigation(
    ...
    if not sample_rows:
        sample_rows = new.get_query_path_results(backfill_date=backfill_date, rows=1)
    try:
    ...
    shredded_select = (
        ...
        # This doesn't convert actual data or dtypes. It's only use to cast NULLs so that data can be UNIONED later.
        + [
    ...
    shredded_join = " AND ".join(
        ...
        + [
            ...
            f" ({previous_agg.query_cte}.{dim.name} IS NULL AND {new_agg.query_cte}.{dim.name} IS NULL))"  # Compare null values.
            for dim in common_dimensions
            if dim.name != previous.partitioning["field"]
            and dim.data_type in (DataTypeGroup.BOOLEAN, DataTypeGroup.DATE)
        ]
