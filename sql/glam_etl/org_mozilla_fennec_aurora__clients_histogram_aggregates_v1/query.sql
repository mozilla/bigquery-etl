-- query for org_mozilla_fennec_aurora__clients_histogram_aggregates_v1;
...
WITH extracted_accumulated AS (
  SELECT
    *
  FROM
    glam_etl.org_mozilla_fennec_aurora__clients_histogram_aggregates_v1
  WHERE
...
filtered_accumulated AS (
  ...
  LEFT JOIN
    glam_etl.org_mozilla_fennec_aurora__latest_versions_v1
  USING
    (channel)
  WHERE
...
extracted_daily AS (
  ...
  FROM
    glam_etl.org_mozilla_fennec_aurora__view_clients_daily_histogram_aggregates_v1,
