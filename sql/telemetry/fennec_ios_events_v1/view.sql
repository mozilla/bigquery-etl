        ...
        (
          ...
          SELECT
            ...
            WHEN
              event_name = 'Fennec-iOS - Setting changed'
            THEN
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'to')
            WHEN
          ...
          SELECT
            ...
            WHEN
              event_name = 'Fennec-iOS - Prompt to translate tab to user\'s native language'
            THEN
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'from')
            WHEN
              event_name = 'Fennec-iOS - Accept or decline offer to translate tab'
            THEN
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'from')
          ...
          SELECT
            ...
            WHEN
              event_name = 'Fennec-iOS - Accept or decline offer to translate tab'
            THEN
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'action')
            END
            AS value
          UNION ALL
          SELECT
            ...
            THEN
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'slide-num')
            WHEN
