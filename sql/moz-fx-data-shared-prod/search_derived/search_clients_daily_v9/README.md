...
- SAP produces `sap_counts_total` (a count of `sap.counts` events) plus session and engagement measures, and derives `profile_age_in_days` from `ping_info.parsed_start_time` against the first run date.
