...
SELECT
  ...
  COALESCE(funnel_last_year.visits, 0) AS visits_last_year,
  COALESCE(funnel_last_year.downloads, 0) AS downloads_last_year,
  COALESCE(funnel_last_year.installs, 0) AS installs_last_year,
  COALESCE(funnel_last_year.new_profiles, 0) AS new_profiles_last_year,
  COALESCE(funnel_last_year.return_user, 0) AS return_user_last_year,
  COALESCE(funnel_last_year.retained_week4, 0) AS retained_week4_last_year
FROM
...
LEFT JOIN
  `moz-fx-data-shared-prod.telemetry.firefox_desktop_marketing_funnel` AS funnel_last_year
  ON funnel.funnel_date = funnel_last_year.funnel_date + INTERVAL 364 DAY
