...
_grouped AS (
  ...
  SELECT
    ...
    sample_id,
    -- Dimensional data
    `moz-fx-data-shared-prod.udf.mode_last`(ARRAY_AGG(app_name)) AS app_name,
    `moz-fx-data-shared-prod.udf.mode_last`(ARRAY_AGG(normalized_app_name)) AS normalized_app_name,
...
_current AS (
  SELECT
    *,
    ...
    `moz-fx-data-shared-prod.udf.int_to_365_bits`(
