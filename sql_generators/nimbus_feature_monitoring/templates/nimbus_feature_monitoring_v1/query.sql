...
{% for source_table in source_tables %}
  source_table_cte_{{loop.index}} AS (
    SELECT
      {{ source_table.unique_id }} AS unique_id,
      {% if loop.first %}
      ARRAY_AGG(DISTINCT rollout_slug) AS rollout_slugs,
      {% endif %}
      {% if source_table.dimension_aggregators != ["last"] %}
        (
          SELECT AS STRUCT
      {% endif %}
      {% if "last" in source_table.dimension_aggregators %}
        ARRAY_AGG(
          STRUCT(
            {% for dimension in table_dimensions %}
              {% if dimension.aggregator == "last" %}
                {{ dimension.field }} AS {{ dimension.name }}{{ "," if not loop.last }}
              {% endif %}
            {% endfor %}
          )
        ...
        {% endif %}
      {% endif %}
      {% for dimension in source_table.dimensions %}
        {% if dimension.aggregator != "last" %}
          {{ dimension.aggregator }}({{ dimension.field }}) AS {{ dimension.name }},
        {% endif %}
      {% endfor %}
      {% if source_table.dimension_aggregators != ["last"] %}
        )
      ...
      AS dimensions,
      STRUCT(
        {% for metric in feature.metrics[source_table.name] %}
          {{ metric.ping_aggregator }}(
            {% if metric.type_.startswith("labeled_") %}
              (
