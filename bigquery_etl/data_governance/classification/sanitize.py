...
def sanitizer_for(config: ClassificationConfig) -> Sanitizer | None:
    """Build the Sanitizer for a run, or None when sanitization is disabled."""
    if not config.sanitize:
        logging.warning(
            "Sanitization disabled: raw sample values will be sent to the LLM, and "
            "evidence.sanitized_samples will be left empty rather than holding them."
        )
        return None
    return Sanitizer(
