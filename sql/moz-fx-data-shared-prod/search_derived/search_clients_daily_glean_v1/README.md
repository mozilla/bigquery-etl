...
Most of those prefixes now reach the output rather than being stripped in `final_cte`, so a consumer can tell from the name alone which side a value can come from, and therefore which columns are zero or `null` on a row that side never reached. The ones still renamed in `final_cte` are the SERP search and ad-click measures that carry a v8 counterpart, where the v8-facing name was judged more useful than the provenance. The prefix distinguishes each counter from the SERP-derived measure of the same events, which is published under its own name.
...
- The ad measures name which half they count, rather than leaving the tagged half as the unmarked default the way v8 did. v8's `ad_click` is `ad_click_total` and `ad_clicks_tagged` is `ad_click_tagged`, beside `ad_click_organic`. v8's `search_with_ads`, which silently meant the tagged half, is `serp_searches_with_ads_tagged_count`, beside `serp_searches_with_ads_organic_count`. Note that `ad_click_total` is not guaranteed to equal `ad_click_tagged` plus `ad_click_organic`: the two halves are split on `is_tagged is true` and `is_tagged is false`, both of which exclude `null`, while the total sums every row. They agree only where `is_tagged` is never `null`.
