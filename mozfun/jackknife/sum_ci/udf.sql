...
SELECT
  submission_date,
  jackknife.sum_ci(ARRAY_AGG(dau_sum)).*
FROM
  bucketed
GROUP BY
  submission_date
...
*/
CREATE OR REPLACE FUNCTION jackknife.sum_ci(n_buckets INT64, counts_per_bucket ARRAY<INT64>)
...
SELECT
  ...
  assert_equals(5, jackknife.sum_ci(20, [5]).total)
