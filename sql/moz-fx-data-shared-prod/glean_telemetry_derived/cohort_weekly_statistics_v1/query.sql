...
initial_cohort_counts AS (
  ...
  FROM
    `moz-fx-data-shared-prod.glean_telemetry_derived.cohort_weekly_cfs_staging_v1`
  GROUP BY
    normalized_app_name,
    normalized_channel,
    app_version,
...
weekly_active_agg AS (
  ...
  FROM
    `moz-fx-data-shared-prod.glean_telemetry_derived.cohort_weekly_cfs_staging_v1` cfs
  JOIN
    `moz-fx-data-shared-prod.glean_telemetry_derived.cohort_weekly_active_clients_v1` wac
