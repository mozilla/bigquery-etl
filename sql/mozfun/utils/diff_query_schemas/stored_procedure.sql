...
BEGIN
  ...
  EXECUTE IMMEDIATE
    "SELECT ARRAY_AGG(STRUCT(COLUMN_NAME, ORDINAL_POSITION, DATA_TYPE)) FROM mozdata.tmp.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = @table_name"
    INTO query_a_schema
  ...
  -- Cleanup
  EXECUTE IMMEDIATE
    "DROP TABLE " || table_a_id;

  EXECUTE IMMEDIATE
    "DROP TABLE " || table_b_id;
END;
