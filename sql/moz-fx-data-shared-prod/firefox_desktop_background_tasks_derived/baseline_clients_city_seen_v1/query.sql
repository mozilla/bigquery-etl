...
WITH base_firefox_desktop_background_tasks AS (
...
overactive_firefox_desktop_background_tasks AS (
  ...
  SELECT
    ...
    client_id
  FROM
    base_firefox_desktop_background_tasks
  WHERE
    {% if is_init() %}
...
clients_daily_firefox_desktop_background_tasks AS (
  SELECT
  ...
  FROM
    base_firefox_desktop_background_tasks
  LEFT JOIN
    overactive_firefox_desktop_background_tasks
    USING (submission_date, client_id)
  WHERE
    overactive_firefox_desktop_background_tasks.client_id IS NULL
...
{% if is_init() %}
  clients_city_first_seen_firefox_desktop_background_tasks AS (
    SELECT
      app_id,
      client_id,
      sample_id,
    ...
    FROM
      clients_daily_firefox_desktop_background_tasks
    WHERE
      geo.city IS NOT NULL
    QUALIFY
      ROW_NUMBER() OVER (PARTITION BY client_id, sample_id ORDER BY submission_date) = 1
  ),
  clients_city_last_seen_firefox_desktop_background_tasks AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% else %}
  _previous_firefox_desktop_background_tasks AS (
    SELECT
      *
    FROM
      `moz-fx-data-shared-prod.firefox_desktop_background_tasks_derived.baseline_clients_city_seen_v1`
    WHERE
      app_id = "firefox_desktop_background_tasks"
  ),
  _current_firefox_desktop_background_tasks AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% if is_init() %}
  SELECT
    ...
    last_seen_country
  FROM
    clients_city_first_seen_firefox_desktop_background_tasks cfs
  FULL OUTER JOIN
    clients_city_last_seen_firefox_desktop_background_tasks cls
...
-- _p.* fields are NULL for clients that are not yet captured in the baseline_city_seen derived table.
  ...
  FROM
    _current_firefox_desktop_background_tasks AS _c
  FULL JOIN
    _previous_firefox_desktop_background_tasks AS _p
    USING (client_id, sample_id, app_id)
{% endif %}
