WITH
  ...
  visit_metadata AS (
  SELECT
    mozfun.map.get_key(event_details,
      "newtab_visit_id") AS newtab_visit_id,
    submission_date,
    ...
    ANY_VALUE( metrics.quantity.topsites_sponsored_tiles_configured ) AS topsites_sponsored_tiles_configured,
    ...
    MIN(
    IF
      (event_name = "opened", event_timestamp, NULL)) AS newtab_visit_started_at,
    MIN(
    IF
      (event_name = "closed", event_timestamp, NULL)) AS newtab_visit_ended_at,
    ANY_VALUE(
    IF
      (event_name = "opened", mozfun.map.get_key(event_details,
          "source"), NULL) ) AS newtab_open_source,
    LOGICAL_OR(event_name IN ("click",
        "issued",
        "save")) AS had_non_impression_engagement,
    LOGICAL_OR(event_name IN ("click",
        "save")) AS had_non_search_engagement,
    ANY_VALUE(metrics.string_list.newtab_selected_topics) AS newtab_selected_topics,
    ANY_VALUE(
    IF
      ( event_name = "opened", SAFE_CAST(mozfun.map.get_key(event_details,
            "window_inner_height") AS INT), NULL ) ) AS newtab_window_inner_height,
    ANY_VALUE(
    IF
      ( event_name = "opened", SAFE_CAST(mozfun.map.get_key(event_details,
            "window_inner_width") AS INT), NULL ) ) AS newtab_window_inner_width,
  FROM
    events_unnested
  GROUP BY
    newtab_visit_id,
  ...
  SELECT
    mozfun.map.get_key(event_details,
      "newtab_visit_id") AS newtab_visit_id,
    `moz-fx-data-shared-prod`.udf.normalize_search_engine( mozfun.map.get_key(event_details,
        "telemetry_id") ) AS search_engine,
    mozfun.map.get_key(event_details,
      "search_access_point") AS search_access_point,
    COUNTIF(event_name = "issued") AS searches,
    COUNTIF( event_name = "impression"
      AND mozfun.map.get_key(event_details,
        "is_tagged") = "true" ) AS tagged_search_ad_impressions,
    COUNTIF( event_name = "impression"
      AND mozfun.map.get_key(event_details,
        "is_follow_on") = "true" ) AS follow_on_search_ad_impressions,
    COUNTIF( event_name = "impression"
      AND mozfun.map.get_key(event_details,
        "is_follow_on") = "true"
      AND mozfun.map.get_key(event_details,
        "is_tagged") = "true" ) AS tagged_follow_on_search_ad_impressions,
    COUNTIF( event_name = "click"
      AND mozfun.map.get_key(event_details,
        "is_tagged") = "true" ) AS tagged_search_ad_clicks,
    COUNTIF( event_name = "click"
      AND mozfun.map.get_key(event_details,
        "is_follow_on") = "true" ) AS follow_on_search_ad_clicks,
    COUNTIF( event_name = "click"
      AND mozfun.map.get_key(event_details,
        "is_follow_on") = "true"
      AND mozfun.map.get_key(event_details,
        "is_tagged") = "true" ) AS tagged_follow_on_search_ad_clicks
  FROM
    events_unnested
  WHERE
    event_category IN ('newtab.search',
      'newtab.search.ad')
  ...
  SELECT
    newtab_visit_id,
    ARRAY_AGG( STRUCT( search_engine,
        search_access_point,
        searches,
        ...
        tagged_follow_on_search_ad_impressions ) ) AS search_interactions
  FROM
    search_events
  GROUP BY
    newtab_visit_id ),
  ...
  SELECT
    mozfun.map.get_key(event_details,
      "newtab_visit_id") AS newtab_visit_id,
    SAFE_CAST(mozfun.map.get_key(event_details,
        "position") AS INT64) AS topsite_tile_position,
    mozfun.map.get_key(event_details,
      "advertiser_name") AS topsite_tile_advertiser_name,
    mozfun.map.get_key(event_details,
      "tile_id") AS topsite_tile_id,
    ...
    COUNTIF( event_name = 'impression'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_topsite_tile_impressions,
    COUNTIF( event_name = 'impression'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "false" ) AS organic_topsite_tile_impressions,
    COUNTIF( event_name = 'click'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_topsite_tile_clicks,
    COUNTIF( event_name = 'click'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "false" ) AS organic_topsite_tile_clicks,
    COUNTIF(event_name = 'dismiss') AS topsite_tile_dismissals,
    COUNTIF( event_name = 'dismiss'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_topsite_tile_dismissals,
    COUNTIF( event_name = 'dismiss'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "false" ) AS organic_topsite_tile_dismissals,
  FROM
    events_unnested
  LEFT JOIN
  ...
  ON
    SAFE_CAST(mozfun.map.get_key(event_details,
        "position") AS INT64) = SAFE_CAST( JSON_EXTRACT(sov, "$.pos") AS INT64 )
  WHERE
  ...
  SELECT
    newtab_visit_id,
    ARRAY_AGG( STRUCT( topsite_tile_advertiser_name,
        topsite_tile_position,
        topsite_tile_id,
        topsite_tile_assigned_sov_branch,
        topsite_tile_displayed_sov_branch,
        ...
        organic_topsite_tile_dismissals ) ) AS topsite_tile_interactions
  FROM
    topsites_events
  GROUP BY
    newtab_visit_id ),
  ...
  SELECT
    mozfun.map.get_key(event_details,
      "newtab_visit_id") AS newtab_visit_id,
    SAFE_CAST(mozfun.map.get_key(event_details,
        "position") AS INT64) AS pocket_story_position,
    mozfun.map.get_key(event_details,
      "tile_id") AS pocket_tile_id,
    mozfun.map.get_key(event_details,
      "recommendation_id") AS pocket_recommendation_id,
    COUNTIF( event_name = 'save'
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS pocket_saves,
    COUNTIF( event_name = 'click'
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS pocket_clicks,
    COUNTIF( event_name = 'impression'
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS pocket_impressions,
    COUNTIF( event_name = 'click'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS sponsored_pocket_clicks,
    COUNTIF( event_name = 'click'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS organic_pocket_clicks,
    COUNTIF( event_name = 'impression'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS sponsored_pocket_impressions,
    COUNTIF( event_name = 'impression'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS organic_pocket_impressions,
    COUNTIF( event_name = 'save'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS sponsored_pocket_saves,
    COUNTIF( event_name = 'save'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS organic_pocket_saves,
    COUNTIF( event_name = 'dismiss'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS sponsored_pocket_dismissals,
    COUNTIF( event_name = 'dismiss'
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true"
      AND COALESCE(mozfun.map.get_key(event_details,
          "is_list_card"), "false") = "false" ) AS organic_pocket_dismissals,
    COUNTIF( event_name = 'thumb_voting_interaction'
      AND mozfun.map.get_key(event_details,
        "thumbs_up") = "true" ) AS pocket_thumbs_up,
    COUNTIF( event_name = 'thumb_voting_interaction'
      AND mozfun.map.get_key(event_details,
        "thumbs_down") = "true" ) AS pocket_thumbs_down,
    SAFE_CAST(mozfun.map.get_key(event_details,
        "received_rank") AS INT) AS pocket_received_rank,
    mozfun.map.get_key( event_details,
      "scheduled_corpus_item_id" ) AS pocket_scheduled_corpus_item_id,
    mozfun.map.get_key(event_details,
      "topic") AS pocket_topic,
    mozfun.map.get_key(event_details,
      "matches_selected_topic") AS pocket_matches_selected_topic,
    COUNTIF( event_name = "click"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true" ) AS list_card_clicks,
    COUNTIF( event_name = "click"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true" ) AS organic_list_card_clicks,
    COUNTIF( event_name = "click"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_list_card_clicks,
    COUNTIF( event_name = "impression"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true" ) AS list_card_impressions,
    COUNTIF( event_name = "impression"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true" ) AS organic_list_card_impressions,
    COUNTIF( event_name = "impression"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_list_card_impressions,
    COUNTIF( event_name = "save"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true" ) AS list_card_saves,
    COUNTIF( event_name = "save"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true" ) AS organic_list_card_saves,
    COUNTIF( event_name = "save"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_list_card_saves,
    COUNTIF( event_name = "dismiss"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true" ) AS list_card_dismissals,
    COUNTIF( event_name = "dismiss"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") != "true" ) AS organic_list_card_dismissals,
    COUNTIF( event_name = "dismiss"
      AND mozfun.map.get_key(event_details,
        "is_list_card") = "true"
      AND mozfun.map.get_key(event_details,
        "is_sponsored") = "true" ) AS sponsored_list_card_dismissals,
  FROM
    events_unnested
  WHERE
  ...
  SELECT
    newtab_visit_id,
    ARRAY_AGG( STRUCT( pocket_story_position,
        pocket_tile_id,
        pocket_recommendation_id,
        pocket_impressions,
        ...
        sponsored_list_card_dismissals ) ) AS pocket_interactions
  FROM
    pocket_events
  GROUP BY
    newtab_visit_id ),
  ...
  SELECT
    mozfun.map.get_key(event_details,
      "newtab_visit_id") AS newtab_visit_id,
    mozfun.map.get_key(event_details,
      "selected_wallpaper") AS wallpaper_selected_wallpaper,
    COUNTIF(event_name = 'wallpaper_click') AS wallpaper_clicks,
    COUNTIF( event_name = 'wallpaper_click'
      AND mozfun.map.get_key(event_details,
        "had_previous_wallpaper") = "true" ) AS wallpaper_clicks_had_previous_wallpaper,
    COUNTIF( event_name = 'wallpaper_click'
      AND mozfun.map.get_key(event_details,
        "had_previous_wallpaper") = "false" ) AS wallpaper_clicks_first_selected_wallpaper,
  ...
  WHERE
    ...
    AND event_name IN ( 'wallpaper_click',
  ...
  SELECT
    newtab_visit_id,
    ARRAY_AGG( STRUCT( wallpaper_selected_wallpaper,
        wallpaper_clicks,
        ...
        wallpaper_highlight_cta_clicks ) ) AS wallpaper_interactions
  FROM
    wallpaper_events
  GROUP BY
    newtab_visit_id ),
  ...
  SELECT
    ...
    COUNTIF( event_name = 'weather_change_display'
      AND mozfun.map.get_key(event_details,
        "weather_display_mode") = "detailed" ) AS weather_widget_change_display_to_detailed,
    COUNTIF( event_name = 'weather_change_display'
      AND mozfun.map.get_key(event_details,
        "weather_display_mode") = "simple" ) AS weather_widget_change_display_to_simple
  FROM
    events_unnested
  WHERE
    event_category = 'newtab'
    AND event_name IN ( 'weather_impression',
  ...
  SELECT
    mozfun.map.get_key(event_details,
      "newtab_visit_id") AS newtab_visit_id,
    mozfun.map.get_key(event_details,
      "previous_topics") AS previous_topics,
    mozfun.map.get_key(event_details,
      "topics") AS topics,
    ...
    COUNTIF( event_name = 'topic_selection_topics_saved'
      AND mozfun.map.get_key(event_details,
        "first_save") = "true" ) AS topic_selection_topics_first_saved,
    COUNTIF( event_name = 'topic_selection_topics_saved'
      AND mozfun.map.get_key(event_details,
        "first_save") != "true" ) AS topic_selection_topics_updated,
  FROM
    events_unnested
  WHERE
    event_category = 'newtab'
    AND event_name IN ( 'topic_selection_dismiss',
  ...
  SELECT
    newtab_visit_id,
    ARRAY_AGG( STRUCT( previous_topics,
        topics,
        topic_selection_open,
        topic_selection_dismiss,
        topic_selection_topics_first_saved,
        topic_selection_topics_updated ) ) AS topic_selection_interactions
  FROM
...
SELECT
  *,
  CASE
    WHEN ( (newtab_open_source = "about:home" AND newtab_homepage_category = "enabled") OR (newtab_open_source = "about:newtab" AND newtab_newtab_category = "enabled") ) THEN "default"
    ELSE "non-default"
END
  AS newtab_default_ui,
FROM
