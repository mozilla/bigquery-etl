...
WITH base AS (
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_social_nightly_stable.baseline_v1`
...
joined AS (
  ...
  FROM
    windowed AS cd
  LEFT JOIN
    `moz-fx-data-shared-prod.org_mozilla_social_nightly_derived.baseline_clients_first_seen_v1` AS cfs
    USING (client_id)
  WHERE
    _n = 1
)
