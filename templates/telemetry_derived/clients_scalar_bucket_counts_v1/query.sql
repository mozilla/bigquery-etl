...
bucketed_scalars AS (
  SELECT
    * EXCEPT(value),
    "linear" AS bucket_type,
    SAFE_CAST(udf_linear_bucket(value) AS STRING) AS bucket
  FROM scalars
  ...
  SELECT
    * EXCEPT(value),
    "exponential" AS bucket_type,
    SAFE_CAST(udf_exponential_bucket(value) AS STRING) AS bucket
  FROM scalars
),
