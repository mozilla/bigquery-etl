...
firefox_first_run AS (
  SELECT
  client_id AS telemetry_client_id,
  TRUE AS did_firefox_first_run
  FROM
  `moz-fx-data-shared-prod.telemetry_derived.clients_daily_v6`
  WHERE was_active is true 
  GROUP BY 1,2 
  HAVING min(submission_date) = @submission_date
) ,
...
firefox_first_ad_click AS (
  SELECT
  client_id AS telemetry_client_id,
  TRUE AS did_click_ad
  FROM
  `moz-fx-data-shared-prod.telemetry_derived.clients_daily_v6`
  WHERE IFNULL(ad_clicks_count_all, 0) > 0
  GROUP BY 1 ,2 
  HAVING min(submission_date) = @submission_date
...
firefox_first_search AS (
  ...
  FROM
  `moz-fx-data-shared-prod.telemetry_derived.clients_daily_v6`
  WHERE was_active is true --fix this
  GROUP BY 1 
  HAVING min(submission_date) = @submission_date
...
telemetry_id_to_activity AS (
  SELECT
    ...
    MAX(COALESCE(did_firefox_first_run, FALSE)) AS did_firefox_first_run
  FROM
    telemetry_id_to_activity_staging a 
    LEFT JOIN 
    firefox_first_run b 
...
SELECT
  activity_date,
  gclid,
  MAX(did_firefox_first_run) AS did_firefox_first_run,
