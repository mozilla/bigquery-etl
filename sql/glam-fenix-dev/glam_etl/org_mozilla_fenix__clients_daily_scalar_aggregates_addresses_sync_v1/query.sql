-- Query generated by: python3 -m bigquery_etl.glam.clients_daily_scalar_aggregates --source-table org_mozilla_fenix_stable.addresses_sync_v1
WITH extracted AS (
  SELECT
    *,
    DATE(submission_timestamp) AS submission_date,
    client_info.client_id,
    "addresses-sync" AS ping_type,
    COALESCE(
...
unlabeled_metrics AS (
  SELECT
    ...
    ARRAY<STRUCT<metric STRING, metric_type STRING, key STRING, agg_type STRING, value FLOAT64>>[
      (
        'addresses_sync_outgoing_batches',
        'counter',
        '',
        'avg',
        AVG(CAST(metrics.counter.addresses_sync_outgoing_batches AS INT64))
      ),
      (
        'addresses_sync_outgoing_batches',
        'counter',
        '',
        'count',
        IF(MIN(metrics.counter.addresses_sync_outgoing_batches) IS NULL, NULL, COUNT(*))
      ),
      (
        'addresses_sync_outgoing_batches',
        'counter',
        '',
        'max',
        MAX(CAST(metrics.counter.addresses_sync_outgoing_batches AS INT64))
      ),
      (
        'addresses_sync_outgoing_batches',
        'counter',
        '',
        'min',
        MIN(CAST(metrics.counter.addresses_sync_outgoing_batches AS INT64))
      ),
      (
        'addresses_sync_outgoing_batches',
        'counter',
        '',
        'sum',
        SUM(CAST(metrics.counter.addresses_sync_outgoing_batches AS INT64))
      )
