WITH stg_prospects AS (SELECT
  ...
  FORMAT_TIMESTAMP('%Y-%m-%dT%H:%M:%E*SZ', TIMESTAMP_SECONDS(contexts_com_pocket_prospect_1[0].created_at)) AS created_at,
  FORMAT_TIMESTAMP('%Y-%m-%dT%H:%M:%E*SZ', TIMESTAMP_SECONDS(contexts_com_pocket_prospect_1[0].reviewed_at)) AS reviewed_at,
...
QUALIFY ROW_NUMBER() OVER (PARTITION BY event_id ORDER BY dvce_created_tstamp) = 1)
...
FROM
  stg_prospects p
QUALIFY ROW_NUMBER() OVER (PARTITION BY prospect_id, object_update_trigger ORDER BY happened_at DESC) = 1;