...
WITH base_org_mozilla_focus AS (
...
overactive_org_mozilla_focus AS (
...
clients_daily_org_mozilla_focus AS (
  SELECT
  ...
  FROM
    base_org_mozilla_focus
  LEFT JOIN
    overactive_org_mozilla_focus
    USING (submission_date, client_id)
  WHERE
    overactive_org_mozilla_focus.client_id IS NULL
...
{% if is_init() %}
  clients_city_first_seen_org_mozilla_focus AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% else %}
  _previous_org_mozilla_focus AS (
    SELECT
      *
    FROM
      `moz-fx-data-shared-prod.focus_android_derived.baseline_clients_city_seen_v1`
    WHERE
      app_id = "org_mozilla_focus"
  ),
  _current_org_mozilla_focus AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% endif %}
base_org_mozilla_focus_beta AS (
  -- Live table dedup logic mimics copy_deduplicate
  SELECT
    submission_timestamp,
    DATE(submission_timestamp) AS submission_date,
  ...
  FROM
    {% if is_init() %}
      `moz-fx-data-shared-prod.org_mozilla_focus_beta_stable.baseline_v1`
    {% else %}
...
overactive_org_mozilla_focus_beta AS (
  -- Find client_ids with over 150 000 pings in a day,
  -- which could cause errors in the next step due to aggregation overflows.
  SELECT
    submission_date,
...
clients_daily_org_mozilla_focus_beta AS (
  SELECT
  ...
  FROM
    base_org_mozilla_focus_beta
  LEFT JOIN
    overactive_org_mozilla_focus_beta
    USING (submission_date, client_id)
  WHERE
    overactive_org_mozilla_focus_beta.client_id IS NULL
    -- `mode_last` can result in struct with all null values if it’s most frequent (or latest among ties).
    -- This exclude structs with all null values so there will always be one non-NULL field.
    AND COALESCE(city, subdivision1, subdivision2, country) IS NOT NULL
    {% if is_init() %}
...
{% if is_init() %}
  clients_city_first_seen_org_mozilla_focus_beta AS (
    SELECT
      app_id,
      client_id,
      sample_id,
      ...
      geo.country AS first_seen_country
    FROM
      clients_daily_org_mozilla_focus_beta
    WHERE
      geo.city IS NOT NULL
    QUALIFY
      ROW_NUMBER() OVER (PARTITION BY client_id, sample_id ORDER BY submission_date) = 1
  ),
  clients_city_last_seen_org_mozilla_focus_beta AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% else %}
  _previous_org_mozilla_focus_beta AS (
    SELECT
      *
    FROM
      `moz-fx-data-shared-prod.focus_android_derived.baseline_clients_city_seen_v1`
    WHERE
      app_id = "org_mozilla_focus_beta"
  ),
  _current_org_mozilla_focus_beta AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% endif %}
base_org_mozilla_focus_nightly AS (
  -- Live table dedup logic mimics copy_deduplicate
  SELECT
    submission_timestamp,
    DATE(submission_timestamp) AS submission_date,
  ...
  FROM
    {% if is_init() %}
      `moz-fx-data-shared-prod.org_mozilla_focus_nightly_stable.baseline_v1`
    {% else %}
...
overactive_org_mozilla_focus_nightly AS (
  -- Find client_ids with over 150 000 pings in a day,
  -- which could cause errors in the next step due to aggregation overflows.
  SELECT
    submission_date,
    client_id
  FROM
    base_org_mozilla_focus_nightly
  WHERE
    {% if is_init() %}
      submission_date >= '2018-01-01'
    {% else %}
...
clients_daily_org_mozilla_focus_nightly AS (
  SELECT
  ...
  FROM
    base_org_mozilla_focus_nightly
  LEFT JOIN
    overactive_org_mozilla_focus_nightly
    USING (submission_date, client_id)
  WHERE
    overactive_org_mozilla_focus_nightly.client_id IS NULL
    -- `mode_last` can result in struct with all null values if it’s most frequent (or latest among ties).
    -- This exclude structs with all null values so there will always be one non-NULL field.
    AND COALESCE(city, subdivision1, subdivision2, country) IS NOT NULL
    {% if is_init() %}
...
{% if is_init() %}
  clients_city_first_seen_org_mozilla_focus_nightly AS (
    SELECT
      app_id,
      client_id,
      sample_id,
      ...
      geo.country AS first_seen_country
    FROM
      clients_daily_org_mozilla_focus_nightly
    WHERE
      geo.city IS NOT NULL
    QUALIFY
      ROW_NUMBER() OVER (PARTITION BY client_id, sample_id ORDER BY submission_date) = 1
  ),
  clients_city_last_seen_org_mozilla_focus_nightly AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% else %}
  _previous_org_mozilla_focus_nightly AS (
    SELECT
      *
    FROM
      `moz-fx-data-shared-prod.focus_android_derived.baseline_clients_city_seen_v1`
    WHERE
      app_id = "org_mozilla_focus_nightly"
  ),
  _current_org_mozilla_focus_nightly AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% if is_init() %}
  SELECT
    ...
    last_seen_country
  FROM
    clients_city_first_seen_org_mozilla_focus cfs
  FULL OUTER JOIN
    clients_city_last_seen_org_mozilla_focus cls
  ...
  SELECT
    ...
    last_seen_country
  FROM
    clients_city_first_seen_org_mozilla_focus_beta cfs
  FULL OUTER JOIN
    clients_city_last_seen_org_mozilla_focus_beta cls
    USING (client_id, sample_id, app_id)
  UNION ALL
  SELECT
    app_id,
    ...
    last_seen_country
  FROM
    clients_city_first_seen_org_mozilla_focus_nightly cfs
  FULL OUTER JOIN
    clients_city_last_seen_org_mozilla_focus_nightly cls
    USING (client_id, sample_id, app_id)
{% else %}
  SELECT
-- _p.* fields are NULL for clients that are not yet captured in the baseline_city_seen derived table.
  ...
  FROM
    _current_org_mozilla_focus AS _c
  FULL JOIN
    _previous_org_mozilla_focus AS _p
    USING (client_id, sample_id, app_id)
  UNION ALL
  SELECT
-- _p.* fields are NULL for clients that are not yet captured in the baseline_city_seen derived table.
  ...
  FROM
    _current_org_mozilla_focus_beta AS _c
  FULL JOIN
    _previous_org_mozilla_focus_beta AS _p
    USING (client_id, sample_id, app_id)
  UNION ALL
  SELECT
-- _p.* fields are NULL for clients that are not yet captured in the baseline_city_seen derived table.
  ...
  FROM
    _current_org_mozilla_focus_nightly AS _c
  FULL JOIN
    _previous_org_mozilla_focus_nightly AS _p
    USING (client_id, sample_id, app_id)
{% endif %}
