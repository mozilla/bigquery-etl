CREATE OR REPLACE PROCEDURE
  event_analysis.create_funnel_steps_query(project STRING, dataset STRING, funnel ARRAY<STRUCT<list ARRAY<STRUCT<category STRING, event_name STRING>>>>, OUT sql STRING)
BEGIN
  ...
  WHILE i <= array_length(funnel) DO
    ...
    SET funnel_regex_assembly = CONCAT(
      'event_analysis.create_funnel_regex(ARRAY_AGG(step_regex ORDER BY step LIMIT ', i, '), TRUE)'
    );
...
BEGIN
  DECLARE result_sql STRING; DECLARE expect STRING DEFAULT """
WITH step_regexes AS (
  SELECT
    1 AS step,
    event_analysis.aggregate_match_strings(ARRAY_AGG(event_analysis.event_index_to_match_string(index))) AS step_regex
...
""";
  ...
  CALL event_analysis.create_funnel_steps_query(
    ...
    [
      STRUCT([
        STRUCT('collections' AS category, 'tab_select_opened' AS event_name)
      ] AS list),
      STRUCT([
