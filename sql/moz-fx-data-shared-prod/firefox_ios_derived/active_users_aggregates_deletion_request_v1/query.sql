WITH baseline AS (
  SELECT
    ...
    isp,
    IF(isp = 'BrowserStack', CONCAT('Firefox iOS', ' BrowserStack'), 'Firefox iOS') AS app_name
  FROM
    `moz-fx-data-shared-prod.firefox_ios.clients_last_seen_joined` AS joined
  INNER JOIN
    `firefox_ios.deletion_request` AS request
  ON
