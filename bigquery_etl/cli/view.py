...
@click.option(
    ...
    default="us",
    help=(
        "BigQuery region whose INFORMATION_SCHEMA is queried to find managed"
        " views. Views in datasets outside this region are not cleaned."
    ),
)
def clean(
    name,
...
def _list_managed_views(
    ...
    """Return the ids of managed views in `project`.

    One region-wide INFORMATION_SCHEMA query rather than one per dataset:
    shared-prod alone has ~580 datasets, and each per-dataset query is its own
    BigQuery job. Views in datasets outside `region` are not returned, so they
    are never cleaned.
    """
    query = f"""
      SELECT
    ...
    return {
        row.table_id
        for row in result
        # `table_id` is already `<project>.<dataset>.<name>`; passing it through
        # sql_table_id() raised AttributeError, so `view clean <name>` crashed.
