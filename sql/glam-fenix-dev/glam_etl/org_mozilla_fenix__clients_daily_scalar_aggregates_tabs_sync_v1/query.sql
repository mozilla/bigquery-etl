-- Query generated by: python3 -m bigquery_etl.glam.clients_daily_scalar_aggregates --source-table org_mozilla_fenix_stable.tabs_sync_v1
WITH extracted AS (
  SELECT
    *,
    DATE(submission_timestamp) AS submission_date,
    client_info.client_id,
    "tabs-sync" AS ping_type,
    COALESCE(
...
unlabeled_metrics AS (
  SELECT
    ...
    ARRAY<STRUCT<metric STRING, metric_type STRING, key STRING, agg_type STRING, value FLOAT64>>[
      (
        'tabs_sync_outgoing_batches',
        'counter',
        '',
        'avg',
        AVG(CAST(metrics.counter.tabs_sync_outgoing_batches AS INT64))
      ),
      (
        'tabs_sync_outgoing_batches',
        'counter',
        '',
        'count',
        IF(MIN(metrics.counter.tabs_sync_outgoing_batches) IS NULL, NULL, COUNT(*))
      ),
      (
        'tabs_sync_outgoing_batches',
        'counter',
        '',
        'max',
        MAX(CAST(metrics.counter.tabs_sync_outgoing_batches AS INT64))
      ),
      (
        'tabs_sync_outgoing_batches',
        'counter',
        '',
        'min',
        MIN(CAST(metrics.counter.tabs_sync_outgoing_batches AS INT64))
      ),
      (
        'tabs_sync_outgoing_batches',
        'counter',
        '',
        'sum',
        SUM(CAST(metrics.counter.tabs_sync_outgoing_batches AS INT64))
      )
...
grouped_labeled_metrics AS (
  SELECT
    ...
    ARRAY<STRUCT<name STRING, type STRING, value ARRAY<STRUCT<key STRING, value INT64>>>>[
      ...
      ('tabs_sync_incoming', 'labeled_counter', metrics.labeled_counter.tabs_sync_incoming),
      ('tabs_sync_outgoing', 'labeled_counter', metrics.labeled_counter.tabs_sync_outgoing)
    ] AS metrics
  FROM
    extracted
),
