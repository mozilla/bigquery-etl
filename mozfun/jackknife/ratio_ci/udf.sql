...
for the weighted mean of an array of ratios for various buckets; see
...
SELECT
  submission_date,
  jackknife.ratio_ci(20,
    ARRAY_AGG(STRUCT(CAST(active_days_in_week AS float64), CAST(wau as FLOAT64)))) AS intensity
FROM
  bucketed
GROUP BY
  submission_date
...
*/
CREATE OR REPLACE FUNCTION jackknife.ratio_ci(
  n_buckets INT64,
...
function average_buckets_with_ci(n_buckets, values_per_bucket) {
    ...
    var mean = array_sum(values_per_bucket.map(x => x.numerator)) / array_sum(values_per_bucket.map(x => x.denominator));
    var resamples = jackknife_resamples(values_per_bucket);
    var jk_means = resamples.map(resample => array_avg(resample.map(x => x.numerator / x.denominator)));
