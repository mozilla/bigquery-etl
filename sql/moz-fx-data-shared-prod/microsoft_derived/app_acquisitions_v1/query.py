...
CSV_FIELDS = [
    ...
    "purchase_price_local_amount",
    "purchase_tax_usd_amount",
    "purchase_tax_local_amount",
]
...
def download_microsoft_store_data(date, application_id, bearer_token):
    ...
    url = f"https://manage.devcenter.microsoft.com/v1.0/my/analytics/appacquisitions?applicationId={app_id}"
    url_params = f"aggregationLevel=day&startDate={start_date}&endDate={end_date}&skip=0&groupBy={','.join(groupBy)}"
    headers = {
...
def clean_json(query_export, date):
    ...
    for val in query_export["Value"]:
        field_dict = {
            ...
            "acquisition_type": val["acquisitionType"],
            "store_client": val["storeClient"],
            "gender": val["gender"],
            "market": val["market"],
            "os_version": val["osVersion"],
            "device_type": val["deviceType"],
            "acquisition_quantity": val["acquisitionQuantity"],
            "purchase_price_usd_amount": val["purchasePriceUSDAmount"],
            "purchase_price_local_amount": val["purchasePriceLocalAmount"],
            "purchase_tax_usd_amount": val["purchaseTaxUSDAmount"],
            "purchase_tax_local_amount": val["purchaseTaxLocalAmount"],
        }
        ...
        with open(tmp_csv.name, "w+b") as f_csv:
            ...
            job_config = bigquery.LoadJobConfig(
                ...
                schema=[
                    ...
                    bigquery.SchemaField("acquisition_type", "STRING"),
                    bigquery.SchemaField("store_client", "STRING"),
                    bigquery.SchemaField("gender", "STRING"),
                    bigquery.SchemaField("market", "STRING"),
                    bigquery.SchemaField("os_version", "STRING"),
                    bigquery.SchemaField("device_type", "STRING"),
                    bigquery.SchemaField("acquisition_quantity", "INT64"),
                    bigquery.SchemaField("purchase_price_usd_amount", "NUMERIC"),
                    bigquery.SchemaField("purchase_price_local_amount", "NUMERIC"),
                    bigquery.SchemaField("purchase_tax_usd_amount", "NUMERIC"),
                    bigquery.SchemaField("purchase_tax_local_amount", "NUMERIC"),
                ],
            )
...
def main():
    ...
    parser.add_argument("--microsoft_store_client_secret", required=True)
