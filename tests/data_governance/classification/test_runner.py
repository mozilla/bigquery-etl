...
class Harness:
    ...
    def install(self, monkeypatch):
        ...
        monkeypatch.setattr(runner, "sanitizer_for", lambda config: self.sanitizer)
...
def make_harness(monkeypatch):
    """Build a harness whose collaborators are already installed."""

    def make(**kwargs):
        return Harness(**kwargs).install(monkeypatch)

    return make


class TestHappyPath:
    def test_every_field_is_populated_from_the_run(self, make_harness):
        ...
        assert names - set(record) == {"input_hash"}
        assert "Table: src_dataset.my_table" in harness.model.prompts[0]


class TestSkipping:
    def test_classified_column_is_not_sent_to_the_model(self, make_harness):
        harness = make_harness(
            columns=one_table([column(), column(column_name="user_pref")]),
            answers=[answer()],
    ...
    def test_fully_classified_table_costs_nothing(self, make_harness):
        harness = make_harness(
    ...
    def test_refresh_reclassifies_without_reading_what_is_done(self, make_harness):
        harness = make_harness(
            columns=one_table([column()]),
            answers=[answer()],
            already={(*TABLE_KEY, "submission_timestamp")},
...
class TestMetricGating:
    def test_ordinary_column_without_a_probe_is_still_classified(self, make_harness):
        harness = make_harness(
            columns=one_table([column()]),
            answers=[answer()],
    ...
    def test_metric_leaf_takes_its_probe(self, make_harness):
        harness = make_harness(
    ...
    def test_metric_leaf_without_an_exact_match_is_dropped(self, make_harness):
        harness = make_harness(
    ...
    def test_unresolved_table_drops_metrics_and_keeps_the_rest(self, make_harness):
        harness = make_harness(
...
class TestPingResolution:
    def test_unresolved_ping_is_not_looked_up(self, make_harness):
        harness = make_harness(
            columns=one_table([column()]),
            answers=[answer()],
...
class TestSanitization:
    def test_high_cardinality_samples_never_reach_dlp(self, make_harness):
        harness = make_harness(
    ...
    def test_dlp_result_is_what_gets_recorded(self, make_harness):
...
class TestEvidence:
    def test_mirrors_the_column(self, make_harness):
        harness = make_harness(columns=one_table([column()]), answers=[answer()])
    ...
    def test_metric_leaf_records_no_samples(self, make_harness):
        harness = make_harness(
        ...
        assert evidence["existing_description"] is None

    def test_samples_are_exactly_what_the_prompt_rendered(self, make_harness):
...
class TestFailureHandling:
    @pytest.mark.parametrize(
        "first_outcome",
        [RuntimeError("boom"), answer(secondary_labels="oops")],
        ids=["model_error", "malformed_answer"],
    )
    def test_failed_answer_is_retried_and_succeeds(self, make_harness, first_outcome):
        harness = make_harness(
            columns=one_table([column()]),
            answers=[first_outcome, answer()],
        )
    ...
    def test_an_answer_without_a_label_is_retried_then_fails(self, make_harness, label):
        harness = make_harness(
            columns=one_table([column()]),
    ...
    def test_permanent_failure_reports_the_column_and_keeps_other_rows(
        ...
        assert len(harness.model.prompts) == 2 + runner.MAX_ANSWER_RETRIES
    ...
    def test_a_success_resets_the_counter(self, monkeypatch, make_harness):
    ...
    def test_the_breaker_flushes_pending_work_and_reports_the_run(
        self, monkeypatch, make_harness, caplog
    ):
        monkeypatch.setattr(runner, "MAX_CONSECUTIVE_FAILURES", 2)
        ...
        # out can have written them; the fifth column is abandoned.
        assert [r["column_name"] for r in harness.records] == ["a", "b"]
        assert len(harness.model.prompts) == 4
    ...
    def test_a_flush_failure_does_not_hide_the_breaker(self, monkeypatch, make_harness):
        monkeypatch.setattr(runner, "MAX_CONSECUTIVE_FAILURES", 2)
        monkeypatch.setattr(runner, "WRITE_BATCH_SIZE", 5)
        harness = make_harness(
...
class TestWrite:
    def test_delete_precedes_the_load_and_names_the_batch(self, make_harness):
        harness = make_harness(
            columns=one_table([column(), column(column_name="user_pref")]),
    ...
    def test_load_appends_against_the_deployed_schema(self, make_harness):
        harness = make_harness(columns=one_table([column()]), answers=[answer()])
    ...
    def test_batches_are_written_as_they_fill(self, monkeypatch, make_harness):
...
class TestTargets:
    ...
    def test_both_targets_run(self, make_harness):
        harness = make_harness(default_answer=answer())
    ...
    def test_failed_target_is_recorded_and_the_run_still_fails(self, make_harness):
        harness = make_harness(default_answer=answer())
        harness.columns_by_target = {
    ...
    def test_unavailable_model_stops_the_run(self, monkeypatch, make_harness):
        monkeypatch.setattr(runner, "MAX_CONSECUTIVE_FAILURES", 1)
        harness = make_harness(answers=[RuntimeError("boom")])
        harness.columns_by_target = {
            ("src-proj", "src_dataset"): one_table([column(column_name="a")]),
            ("src-proj", "other_dataset"): {self.SECOND_KEY: [column(column_name="b")]},
        }
