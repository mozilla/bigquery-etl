...
IF
  NOT EXISTS `moz-fx-data-shared-prod.org_mozilla_ios_firefoxvpn_network_extension_derived.event_monitoring_live_v1`
  PARTITION BY
  ...
  FROM
    (
      ...
      FROM
        `moz-fx-data-shared-prod.org_mozilla_ios_firefoxvpn_network_extension_live.events_v1`
      UNION ALL
      SELECT
        submission_timestamp,
        events,
      ...
      FROM
        `moz-fx-data-shared-prod.org_mozilla_ios_firefoxvpn_network_extension_live.extensionsession_v1`
      UNION ALL
      SELECT
        submission_timestamp,
        events,
  ...
  WHERE
    DATE(submission_timestamp) >= "2025-02-12"
  GROUP BY
    submission_date,
    window_start,
    window_end,
