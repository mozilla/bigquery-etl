...
--     (SELECT SUM(value) FROM UNNEST(histogram.value)) = 0
  ARRAY(
    ...
    INNER JOIN UNNEST(percentiles) AS percentile
...
SELECT
  assert_array_equals(
    ...
    udf.histogram_percentiles(
      STRUCT(ARRAY(SELECT STRUCT(v AS key, 1 AS value) FROM UNNEST(generate_array(1, 100)) AS v) AS values),
      [.1, .5, .9]
    )
  ),
  assert_array_equals(
    [100],
    udf.histogram_percentiles(
      STRUCT([STRUCT(99 AS key, 100 AS value), STRUCT(100, 101)] AS values), [.5]
    )
  ),
  assert_array_equals(
    CAST(NULL AS ARRAY<INT64>),
    udf.histogram_percentiles(
      STRUCT([STRUCT(0 AS key, 0 AS value)] AS values),
      [.2, .5]
    )
  );
