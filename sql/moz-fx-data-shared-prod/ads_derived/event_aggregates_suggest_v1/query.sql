{% if is_init() %}
  -- Initialize this table as a copy of the legacy table it is migrating from.
  -- The upstream ping data is only retained for 30 days (see the ERROR() guard in
  -- the incremental branch below), so history can only come from the legacy table.
  -- The lower bound satisfies require_partition_filter on the source and predates
  -- its first partition; narrow it if you only want part of the history.
  SELECT
    submission_date,
    form_factor,
    country,
