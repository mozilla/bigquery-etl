...
IF
  NOT EXISTS `moz-fx-data-shared-prod.org_mozilla_ios_tiktok_reporter_tiktok_reportershare_derived.event_monitoring_live_v1`
  OPTIONS
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_ios_tiktok_reporter_tiktok_reportershare_live.events_v1`
  CROSS JOIN
  ...
  WHERE
    DATE(submission_timestamp) >= "2024-02-28"
  GROUP BY
    submission_date,
    window_start,
    window_end,
