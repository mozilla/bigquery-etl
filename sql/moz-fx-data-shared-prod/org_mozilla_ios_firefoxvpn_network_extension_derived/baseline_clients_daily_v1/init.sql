...
CREATE TABLE IF NOT EXISTS
  `org_mozilla_ios_firefoxvpn_network_extension_derived.baseline_clients_daily_v1`
PARTITION BY
  submission_date
CLUSTER BY
  normalized_channel,
...
WITH base AS (
  ...
  FROM
    `org_mozilla_ios_firefoxvpn_network_extension_stable.baseline_v1`
...
joined AS (
  ...
  FROM
    windowed AS cd
  LEFT JOIN
    `org_mozilla_ios_firefoxvpn_network_extension_derived.baseline_clients_first_seen_v1` AS cfs
  USING
    (client_id)
  WHERE
    _n = 1
