...
overactive_org_mozilla_klar AS (
...
clients_daily_org_mozilla_klar AS (
  SELECT
  ...
  FROM
    base_org_mozilla_klar
  LEFT JOIN
    overactive_org_mozilla_klar
    USING (submission_date, client_id)
  WHERE
    overactive_org_mozilla_klar.client_id IS NULL
...
{% if is_init() %}
  clients_city_first_seen_org_mozilla_klar AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% else %}
  _previous_org_mozilla_klar AS (
    SELECT
      *
    FROM
      `moz-fx-data-shared-prod.klar_android_derived.baseline_clients_city_seen_v1`
    WHERE
      app_id = "org_mozilla_klar"
  ),
  _current_org_mozilla_klar AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% if is_init() %}
  SELECT
    ...
    last_seen_country
  FROM
    clients_city_first_seen_org_mozilla_klar cfs
  FULL OUTER JOIN
    clients_city_last_seen_org_mozilla_klar cls
...
-- _p.* fields are NULL for clients that are not yet captured in the baseline_city_seen derived table.
  ...
  FROM
    _current_org_mozilla_klar AS _c
  FULL JOIN
    _previous_org_mozilla_klar AS _p
    USING (client_id, sample_id, app_id)
{% endif %}
