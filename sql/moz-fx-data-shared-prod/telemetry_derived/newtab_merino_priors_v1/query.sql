WITH
  ...
  corpus_items AS (
    SELECT DISTINCT corpus_item_id
    FROM `moz-fx-data-shared-prod.snowflake_migration_derived.corpus_items_current_v1`
    WHERE DATE(corpus_item_updated_at) > (SELECT start_date_items FROM params)
  ),
  base AS (
    SELECT
      corpus_item_id,
      ...
      impression_count,
      click_count
    FROM `moz-fx-data-shared-prod.firefox_desktop_derived.newtab_content_items_daily_combined_v1`
    WHERE corpus_item_id IS NOT NULL
     AND country is NOT NULL
      AND submission_date BETWEEN (SELECT start_date FROM params) AND (SELECT end_date FROM params)
  ),
  ...
  aggregated_events AS (
    ...
    FROM base b
    INNER JOIN corpus_items c
      ON b.corpus_item_id = c.corpus_item_id
    GROUP BY b.corpus_item_id, b.region
  ),
  ...
  per_region_stats_with_impressions AS (
    ...
    FROM per_region_stats s
    JOIN per_region_impressions_per_item i USING (region)
  ),
  ...
  global_stats_with_impressions AS (
    SELECT
      ...
      i.impressions_per_item
    FROM global_stats s
    CROSS JOIN global_impressions_per_item i
  )
...
FROM per_region_stats_with_impressions
UNION ALL
SELECT
  region,
  average_ctr_top2_items,
  impressions_per_item
FROM global_stats_with_impressions
ORDER BY impressions_per_item DESC;
