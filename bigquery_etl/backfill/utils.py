...
def validate_table_metadata(
    ...
    override_depends_on_past: bool = False,
) -> List[str]:
    ...
    if not validate_depends_on_past(
        ...
        errors.append(
            ...
            "or set override_depends_on_past on a custom_query_path entry to backfill it per-partition."
        )
...
def validate_depends_on_past(
    ...
    - the backfill reinitializes the table via its is_init() query
      ...
      prior partitions, or
    - override_depends_on_past=true, used with a custom_query_path that processes each
