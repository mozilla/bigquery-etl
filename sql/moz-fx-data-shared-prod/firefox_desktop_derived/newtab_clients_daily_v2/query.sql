WITH events_unnested AS (
  SELECT
    ...
    metrics.boolean.pocket_sponsored_stories_enabled AS sponsored_ads_enabled,
...
flattened_visits AS (
  SELECT
    ...
    ANY_VALUE(sponsored_ads_enabled) AS sponsored_ads_enabled,
...
SELECT
  ...
  `moz-fx-data-shared-prod.udf.mode_last`(
    ARRAY_AGG(sponsored_ads_enabled)
  ) AS sponsored_ads_enabled,
  `moz-fx-data-shared-prod.udf.mode_last`(
