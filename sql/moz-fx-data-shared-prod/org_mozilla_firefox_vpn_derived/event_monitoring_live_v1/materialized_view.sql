...
IF
  NOT EXISTS `moz-fx-data-shared-prod.org_mozilla_firefox_vpn_derived.event_monitoring_live_v1`
  PARTITION BY
  ...
  base_events_v1 AS (
    ...
    FROM
      `moz-fx-data-shared-prod.org_mozilla_firefox_vpn_live.events_v1`
    CROSS JOIN
      UNNEST(events) AS event
    CROSS JOIN
          -- Iterator for accessing experiments.
  ...
  base_extensionsession_v1 AS (
    ...
    FROM
      `moz-fx-data-shared-prod.org_mozilla_firefox_vpn_live.extensionsession_v1`
    CROSS JOIN
      UNNEST(events) AS event
    CROSS JOIN
          -- Iterator for accessing experiments.
