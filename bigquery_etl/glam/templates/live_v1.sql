MERGE INTO
  `moz-fx-data-glam-prod-fca7.glam_etl.live_{{ product }}_{{ channel }}_v1` T
USING
  (
  SELECT
    *
  FROM
    `moz-fx-data-glam-prod-fca7.glam_etl.firefox_desktop_glam_{{ channel }}__extract_probe_counts_v1`) S
ON
    T.version = S.version
