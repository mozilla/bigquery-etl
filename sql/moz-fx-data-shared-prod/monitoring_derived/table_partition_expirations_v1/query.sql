...
first_partition_experimenter_backend_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.experimenter_backend_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_experimenter_backend_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_glam_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.glam_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_glam_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_mdn_fred_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.mdn_fred_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_mdn_fred_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_mdn_mcp_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.mdn_mcp_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_mdn_mcp_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_subscription_platform_backend_cirrus_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.subscription_platform_backend_cirrus_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_subscription_platform_backend_cirrus_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_subscription_platform_backend_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.subscription_platform_backend_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_subscription_platform_backend_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_subscription_platform_frontend_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.subscription_platform_frontend_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_subscription_platform_frontend_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
first_partition_thunderbird_crashreporter_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
    ...
    total_rows AS first_partition_row_count,
  FROM
    `moz-fx-data-shared-prod.thunderbird_crashreporter_stable.INFORMATION_SCHEMA.PARTITIONS`
  WHERE
    partition_id != "__NULL__"
  QUALIFY
    ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY partition_id) = 1
),
first_non_empty_partition_thunderbird_crashreporter_stable AS (
  SELECT
    table_name,
    PARSE_DATE("%Y%m%d", MIN(partition_id)) AS first_non_empty_partition_current,
  FROM
...
current_partitions AS (
  ...
  SELECT
    ...
    first_partition_row_count,
  FROM
    first_partition_experimenter_backend_stable
  LEFT JOIN
    first_non_empty_partition_experimenter_backend_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
  ...
  SELECT
    ...
    first_partition_row_count,
  FROM
    first_partition_glam_stable
  LEFT JOIN
    first_non_empty_partition_glam_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
  ...
  SELECT
    ...
    first_partition_row_count,
  FROM
    first_partition_mdn_fred_stable
  LEFT JOIN
    first_non_empty_partition_mdn_fred_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
    ...
    first_partition_row_count,
  FROM
    first_partition_mdn_mcp_stable
  LEFT JOIN
    first_non_empty_partition_mdn_mcp_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
  ...
  SELECT
    ...
    first_partition_row_count,
  FROM
    first_partition_subscription_platform_backend_cirrus_stable
  LEFT JOIN
    first_non_empty_partition_subscription_platform_backend_cirrus_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
    ...
    first_partition_row_count,
  FROM
    first_partition_subscription_platform_backend_stable
  LEFT JOIN
    first_non_empty_partition_subscription_platform_backend_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
    ...
    first_partition_row_count,
  FROM
    first_partition_subscription_platform_frontend_stable
  LEFT JOIN
    first_non_empty_partition_subscription_platform_frontend_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
  ...
  SELECT
    ...
    first_partition_row_count,
  FROM
    first_partition_thunderbird_crashreporter_stable
  LEFT JOIN
    first_non_empty_partition_thunderbird_crashreporter_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
