...
FROM
  `moz-fx-data-shared-prod.org_mozilla_ios_tiktok_reporter.events` AS e
CROSS JOIN
...
SELECT
  ...
  event.extra AS event_extra
FROM
  `moz-fx-data-shared-prod.org_mozilla_ios_tiktok_reporter_tiktok_reportershare.events` AS e
CROSS JOIN
  UNNEST(e.events) AS event
