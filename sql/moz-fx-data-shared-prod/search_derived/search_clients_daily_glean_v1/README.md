...
The prefix on a column name says which side it can come from. A coalesced column has no prefix. A `serp_`, `sap_` or `legacy_` prefix that survives into this CTE means the value exists on that side only — `sap_counts_total`, `sap_provider_id` and `sap_provider_name` from SAP, `serp_counts_total`, `serp_ad_click_target`, `serp_ad_blocker_inferred` and the SERP ad and engagement counts from SERP, and the seven `legacy_` counters from the metrics ping. The `legacy_` ones keep their prefix in the output, as do `serp_counts_total`, `serp_searches_organic_count` and the `sap_provider` pair; every other prefix is stripped in `final_cte`. The prefix distinguishes each counter from the SERP-derived measure of the same events, which is published under its own name.
