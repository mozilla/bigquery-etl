...
WITH _current AS (
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_fennec_aurora_derived.usage_reporting_clients_daily_v1`
  WHERE
...
_previous AS (
  SELECT
    usage_profile_id,
  FROM
    `moz-fx-data-shared-prod.org_mozilla_fennec_aurora_derived.usage_reporting_clients_first_seen_v1`
  WHERE
    {% if is_init() %}
      FALSE
    {% else %}
