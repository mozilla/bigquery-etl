...
WITH base_firefox_desktop_background_update AS (
  ...
  FROM
    ...
    {% else %}
      `moz-fx-data-shared-prod.firefox_desktop_background_update_live.baseline_v1`
    {% endif %}
  WHERE
...
overactive_firefox_desktop_background_update AS (
  ...
  SELECT
    ...
    client_id
  FROM
    base_firefox_desktop_background_update
  WHERE
    {% if is_init() %}
...
clients_daily_firefox_desktop_background_update AS (
  SELECT
    "firefox_desktop_background_update" AS app_id,
    submission_date,
    client_id,
    sample_id,
  ...
  FROM
    base_firefox_desktop_background_update
  LEFT JOIN
    overactive_firefox_desktop_background_update
    USING (submission_date, client_id)
  WHERE
    overactive_firefox_desktop_background_update.client_id IS NULL
...
{% if is_init() %}
  clients_city_first_seen_firefox_desktop_background_update AS (
    SELECT
      app_id,
      client_id,
      sample_id,
    ...
    FROM
      clients_daily_firefox_desktop_background_update
    WHERE
      geo.city IS NOT NULL
    QUALIFY
      ROW_NUMBER() OVER (PARTITION BY client_id, sample_id ORDER BY submission_date) = 1
  ),
  clients_city_last_seen_firefox_desktop_background_update AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% else %}
  _previous_firefox_desktop_background_update AS (
    SELECT
      *
    FROM
      `moz-fx-data-shared-prod.firefox_desktop_background_update_derived.baseline_clients_city_seen_v1`
    WHERE
      app_id = "firefox_desktop_background_update"
  ),
  _current_firefox_desktop_background_update AS (
    SELECT
      app_id,
      client_id,
      sample_id,
...
{% if is_init() %}
  SELECT
    ...
    last_seen_country
  FROM
    clients_city_first_seen_firefox_desktop_background_update cfs
  FULL OUTER JOIN
    clients_city_last_seen_firefox_desktop_background_update cls
...
-- _p.* fields are NULL for clients that are not yet captured in the baseline_city_seen derived table.
  ...
  FROM
    _current_firefox_desktop_background_update AS _c
  FULL JOIN
    _previous_firefox_desktop_background_update AS _p
    USING (client_id, sample_id, app_id)
{% endif %}
