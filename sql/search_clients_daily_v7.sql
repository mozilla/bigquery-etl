...
WITH
  ...
  SELECT
    ...
    engine,
    source,
    udf_mode_last(country) OVER w1 AS country,
    udf_mode_last(app_version) OVER w1 AS app_version,
    udf_mode_last(distribution_id) OVER w1 AS distribution_id,
    udf_mode_last(locale) OVER w1 AS locale,
    udf_mode_last(user_pref_browser_search_region) OVER w1 AS user_pref_browser_search_region,
    udf_mode_last(search_cohort) OVER w1 AS search_cohort,
    udf_mode_last(os) OVER w1 AS os,
    udf_mode_last(os_version) OVER w1 AS os_version,
    udf_mode_last(channel) OVER w1 AS channel,
    udf_mode_last(profile_creation_date) OVER w1 AS profile_creation_date,
    udf_mode_last(default_search_engine) OVER w1 AS default_search_engine,
    udf_mode_last(default_search_engine_data_load_path) OVER w1 AS default_search_engine_data_load_path,
    udf_mode_last(default_search_engine_data_submission_url) OVER w1 AS default_search_engine_data_submission_url,
    udf_mode_last(sample_id) OVER w1 AS sample_id,
