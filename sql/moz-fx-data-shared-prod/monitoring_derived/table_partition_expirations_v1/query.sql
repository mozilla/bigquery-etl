WITH first_partition_firefox_desktop_background_defaultagent_stable AS (
  SELECT
    table_catalog,
    table_schema,
    table_name,
...
current_partitions AS (
  ...
  SELECT
    ...
    first_partition_row_count,
  FROM
    first_partition_mlhackweek_search_stable
  LEFT JOIN
    first_non_empty_partition_mlhackweek_search_stable
    USING (table_name)
  UNION ALL
  SELECT
    {% if is_init() %}
