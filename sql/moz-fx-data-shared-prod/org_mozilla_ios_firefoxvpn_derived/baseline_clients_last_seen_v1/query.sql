...
{% if is_init() %}
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_ios_firefoxvpn_derived.baseline_clients_daily_v1`
  WHERE
...
{% else %}
  ...
  _previous AS (
    ...
    FROM
      `moz-fx-data-shared-prod.org_mozilla_ios_firefoxvpn_derived.baseline_clients_last_seen_v1`
    WHERE
