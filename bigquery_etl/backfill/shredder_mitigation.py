...
from bigquery_etl.cli import check
from bigquery_etl.cli.utils import paths_matching_name_pattern, extract_last_group_by_from_query
from bigquery_etl.format_sql.formatter import reformat
from bigquery_etl.metadata.parse_metadata import Metadata, METADATA_FILE, PartitionType
...
class Subset:
    ...
    def partitioning(self) -> str:
        ...
        if metadata.bigquery and metadata.bigquery.time_partitioning:
            partitioning = {"type": metadata.bigquery.time_partitioning.type,
                            'field': metadata.bigquery.time_partitioning.field}
        return partitioning

    def generate_query(self, select_list, from_clause=None, where_clause=None, group_by_clause=None, order_by_clause=None):
    ...
    def get_query_path_results(
        ...
        partition_type = 'DATE' if self.partitioning["type"] == PartitionType.DAY else self.partitioning["type"]
    ...
    def compare_current_and_previous_version(
            self, date_partition_parameter, backfill_date_str, columns_to_compare, metrics_to_compare
    ):
...
def generate_query_with_shredder_mitigation(
    ...
    new = Subset(client, destination_table, None, 'new_version', dataset, project_id, None)
    previous = Subset(client, new.name_previous_version, None, 'previous', dataset, project_id, None)
    ...
    sample_rows = new.get_query_path_results(backfill_date=backfill_date, rows=1, having_clause=having_clause)
    if not sample_rows:
        new_table_row = new.get_query_path_results(backfill_date=backfill_date, rows=1)
    try:
        new_table_row = sample_rows[0]
        common_dimensions, added_dimensions, removed_dimensions, metrics, undetermined_columns = classify_columns(
            new_table_row,
            previous_group_by,
            new_group_by)
    except TypeError:
    ...
    previous_agg = Subset(client, previous.name, None, 'previous_agg', TEMP_DATASET, project_id, None)
    # select_list=[previous.partitioning["field"]]+
    common_select=([previous.partitioning['field']]+
                   [f"COALESCE({dim.name}, '??') AS {dim.name}" for dim in common_dimensions
                    if (dim.name != previous.partitioning["field"] and dim.data_type == DataTypeGroup.STRING)] +
                   [f"COALESCE({dim.name}, -999) AS {dim.name}" for dim in common_dimensions
                    if (dim.name != new.partitioning["field"] and dim.data_type == DataTypeGroup.NUMERIC)] +
                   [f"COALESCE({dim.name}, -999) AS {dim.name}" for dim in common_dimensions
                    if (dim.name != new.partitioning["field"] and dim.data_type == DataTypeGroup.FLOAT)] +
                   [dim.name for dim in common_dimensions
                    if (dim.name != new.partitioning["field"] and dim.data_type in (DataTypeGroup.BOOLEAN, DataTypeGroup.DATE))] +
    ...
    shredded = Subset(client, destination_table, None, 'shredded', TEMP_DATASET, project_id, None)
    shredded_join = ' AND '.join(
        [f"{previous_agg.query_cte}.{previous.partitioning['field']} = {new_agg.query_cte}.{new.partitioning['field']}"]+
        [f"{previous_agg.query_cte}.{dim.name} = {new_agg.query_cte}.{dim.name}" for dim in common_dimensions if dim.name != previous.partitioning['field'] and dim.data_type not in (DataTypeGroup.BOOLEAN, DataTypeGroup.DATE)]+
        [f"({previous_agg.query_cte}.{dim.name} = {new_agg.query_cte}.{dim.name} OR ({previous_agg.query_cte}.{dim.name} IS NULL AND {new_agg.query_cte}.{dim.name} IS NULL))" for dim in common_dimensions if dim.name != previous.partitioning['field'] and dim.data_type in (DataTypeGroup.BOOLEAN, DataTypeGroup.DATE)]
        )
    shredded_query = shredded.generate_query(
        select_list=[f"{previous_agg.query_cte}.{dim.name}" for dim in common_dimensions]+
                    [f"'??' AS {dim.name}" for dim in added_dimensions] +
                    [f"{previous_agg.query_cte}.{metric.name} - IFNULL({new_agg.query_cte}.{metric.name}, 0) AS {metric.name}" for metric in metrics if metric.data_type != DataTypeGroup.FLOAT] +
                    [f"ROUND({previous_agg.query_cte}.{metric.name}, 3) - ROUND(IFNULL({new_agg.query_cte}.{metric.name}, 0), 3) AS {metric.name}" for metric in metrics if metric.data_type == DataTypeGroup.FLOAT],
    ...
    query_with_mitigation_template = env.get_template(f"{query_with_mitigation_name}_template.sql")
