...
def _deploy_view_artifact(file_path: Path, options: dict):
    ...
    # Add managed label if requested. This has to happen before the
    # has_changes check below, which diffs the view's labels against the
    # deployed ones.
    ...
    if not options["view_force"]:
        # Read outside the try so a missing option surfaces as a KeyError
        # rather than being swallowed as "couldn't check, republish".
        use_cloud_function = options["use_cloud_function"]
        try:
            changed = view.has_changes(
                target_project,
                client=client,
                use_cloud_function=use_cloud_function,
            )
        except Exception as e:
            # The check is an optimization, not a correctness gate, so a
            # failure here republishes rather than failing the artifact.
            log.warning(f"Could not check {file_path} for changes, publishing: {e}")
            changed = True
        if not changed:
            raise SkippedDeployException("no changes")
