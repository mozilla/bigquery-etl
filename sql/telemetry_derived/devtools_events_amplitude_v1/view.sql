...
all_events AS (
  SELECT
    ...
    WHEN
      event_object = 'tools'
    THEN
      CONCAT('dt - ', event_method)
    WHEN
      event_object = 'debugger'
    THEN
      CONCAT('dt_jsdebugger - ', event_method)
    WHEN
      event_object = 'aboutdebugging'
    THEN
      CONCAT('dt_adbg - ', event_method)
    WHEN
      event_category = 'devtools.main'
    THEN
      CONCAT('dt_', event_object, ' - ', event_method)
    END
    AS event_name,
  ...
  WHERE
    (
      doc_type IN ('main', 'event')
      AND app_name = 'Firefox'
      AND normalized_channel IN ('nightly', 'beta', 'aurora')
    )
    OR (
      doc_type = 'event'
      AND app_name = 'Firefox'
      ...
      (
        ...
        FROM
          (
            SELECT
              'host' AS key,
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'host') AS value
            UNION ALL
            SELECT
              'width' AS key,
              `moz-fx-data-derived-datasets.udf.get_key`(event_map_values, 'width') AS value
            UNION ALL
            SELECT
              'channel' AS key,
              normalized_channel AS value
            UNION ALL
            SELECT
              'app_build_id' AS key,
              application_build_id AS value
            UNION ALL
            SELECT
              'app_name' AS key,
              app_name AS value
            ...
            UNION ALL
            SELECT
              'env_build_arch' AS key,
              env_build_arch AS value
            UNION ALL
            SELECT
              'source' AS key,
              attribution_source AS value
            UNION ALL
            SELECT
              'profile_creation_date' AS key,
              CAST(
                SAFE.DATE_FROM_UNIX_DATE(CAST(profile_creation_date AS INT64)) AS STRING
              ) AS value
          )
      ),
      (
        ...
        FROM
          (
            SELECT
              'experiments' AS key,
              CONCAT('["', ARRAY_TO_STRING(experiments, '","'), '"]') AS value
            UNION ALL
            SELECT
              'sample_id' AS key,
              CAST(sample_id AS STRING) AS value
            UNION ALL
            SELECT
              'is_default_browser' AS key,
              CAST(is_default_browser AS STRING) AS value
            UNION ALL
            SELECT
              'is_wow64' AS key,
              CAST(is_wow64 AS STRING) AS value
            UNION ALL
            SELECT
              'memory_mb' AS key,
              CAST(memory_mb AS STRING) AS value
          )
      )
    ) AS user_props
  FROM
