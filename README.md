...
- To schedule a specific query, add a `metadata.yaml` file that includes a `scheduling` section, for example:
  ...
  - Additional scheduling options:
    ...
    ```
      ...
      - `destination_table`: The table to write to. If unspecified, defaulst to the query destination; if None, no destination table is used (the query is simply run as-is).
