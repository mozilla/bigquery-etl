...
IF
  NOT EXISTS `moz-fx-data-shared-prod.org_mozilla_bergamot_derived.event_monitoring_live_v1`
  PARTITION BY
  ...
  WITH base_custom_v1 AS (
    ...
    FROM
      `moz-fx-data-shared-prod.org_mozilla_bergamot_live.custom_v1`
    CROSS JOIN
  ...
  base_events_v1 AS (
    ...
    FROM
      `moz-fx-data-shared-prod.org_mozilla_bergamot_live.events_v1`
    CROSS JOIN
      UNNEST(events) AS event
    CROSS JOIN
          -- Iterator for accessing experiments.
