...
WITH daily_users_sum AS (
  SELECT
    SUM(daily_users),
  FROM
    `moz-fx-data-shared-prod.focus_ios_derived.active_users_aggregates_v3`
  WHERE
...
SELECT
  IF(
    ...
    ERROR(
      CONCAT(
        "Daily users mismatch between the focus_ios live across all channels (`moz-fx-data-shared-prod.org_mozilla_ios_focus_live.baseline_v1`,) and active_users_aggregates (`focus_ios_derived.active_users_aggregates_v3`) tables is greater than 10.",
        " Live table count: ",
