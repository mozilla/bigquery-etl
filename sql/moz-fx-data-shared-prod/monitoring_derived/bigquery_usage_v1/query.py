    ...
    return f"""
        SELECT
         "{source_project}" AS source_project,
          ...
          dataset_id AS reference_project_id,
          destination_table.project_id AS  destination_project_id,
          destination_table.dataset_id AS  destination_dataset_id,
          destination_table.table_id AS  destination_dataset_id,
          user_email,
          REGEXP_EXTRACT(query, r"Username: (.*?),") AS Username,
          REGEXP_EXTRACT(query, r"Query ID: (\\w+), ") AS query_id,
          REGEXP_EXTRACT(query, r"Scheduled: (\\w+) ") AS is_scheduled_query,
          total_bytes_billed / 1024 / 1024 / 1024 / 1024 AS total_terabytes_billed,
