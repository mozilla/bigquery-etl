-- Generated via bigquery_etl.glean_usage
CREATE OR REPLACE VIEW
  `moz-fx-data-shared-prod.firefox_desktop.events_stream`
AS
SELECT
  event_id,
  * EXCEPT (event_id),
  STRUCT(
    STRUCT(
      LAX_BOOL(event_extra.after_reset) AS `after_reset`,
      LAX_BOOL(event_extra.all_tabs_unloaded) AS `all_tabs_unloaded`,
      LAX_BOOL(event_extra.alt_text_modal) AS `alt_text_modal`,
      LAX_BOOL(event_extra.always_ask) AS `always_ask`,
      LAX_BOOL(event_extra.auto_show) AS `auto_show`,
      LAX_BOOL(event_extra.auto_translate) AS `auto_translate`,
      LAX_BOOL(event_extra.baseline) AS `baseline`,
      LAX_BOOL(event_extra.blocked) AS `blocked`,
      LAX_BOOL(event_extra.breached) AS `breached`,
      LAX_BOOL(event_extra.cache) AS `cache`,
      LAX_BOOL(event_extra.cached) AS `cached`,
      LAX_BOOL(event_extra.callout) AS `callout`,
      LAX_BOOL(event_extra.can_stage) AS `can_stage`,
      LAX_BOOL(event_extra.cancelled) AS `cancelled`,
      LAX_BOOL(event_extra.ccEnabled) AS `ccEnabled`,
      LAX_BOOL(event_extra.checked) AS `checked`,
      LAX_BOOL(event_extra.confident) AS `confident`,
      LAX_BOOL(event_extra.cookies_and_storage) AS `cookies_and_storage`,
      LAX_BOOL(event_extra.cta_shown) AS `cta_shown`,
      LAX_BOOL(event_extra.description) AS `description`,
      LAX_BOOL(event_extra.description_changed) AS `description_changed`,
      LAX_BOOL(event_extra.did_confirm) AS `did_confirm`,
      LAX_BOOL(event_extra.did_edit_pw) AS `did_edit_pw`,
      LAX_BOOL(event_extra.did_edit_un) AS `did_edit_un`,
      LAX_BOOL(event_extra.did_select_pw) AS `did_select_pw`,
      LAX_BOOL(event_extra.did_select_un) AS `did_select_un`,
      LAX_BOOL(event_extra.disableDialog) AS `disableDialog`,
      LAX_BOOL(event_extra.during_shutdown) AS `during_shutdown`,
      LAX_BOOL(event_extra.enabled) AS `enabled`,
      LAX_BOOL(event_extra.encrypted) AS `encrypted`,
      LAX_BOOL(event_extra.ended_in_error) AS `ended_in_error`,
      LAX_BOOL(event_extra.enrolled) AS `enrolled`,
      LAX_BOOL(event_extra.enter) AS `enter`,
      LAX_BOOL(event_extra.error) AS `error`,
      LAX_BOOL(event_extra.excluded) AS `excluded`,
      LAX_BOOL(event_extra.expand) AS `expand`,
      LAX_BOOL(event_extra.experiments_empty) AS `experiments_empty`,
      LAX_BOOL(event_extra.experiments_success) AS `experiments_success`,
      LAX_BOOL(event_extra.filled) AS `filled`,
      LAX_BOOL(event_extra.first_site_load_in_tab) AS `first_site_load_in_tab`,
      LAX_BOOL(event_extra.first_visit) AS `first_visit`,
      LAX_BOOL(event_extra.firstTime) AS `firstTime`,
      LAX_BOOL(event_extra.firstTimeToggle) AS `firstTimeToggle`,
      LAX_BOOL(event_extra.force_sync) AS `force_sync`,
      LAX_BOOL(event_extra.form_action_origin_fixable) AS `form_action_origin_fixable`,
      LAX_BOOL(event_extra.form_data) AS `form_data`,
      LAX_BOOL(event_extra.fxa) AS `fxa`,
      LAX_BOOL(event_extra.had_errors) AS `had_errors`,
      LAX_BOOL(event_extra.hadPreviousJwt) AS `hadPreviousJwt`,
      LAX_BOOL(event_extra.has_ai_summary) AS `has_ai_summary`,
      LAX_BOOL(event_extra.has_been_saved) AS `has_been_saved`,
      LAX_BOOL(event_extra.has_click_rule) AS `has_click_rule`,
      LAX_BOOL(event_extra.has_cookie_rule) AS `has_cookie_rule`,
      LAX_BOOL(event_extra.has_description) AS `has_description`,
      LAX_BOOL(event_extra.has_devices) AS `has_devices`,
      LAX_BOOL(event_extra.has_empty_password) AS `has_empty_password`,
      LAX_BOOL(event_extra.has_form_action_origin) AS `has_form_action_origin`,
      LAX_BOOL(event_extra.has_ftp_origin) AS `has_ftp_origin`,
      LAX_BOOL(event_extra.has_http_realm) AS `has_http_realm`,
      LAX_BOOL(event_extra.has_non_http_origin) AS `has_non_http_origin`,
      LAX_BOOL(event_extra.has_origin) AS `has_origin`,
      LAX_BOOL(event_extra.has_punycode_form_action_origin) AS `has_punycode_form_action_origin`,
      LAX_BOOL(event_extra.has_punycode_origin) AS `has_punycode_origin`,
      LAX_BOOL(event_extra.has_rtcp_mux) AS `has_rtcp_mux`,
      LAX_BOOL(event_extra.has_single_dot_origin) AS `has_single_dot_origin`,
      LAX_BOOL(event_extra.has_sts) AS `has_sts`,
      LAX_BOOL(event_extra.has_username_line_break) AS `has_username_line_break`,
      LAX_BOOL(event_extra.has_username_nul) AS `has_username_nul`,
      LAX_BOOL(event_extra.hidden) AS `hidden`,
      LAX_BOOL(event_extra.history_and_downloads) AS `history_and_downloads`,
      LAX_BOOL(event_extra.history_form_data_downloads) AS `history_form_data_downloads`,
      LAX_BOOL(event_extra.hit) AS `hit`,
      LAX_BOOL(event_extra.hls_decoder) AS `hls_decoder`,
      LAX_BOOL(event_extra.hyphen_compat) AS `hyphen_compat`,
      LAX_BOOL(event_extra.image) AS `image`,
      LAX_BOOL(event_extra.in_shutdown) AS `in_shutdown`,
      LAX_BOOL(event_extra.inPrivateBrowsing) AS `inPrivateBrowsing`,
      LAX_BOOL(event_extra.is_addon_active) AS `is_addon_active`,
      LAX_BOOL(event_extra.is_background) AS `is_background`,
      LAX_BOOL(event_extra.is_customizing) AS `is_customizing`,
      LAX_BOOL(event_extra.is_deleted) AS `is_deleted`,
      LAX_BOOL(event_extra.is_destroying) AS `is_destroying`,
      LAX_BOOL(event_extra.is_extensions_button_visible) AS `is_extensions_button_visible`,
      LAX_BOOL(event_extra.is_extensions_panel_empty) AS `is_extensions_panel_empty`,
      LAX_BOOL(event_extra.is_first_startup) AS `is_first_startup`,
      LAX_BOOL(event_extra.is_frame) AS `is_frame`,
      LAX_BOOL(event_extra.is_hardware_accelerated) AS `is_hardware_accelerated`,
      LAX_BOOL(event_extra.is_hardware_decoding) AS `is_hardware_decoding`,
      LAX_BOOL(event_extra.is_hdr) AS `is_hdr`,
      LAX_BOOL(event_extra.is_lang_attribute_valid) AS `is_lang_attribute_valid`,
      LAX_BOOL(event_extra.is_local_tab) AS `is_local_tab`,
      LAX_BOOL(event_extra.is_patch) AS `is_patch`,
      LAX_BOOL(event_extra.is_persistent) AS `is_persistent`,
      LAX_BOOL(event_extra.is_private) AS `is_private`,
      LAX_BOOL(event_extra.is_remote_ice_lite) AS `is_remote_ice_lite`,
      LAX_BOOL(event_extra.is_retry) AS `is_retry`,
      LAX_BOOL(event_extra.is_shareable) AS `is_shareable`,
      LAX_BOOL(event_extra.is_shopping_page) AS `is_shopping_page`,
      LAX_BOOL(event_extra.is_signed_in) AS `is_signed_in`,
      LAX_BOOL(event_extra.is_success) AS `is_success`,
      LAX_BOOL(event_extra.is_temporarily_shown) AS `is_temporarily_shown`,
      LAX_BOOL(event_extra.is_top_level_frame) AS `is_top_level_frame`,
      LAX_BOOL(event_extra.is_valid_section) AS `is_valid_section`,
      LAX_BOOL(event_extra.is_window_closed) AS `is_window_closed`,
      LAX_BOOL(event_extra.isBlock) AS `isBlock`,
      LAX_BOOL(event_extra.isDecryptSuccess) AS `isDecryptSuccess`,
      LAX_BOOL(event_extra.isIntegratedSidebar) AS `isIntegratedSidebar`,
      LAX_BOOL(event_extra.isProductPage) AS `isProductPage`,
      LAX_BOOL(event_extra.issued_by_cca) AS `issued_by_cca`,
      LAX_BOOL(event_extra.isSupportedSite) AS `isSupportedSite`,
      LAX_BOOL(event_extra.key_points) AS `key_points`,
      LAX_BOOL(event_extra.lang_tags_match) AS `lang_tags_match`,
      LAX_BOOL(event_extra.matches_pinned) AS `matches_pinned`,
      LAX_BOOL(event_extra.native_theme) AS `native_theme`,
      LAX_BOOL(event_extra.new_window) AS `new_window`,
      LAX_BOOL(event_extra.next_to_active_tab) AS `next_to_active_tab`,
      LAX_BOOL(event_extra.no_connectivity) AS `no_connectivity`,
      LAX_BOOL(event_extra.no_rule) AS `no_rule`,
      LAX_BOOL(event_extra.on_startup) AS `on_startup`,
      LAX_BOOL(event_extra.onboarding) AS `onboarding`,
      LAX_BOOL(event_extra.opened) AS `opened`,
      LAX_BOOL(event_extra.origin_fixable) AS `origin_fixable`,
      LAX_BOOL(event_extra.outcome) AS `outcome`,
      LAX_BOOL(event_extra.overridden_by_third_party) AS `overridden_by_third_party`,
      LAX_BOOL(event_extra.poisoned) AS `poisoned`,
      LAX_BOOL(event_extra.primary_password_set) AS `primary_password_set`,
      LAX_BOOL(event_extra.psi_available) AS `psi_available`,
      LAX_BOOL(event_extra.reader_mode) AS `reader_mode`,
      LAX_BOOL(event_extra.recomputed) AS `recomputed`,
      LAX_BOOL(event_extra.recovered) AS `recovered`,
      LAX_BOOL(event_extra.reset_disabled) AS `reset_disabled`,
      LAX_BOOL(event_extra.restricted) AS `restricted`,
      LAX_BOOL(event_extra.result_is_default) AS `result_is_default`,
      LAX_BOOL(event_extra.sampled_in) AS `sampled_in`,
      LAX_BOOL(event_extra.saved) AS `saved`,
      LAX_BOOL(event_extra.say_hello) AS `say_hello`,
      LAX_BOOL(event_extra.secure_experiments_empty) AS `secure_experiments_empty`,
      LAX_BOOL(event_extra.secure_experiments_success) AS `secure_experiments_success`,
      LAX_BOOL(event_extra.sent_with_blocked_trackers) AS `sent_with_blocked_trackers`,
      LAX_BOOL(event_extra.shopping_tab_displayed) AS `shopping_tab_displayed`,
      LAX_BOOL(event_extra.should_hide) AS `should_hide`,
      LAX_BOOL(event_extra.sidebar_open) AS `sidebar_open`,
      LAX_BOOL(event_extra.signed_in) AS `signed_in`,
      LAX_BOOL(event_extra.site_settings) AS `site_settings`,
      LAX_BOOL(event_extra.sitename) AS `sitename`,
      LAX_BOOL(event_extra.skipped) AS `skipped`,
      LAX_BOOL(event_extra.smart_window) AS `smart_window`,
      LAX_BOOL(event_extra.smartblockEmbedTogglesShown) AS `smartblockEmbedTogglesShown`,
      LAX_BOOL(event_extra.smartblockToggleClicked) AS `smartblockToggleClicked`,
      LAX_BOOL(event_extra.sponsored) AS `sponsored`,
      LAX_BOOL(event_extra.starter) AS `starter`,
      LAX_BOOL(event_extra.status) AS `status`,
      LAX_BOOL(event_extra.succeeded) AS `succeeded`,
      LAX_BOOL(event_extra.success) AS `success`,
      LAX_BOOL(event_extra.suggestion_shown) AS `suggestion_shown`,
      LAX_BOOL(event_extra.sync) AS `sync`,
      LAX_BOOL(event_extra.synced_tabs_loaded) AS `synced_tabs_loaded`,
      LAX_BOOL(event_extra.tagged) AS `tagged`,
      LAX_BOOL(event_extra.title) AS `title`,
      LAX_BOOL(event_extra.toggle) AS `toggle`,
      LAX_BOOL(event_extra.toggle_state) AS `toggle_state`,
      LAX_BOOL(event_extra.toggled_on) AS `toggled_on`,
      LAX_BOOL(event_extra.token_available) AS `token_available`,
      LAX_BOOL(event_extra.top_position) AS `top_position`,
      LAX_BOOL(event_extra.trackers_blocked) AS `trackers_blocked`,
      LAX_BOOL(event_extra.trigger_sampled) AS `trigger_sampled`,
      LAX_BOOL(event_extra.triggered_externally) AS `triggered_externally`,
      LAX_BOOL(event_extra.trr_only) AS `trr_only`,
      LAX_BOOL(event_extra.undo_available) AS `undo_available`,
      LAX_BOOL(event_extra.uniform_sampled) AS `uniform_sampled`,
      LAX_BOOL(event_extra.unload_selected_tab) AS `unload_selected_tab`,
      LAX_BOOL(event_extra.user_triggered) AS `user_triggered`,
      LAX_BOOL(event_extra.userAction) AS `userAction`,
      LAX_BOOL(event_extra.vulnerable) AS `vulnerable`,
      LAX_BOOL(event_extra.waited_out) AS `waited_out`,
      LAX_BOOL(event_extra.warning) AS `warning`,
      LAX_BOOL(event_extra.wasActive) AS `wasActive`,
      LAX_BOOL(event_extra.webVTTSubtitles) AS `webVTTSubtitles`
    ) AS `boolean`,
    STRUCT(
      LAX_INT64(event_extra.action_position) AS `action_position`,
      LAX_INT64(event_extra.active_age) AS `active_age`,
      LAX_INT64(event_extra.ads_hidden) AS `ads_hidden`,
      LAX_INT64(event_extra.ads_loaded) AS `ads_loaded`,
      LAX_INT64(event_extra.ads_visible) AS `ads_visible`,
      LAX_INT64(event_extra.age) AS `age`,
      LAX_INT64(event_extra.attempt) AS `attempt`,
      LAX_INT64(event_extra.attemptCount) AS `attemptCount`,
      LAX_INT64(event_extra.attempts) AS `attempts`,
      LAX_INT64(event_extra.auto_adjustments_prior) AS `auto_adjustments_prior`,
      LAX_INT64(event_extra.average_chunk_time) AS `average_chunk_time`,
      LAX_INT64(event_extra.average_words_per_request) AS `average_words_per_request`,
      LAX_INT64(event_extra.average_words_per_second) AS `average_words_per_second`,
      LAX_INT64(event_extra.batches_failed) AS `batches_failed`,
      LAX_INT64(event_extra.batches_total) AS `batches_total`,
      LAX_INT64(event_extra.buffering_time) AS `buffering_time`,
      LAX_INT64(event_extra.buttonType) AS `buttonType`,
      LAX_INT64(event_extra.candidates) AS `candidates`,
      LAX_INT64(event_extra.cc_number_multi_parts) AS `cc_number_multi_parts`,
      LAX_INT64(event_extra.channel_status) AS `channel_status`,
      LAX_INT64(event_extra.character_count) AS `character_count`,
      LAX_INT64(event_extra.check_weekday) AS `check_weekday`,
      LAX_INT64(event_extra.columnnumber) AS `columnnumber`,
      LAX_INT64(event_extra.comments_count) AS `comments_count`,
      LAX_INT64(event_extra.containers) AS `containers`,
      LAX_INT64(event_extra.cores) AS `cores`,
      LAX_INT64(event_extra.count) AS `count`,
      LAX_INT64(event_extra.cpu_milliseconds) AS `cpu_milliseconds`,
      LAX_INT64(event_extra.cpu_utilization) AS `cpu_utilization`,
      LAX_INT64(event_extra.create_rust_store_ms) AS `create_rust_store_ms`,
      LAX_INT64(event_extra.current) AS `current`,
      LAX_INT64(event_extra.db_active_count) AS `db_active_count`,
      LAX_INT64(event_extra.decoding_time) AS `decoding_time`,
      LAX_INT64(event_extra.delay) AS `delay`,
      LAX_INT64(event_extra.deleted) AS `deleted`,
      LAX_INT64(event_extra.diverged) AS `diverged`,
      LAX_INT64(event_extra.download) AS `download`,
      LAX_INT64(event_extra.download_time) AS `download_time`,
      LAX_INT64(event_extra.dropped_frames) AS `dropped_frames`,
      LAX_INT64(event_extra.duplicate_tabs) AS `duplicate_tabs`,
      LAX_INT64(event_extra.duration) AS `duration`,
      LAX_INT64(event_extra.duration_ms) AS `duration_ms`,
      LAX_INT64(event_extra.edited) AS `edited`,
      LAX_INT64(event_extra.editor_opens) AS `editor_opens`,
      LAX_INT64(event_extra.error_action) AS `error_action`,
      LAX_INT64(event_extra.error_code) AS `error_code`,
      LAX_INT64(event_extra.error_stage) AS `error_stage`,
      LAX_INT64(event_extra.error_type) AS `error_type`,
      LAX_INT64(event_extra.execution_seq) AS `execution_seq`,
      LAX_INT64(event_extra.extracted_code_units) AS `extracted_code_units`,
      LAX_INT64(event_extra.extraction_time) AS `extraction_time`,
      LAX_INT64(event_extra.failed) AS `failed`,
      LAX_INT64(event_extra.failed_deletions) AS `failed_deletions`,
      LAX_INT64(event_extra.field_seq) AS `field_seq`,
      LAX_INT64(event_extra.fields_filled) AS `fields_filled`,
      LAX_INT64(event_extra.fields_total) AS `fields_total`,
      LAX_INT64(event_extra.fields_typed) AS `fields_typed`,
      LAX_INT64(event_extra.fields_unknown) AS `fields_unknown`,
      LAX_INT64(event_extra.filled_length) AS `filled_length`,
      LAX_INT64(event_extra.final_length) AS `final_length`,
      LAX_INT64(event_extra.first_frame_loaded_time) AS `first_frame_loaded_time`,
      LAX_INT64(event_extra.gate_wait_ms) AS `gate_wait_ms`,
      LAX_INT64(event_extra.generated_length) AS `generated_length`,
      LAX_INT64(event_extra.grouped_id) AS `grouped_id`,
      LAX_INT64(event_extra.grouped_tabs) AS `grouped_tabs`,
      LAX_INT64(event_extra.groups) AS `groups`,
      LAX_INT64(event_extra.has_alt_text) AS `has_alt_text`,
      LAX_INT64(event_extra.has_no_alt_text) AS `has_no_alt_text`,
      LAX_INT64(event_extra.height) AS `height`,
      LAX_INT64(event_extra.http_status) AS `http_status`,
      LAX_INT64(event_extra.httpStatus) AS `httpStatus`,
      LAX_INT64(event_extra.identification_time) AS `identification_time`,
      LAX_INT64(event_extra.iframe_count) AS `iframe_count`,
      LAX_INT64(event_extra.in_use) AS `in_use`,
      LAX_INT64(event_extra.inference_time) AS `inference_time`,
      LAX_INT64(event_extra.init_rust_components_ms) AS `init_rust_components_ms`,
      LAX_INT64(event_extra.input_classified_size) AS `input_classified_size`,
      LAX_INT64(event_extra.input_inconclusive_size) AS `input_inconclusive_size`,
      LAX_INT64(event_extra.input_size) AS `input_size`,
      LAX_INT64(event_extra.input_tokens) AS `input_tokens`,
      LAX_INT64(event_extra.interest_top_1_hits) AS `interest_top_1_hits`,
      LAX_INT64(event_extra.interest_top_2_hits) AS `interest_top_2_hits`,
      LAX_INT64(event_extra.interest_top_3_hits) AS `interest_top_3_hits`,
      LAX_INT64(event_extra.items_count) AS `items_count`,
      LAX_INT64(event_extra.items_over_quota) AS `items_over_quota`,
      LAX_INT64(event_extra.last_install) AS `last_install`,
      LAX_INT64(event_extra.last_line_len) AS `last_line_len`,
      LAX_INT64(event_extra.last_platform_error) AS `last_platform_error`,
      LAX_INT64(event_extra.last_used) AS `last_used`,
      LAX_INT64(event_extra.latency) AS `latency`,
      LAX_INT64(event_extra.latency_ms) AS `latency_ms`,
      LAX_INT64(event_extra.length) AS `length`,
      LAX_INT64(event_extra.levenshtein_distance) AS `levenshtein_distance`,
      LAX_INT64(event_extra.lineno) AS `lineno`,
      LAX_INT64(event_extra.linenumber) AS `linenumber`,
      LAX_INT64(event_extra.mean_tabs) AS `mean_tabs`,
      LAX_INT64(event_extra.median_tabs) AS `median_tabs`,
      LAX_INT64(event_extra.memories) AS `memories`,
      LAX_INT64(event_extra.memories_similarity_avg) AS `memories_similarity_avg`,
      LAX_INT64(event_extra.memories_similarity_max) AS `memories_similarity_max`,
      LAX_INT64(event_extra.memories_similarity_min) AS `memories_similarity_min`,
      LAX_INT64(event_extra.memories_used) AS `memories_used`,
      LAX_INT64(event_extra.memories_used_similarity_avg) AS `memories_used_similarity_avg`,
      LAX_INT64(event_extra.memories_used_similarity_max) AS `memories_used_similarity_max`,
      LAX_INT64(event_extra.memories_used_similarity_min) AS `memories_used_similarity_min`,
      LAX_INT64(event_extra.memory_after) AS `memory_after`,
      LAX_INT64(event_extra.memory_before) AS `memory_before`,
      LAX_INT64(event_extra.memory_bytes) AS `memory_bytes`,
      LAX_INT64(event_extra.mentions) AS `mentions`,
      LAX_INT64(event_extra.mentions_available) AS `mentions_available`,
      LAX_INT64(event_extra.message_seq) AS `message_seq`,
      LAX_INT64(event_extra.metadata_loaded_time) AS `metadata_loaded_time`,
      LAX_INT64(event_extra.migrated) AS `migrated`,
      LAX_INT64(event_extra.ml_label_length) AS `ml_label_length`,
      LAX_INT64(event_extra.mode) AS `mode`,
      LAX_INT64(event_extra.models) AS `models`,
      LAX_INT64(event_extra.monitors) AS `monitors`,
      LAX_INT64(event_extra.n_chars) AS `n_chars`,
      LAX_INT64(event_extra.n_results) AS `n_results`,
      LAX_INT64(event_extra.n_tab_deselect) AS `n_tab_deselect`,
      LAX_INT64(event_extra.n_words) AS `n_words`,
      LAX_INT64(event_extra.negotiation_count) AS `negotiation_count`,
      LAX_INT64(event_extra.note_age_hours) AS `note_age_hours`,
      LAX_INT64(event_extra.note_length) AS `note_length`,
      LAX_INT64(event_extra.num_messages) AS `num_messages`,
      LAX_INT64(event_extra.num_msections_audio_recvonly) AS `num_msections_audio_recvonly`,
      LAX_INT64(event_extra.num_msections_audio_sendonly) AS `num_msections_audio_sendonly`,
      LAX_INT64(event_extra.num_msections_audio_sendrecv) AS `num_msections_audio_sendrecv`,
      LAX_INT64(event_extra.num_msections_data) AS `num_msections_data`,
      LAX_INT64(event_extra.num_msections_video_recvonly) AS `num_msections_video_recvonly`,
      LAX_INT64(event_extra.num_msections_video_sendonly) AS `num_msections_video_sendonly`,
      LAX_INT64(event_extra.num_msections_video_sendrecv) AS `num_msections_video_sendrecv`,
      LAX_INT64(event_extra.num_send_simulcast_layers) AS `num_send_simulcast_layers`,
      LAX_INT64(event_extra.num_strings) AS `num_strings`,
      LAX_INT64(event_extra.num_tabs_approved) AS `num_tabs_approved`,
      LAX_INT64(event_extra.num_tabs_in_group) AS `num_tabs_in_group`,
      LAX_INT64(event_extra.num_tabs_in_window) AS `num_tabs_in_window`,
      LAX_INT64(event_extra.num_tabs_removed) AS `num_tabs_removed`,
      LAX_INT64(event_extra.num_tabs_suggested) AS `num_tabs_suggested`,
      LAX_INT64(event_extra.num_transports) AS `num_transports`,
      LAX_INT64(event_extra.number_of_logins) AS `number_of_logins`,
      LAX_INT64(event_extra.number_of_logins_added) AS `number_of_logins_added`,
      LAX_INT64(event_extra.number_of_logins_failed) AS `number_of_logins_failed`,
      LAX_INT64(event_extra.number_of_logins_migrated) AS `number_of_logins_migrated`,
      LAX_INT64(event_extra.number_of_logins_quarantined) AS `number_of_logins_quarantined`,
      LAX_INT64(event_extra.number_of_logins_skipped) AS `number_of_logins_skipped`,
      LAX_INT64(event_extra.number_of_logins_to_delete) AS `number_of_logins_to_delete`,
      LAX_INT64(event_extra.number_of_logins_to_migrate) AS `number_of_logins_to_migrate`,
      LAX_INT64(event_extra.number_of_logins_to_restore) AS `number_of_logins_to_restore`,
      LAX_INT64(event_extra.number_of_logins_updated) AS `number_of_logins_updated`,
      LAX_INT64(event_extra.number_of_vulnerable_passwords) AS `number_of_vulnerable_passwords`,
      LAX_INT64(event_extra.opened_tabs) AS `opened_tabs`,
      LAX_INT64(event_extra.other) AS `other`,
      LAX_INT64(event_extra.output_interest_size) AS `output_interest_size`,
      LAX_INT64(event_extra.output_tokens) AS `output_tokens`,
      LAX_INT64(event_extra.pc_negotiation_count) AS `pc_negotiation_count`,
      LAX_INT64(event_extra.percentage) AS `percentage`,
      LAX_INT64(event_extra.platform_error) AS `platform_error`,
      LAX_INT64(event_extra.platform_error_code) AS `platform_error_code`,
      LAX_INT64(event_extra.played_time) AS `played_time`,
      LAX_INT64(event_extra.position) AS `position`,
      LAX_INT64(event_extra.pre_llm_confidence) AS `pre_llm_confidence`,
      LAX_INT64(event_extra.preselected) AS `preselected`,
      LAX_INT64(event_extra.previous) AS `previous`,
      LAX_INT64(event_extra.processing_duration) AS `processing_duration`,
      LAX_INT64(event_extra.profile_count) AS `profile_count`,
      LAX_INT64(event_extra.prompts) AS `prompts`,
      LAX_INT64(event_extra.recovery_count) AS `recovery_count`,
      LAX_INT64(event_extra.rendered_frames) AS `rendered_frames`,
      LAX_INT64(event_extra.restore_version) AS `restore_version`,
      LAX_INT64(event_extra.results_retrieved) AS `results_retrieved`,
      LAX_INT64(event_extra.results_returned) AS `results_returned`,
      LAX_INT64(event_extra.resume_prompts) AS `resume_prompts`,
      LAX_INT64(event_extra.retrieval_duration) AS `retrieval_duration`,
      LAX_INT64(event_extra.reviewed_length) AS `reviewed_length`,
      LAX_INT64(event_extra.saved_count) AS `saved_count`,
      LAX_INT64(event_extra.screenX) AS `screenX`,
      LAX_INT64(event_extra.screenY) AS `screenY`,
      LAX_INT64(event_extra.selected) AS `selected`,
      LAX_INT64(event_extra.selected_position) AS `selected_position`,
      LAX_INT64(event_extra.selection) AS `selection`,
      LAX_INT64(event_extra.sentences) AS `sentences`,
      LAX_INT64(event_extra.seq) AS `seq`,
      LAX_INT64(event_extra.session_duration) AS `session_duration`,
      LAX_INT64(event_extra.session_seq) AS `session_seq`,
      LAX_INT64(event_extra.size) AS `size`,
      LAX_INT64(event_extra.snippet_chars_returned) AS `snippet_chars_returned`,
      LAX_INT64(event_extra.snippet_chars_truncated) AS `snippet_chars_truncated`,
      LAX_INT64(event_extra.source_line) AS `source_line`,
      LAX_INT64(event_extra.source_text_code_units) AS `source_text_code_units`,
      LAX_INT64(event_extra.source_text_word_count) AS `source_text_word_count`,
      LAX_INT64(event_extra.source_total) AS `source_total`,
      LAX_INT64(event_extra.status_code) AS `status_code`,
      LAX_INT64(event_extra.step) AS `step`,
      LAX_INT64(event_extra.store_active_count) AS `store_active_count`,
      LAX_INT64(event_extra.suggested_groups) AS `suggested_groups`,
      LAX_INT64(event_extra.system_memory_mb) AS `system_memory_mb`,
      LAX_INT64(event_extra.tab_count) AS `tab_count`,
      LAX_INT64(event_extra.tabs) AS `tabs`,
      LAX_INT64(event_extra.tabs_added) AS `tabs_added`,
      LAX_INT64(event_extra.tabs_affected) AS `tabs_affected`,
      LAX_INT64(event_extra.tabs_approved) AS `tabs_approved`,
      LAX_INT64(event_extra.tabs_available) AS `tabs_available`,
      LAX_INT64(event_extra.tabs_final) AS `tabs_final`,
      LAX_INT64(event_extra.tabs_high) AS `tabs_high`,
      LAX_INT64(event_extra.tabs_in_group) AS `tabs_in_group`,
      LAX_INT64(event_extra.tabs_in_window) AS `tabs_in_window`,
      LAX_INT64(event_extra.tabs_inside_groups) AS `tabs_inside_groups`,
      LAX_INT64(event_extra.tabs_kept) AS `tabs_kept`,
      LAX_INT64(event_extra.tabs_low) AS `tabs_low`,
      LAX_INT64(event_extra.tabs_medium) AS `tabs_medium`,
      LAX_INT64(event_extra.tabs_open) AS `tabs_open`,
      LAX_INT64(event_extra.tabs_outside_groups) AS `tabs_outside_groups`,
      LAX_INT64(event_extra.tabs_per_active_group_average) AS `tabs_per_active_group_average`,
      LAX_INT64(event_extra.tabs_per_active_group_max) AS `tabs_per_active_group_max`,
      LAX_INT64(event_extra.tabs_per_active_group_median) AS `tabs_per_active_group_median`,
      LAX_INT64(event_extra.tabs_per_active_group_min) AS `tabs_per_active_group_min`,
      LAX_INT64(event_extra.tabs_preselected) AS `tabs_preselected`,
      LAX_INT64(event_extra.tabs_removed) AS `tabs_removed`,
      LAX_INT64(event_extra.tabs_restored) AS `tabs_restored`,
      LAX_INT64(event_extra.tabs_selected) AS `tabs_selected`,
      LAX_INT64(event_extra.tabs_sent) AS `tabs_sent`,
      LAX_INT64(event_extra.tabs_suggested) AS `tabs_suggested`,
      LAX_INT64(event_extra.tabs_unloaded) AS `tabs_unloaded`,
      LAX_INT64(event_extra.tabs_used) AS `tabs_used`,
      LAX_INT64(event_extra.target_total) AS `target_total`,
      LAX_INT64(event_extra.threshold) AS `threshold`,
      LAX_INT64(event_extra.time) AS `time`,
      LAX_INT64(event_extra.time_delta) AS `time_delta`,
      LAX_INT64(event_extra.time_local) AS `time_local`,
      LAX_INT64(event_extra.time_per_output_token) AS `time_per_output_token`,
      LAX_INT64(event_extra.time_remote) AS `time_remote`,
      LAX_INT64(event_extra.time_to_first_chunk) AS `time_to_first_chunk`,
      LAX_INT64(event_extra.time_to_first_token) AS `time_to_first_token`,
      LAX_INT64(event_extra.time_to_unload_in_ms) AS `time_to_unload_in_ms`,
      LAX_INT64(event_extra.token_count) AS `token_count`,
      LAX_INT64(event_extra.tokenizing_time) AS `tokenizing_time`,
      LAX_INT64(event_extra.tokens) AS `tokens`,
      LAX_INT64(event_extra.tokens_available) AS `tokens_available`,
      LAX_INT64(event_extra.tokens_per_second) AS `tokens_per_second`,
      LAX_INT64(event_extra.total) AS `total`,
      LAX_INT64(event_extra.total_collapsed) AS `total_collapsed`,
      LAX_INT64(event_extra.total_completed_requests) AS `total_completed_requests`,
      LAX_INT64(event_extra.total_db_count) AS `total_db_count`,
      LAX_INT64(event_extra.total_duration) AS `total_duration`,
      LAX_INT64(event_extra.total_expanded) AS `total_expanded`,
      LAX_INT64(event_extra.total_inference_seconds) AS `total_inference_seconds`,
      LAX_INT64(event_extra.total_length) AS `total_length`,
      LAX_INT64(event_extra.total_ms) AS `total_ms`,
      LAX_INT64(event_extra.total_retries) AS `total_retries`,
      LAX_INT64(event_extra.total_size_bytes) AS `total_size_bytes`,
      LAX_INT64(event_extra.total_store_count) AS `total_store_count`,
      LAX_INT64(event_extra.total_time) AS `total_time`,
      LAX_INT64(event_extra.total_translated_words) AS `total_translated_words`,
      LAX_INT64(event_extra.total_waiting_data_time) AS `total_waiting_data_time`,
      LAX_INT64(event_extra.total_words) AS `total_words`,
      LAX_INT64(event_extra.trigger_sampling_probability) AS `trigger_sampling_probability`,
      LAX_INT64(event_extra.turn_number) AS `turn_number`,
      LAX_INT64(event_extra.uniform_sampling_probability) AS `uniform_sampling_probability`,
      LAX_INT64(event_extra.urls) AS `urls`,
      LAX_INT64(event_extra.user_label_length) AS `user_label_length`,
      LAX_INT64(event_extra.user_prefs) AS `user_prefs`,
      LAX_INT64(event_extra.value) AS `value`,
      LAX_INT64(event_extra.view_time) AS `view_time`,
      LAX_INT64(event_extra.visible_topsites) AS `visible_topsites`,
      LAX_INT64(event_extra.wait_count) AS `wait_count`,
      LAX_INT64(event_extra.wall_milliseconds) AS `wall_milliseconds`,
      LAX_INT64(event_extra.when) AS `when`,
      LAX_INT64(event_extra.width) AS `width`,
      LAX_INT64(event_extra.window_height) AS `window_height`,
      LAX_INT64(event_extra.window_width) AS `window_width`,
      LAX_INT64(event_extra.words_added) AS `words_added`,
      LAX_INT64(event_extra.words_removed) AS `words_removed`
    ) AS `quantity`,
    STRUCT(
      JSON_VALUE(event_extra.abandonment_type) AS `abandonment_type`,
      JSON_VALUE(event_extra.about_config_change) AS `about_config_change`,
      JSON_VALUE(event_extra.acFieldName) AS `acFieldName`,
      JSON_VALUE(event_extra.action) AS `action`,
      JSON_VALUE(event_extra.action_id) AS `action_id`,
      JSON_VALUE(event_extra.action_type) AS `action_type`,
      JSON_VALUE(event_extra.actions) AS `actions`,
      JSON_VALUE(event_extra.active) AS `active`,
      JSON_VALUE(event_extra.adapter_device_id) AS `adapter_device_id`,
      JSON_VALUE(event_extra.adapter_driver_version) AS `adapter_driver_version`,
      JSON_VALUE(event_extra.adapter_vendor_id) AS `adapter_vendor_id`,
      JSON_VALUE(event_extra.added) AS `added`,
      JSON_VALUE(event_extra.additional_name) AS `additional_name`,
      JSON_VALUE(event_extra.addon_id) AS `addon_id`,
      JSON_VALUE(event_extra.addon_name) AS `addon_name`,
      JSON_VALUE(event_extra.addon_type) AS `addon_type`,
      JSON_VALUE(event_extra.addon_version) AS `addon_version`,
      JSON_VALUE(event_extra.addonId) AS `addonId`,
      JSON_VALUE(event_extra.addonVersion) AS `addonVersion`,
      JSON_VALUE(event_extra.address_level1) AS `address_level1`,
      JSON_VALUE(event_extra.address_level2) AS `address_level2`,
      JSON_VALUE(event_extra.address_line1) AS `address_line1`,
      JSON_VALUE(event_extra.address_line2) AS `address_line2`,
      JSON_VALUE(event_extra.address_line3) AS `address_line3`,
      JSON_VALUE(event_extra.admin_user) AS `admin_user`,
      JSON_VALUE(event_extra.age) AS `age`,
      JSON_VALUE(event_extra.agent) AS `agent`,
      JSON_VALUE(event_extra.alt_text_type) AS `alt_text_type`,
      JSON_VALUE(event_extra.appearance) AS `appearance`,
      JSON_VALUE(event_extra.attribute_name) AS `attribute_name`,
      JSON_VALUE(event_extra.attribute_value) AS `attribute_value`,
      JSON_VALUE(event_extra.audio_codec) AS `audio_codec`,
      JSON_VALUE(event_extra.auto_admin) AS `auto_admin`,
      JSON_VALUE(event_extra.available_semantic_sources) AS `available_semantic_sources`,
      JSON_VALUE(event_extra.backend) AS `backend`,
      JSON_VALUE(event_extra.backend_source_revision) AS `backend_source_revision`,
      JSON_VALUE(event_extra.backup_step) AS `backup_step`,
      JSON_VALUE(event_extra.blockeduridetails) AS `blockeduridetails`,
      JSON_VALUE(event_extra.blockeduritype) AS `blockeduritype`,
      JSON_VALUE(event_extra.blocklist_state) AS `blocklist_state`,
      JSON_VALUE(event_extra.bookmarks) AS `bookmarks`,
      JSON_VALUE(event_extra.branch) AS `branch`,
      JSON_VALUE(event_extra.branches) AS `branches`,
      JSON_VALUE(event_extra.branchSlug) AS `branchSlug`,
      JSON_VALUE(event_extra.breach_name) AS `breach_name`,
      JSON_VALUE(event_extra.breach_status) AS `breach_status`,
      JSON_VALUE(event_extra.build_id) AS `build_id`,
      JSON_VALUE(event_extra.bundle_policy) AS `bundle_policy`,
      JSON_VALUE(event_extra.button) AS `button`,
      JSON_VALUE(event_extra.caller) AS `caller`,
      JSON_VALUE(event_extra.can_load) AS `can_load`,
      JSON_VALUE(event_extra.canaries) AS `canaries`,
      JSON_VALUE(event_extra.cancel_reason) AS `cancel_reason`,
      JSON_VALUE(event_extra.captive_portal_state) AS `captive_portal_state`,
      JSON_VALUE(event_extra.captivePortal) AS `captivePortal`,
      JSON_VALUE(event_extra.captiveState) AS `captiveState`,
      JSON_VALUE(event_extra.category) AS `category`,
      JSON_VALUE(event_extra.caught_exceptio) AS `caught_exceptio`,
      JSON_VALUE(event_extra.cause) AS `cause`,
      JSON_VALUE(event_extra.cc_exp) AS `cc_exp`,
      JSON_VALUE(event_extra.cc_exp_month) AS `cc_exp_month`,
      JSON_VALUE(event_extra.cc_exp_year) AS `cc_exp_year`,
      JSON_VALUE(event_extra.cc_name) AS `cc_name`,
      JSON_VALUE(event_extra.cc_number) AS `cc_number`,
      JSON_VALUE(event_extra.cc_type) AS `cc_type`,
      JSON_VALUE(event_extra.change_reason) AS `change_reason`,
      JSON_VALUE(event_extra.change_source) AS `change_source`,
      JSON_VALUE(event_extra.change_type) AS `change_type`,
      JSON_VALUE(event_extra.changed) AS `changed`,
      JSON_VALUE(event_extra.changed_pref) AS `changed_pref`,
      JSON_VALUE(event_extra.changedPref) AS `changedPref`,
      JSON_VALUE(event_extra.chat_id) AS `chat_id`,
      JSON_VALUE(event_extra.chat_version) AS `chat_version`,
      JSON_VALUE(event_extra.check_time) AS `check_time`,
      JSON_VALUE(event_extra.choice) AS `choice`,
      JSON_VALUE(event_extra.click_type) AS `click_type`,
      JSON_VALUE(event_extra.closing_reason) AS `closing_reason`,
      JSON_VALUE(event_extra.code) AS `code`,
      JSON_VALUE(event_extra.code_type) AS `code_type`,
      JSON_VALUE(event_extra.codecs) AS `codecs`,
      JSON_VALUE(event_extra.cold) AS `cold`,
      JSON_VALUE(event_extra.collection) AS `collection`,
      JSON_VALUE(event_extra.component) AS `component`,
      JSON_VALUE(event_extra.component_stack) AS `component_stack`,
      JSON_VALUE(event_extra.computed) AS `computed`,
      JSON_VALUE(event_extra.confidence) AS `confidence`,
      JSON_VALUE(event_extra.configuration) AS `configuration`,
      JSON_VALUE(event_extra.configured) AS `configured`,
      JSON_VALUE(event_extra.conflict_slug) AS `conflict_slug`,
      JSON_VALUE(event_extra.conflicting_slug) AS `conflicting_slug`,
      JSON_VALUE(event_extra.conflictingSlug) AS `conflictingSlug`,
      JSON_VALUE(event_extra.connection) AS `connection`,
      JSON_VALUE(event_extra.connection_id) AS `connection_id`,
      JSON_VALUE(event_extra.connection_type) AS `connection_type`,
      JSON_VALUE(event_extra.container_id) AS `container_id`,
      JSON_VALUE(event_extra.content_type) AS `content_type`,
      JSON_VALUE(event_extra.contenttype) AS `contenttype`,
      JSON_VALUE(event_extra.context) AS `context`,
      JSON_VALUE(event_extra.context_type) AS `context_type`,
      JSON_VALUE(event_extra.contextReason) AS `contextReason`,
      JSON_VALUE(event_extra.country) AS `country`,
      JSON_VALUE(event_extra.cross_origin) AS `cross_origin`,
      JSON_VALUE(event_extra.current) AS `current`,
      JSON_VALUE(event_extra.current_intent) AS `current_intent`,
      JSON_VALUE(event_extra.current_state) AS `current_state`,
      JSON_VALUE(event_extra.data_migrated) AS `data_migrated`,
      JSON_VALUE(event_extra.data_type) AS `data_type`,
      JSON_VALUE(event_extra.decoder_name) AS `decoder_name`,
      JSON_VALUE(event_extra.default_path) AS `default_path`,
      JSON_VALUE(event_extra.deletedBy) AS `deletedBy`,
      JSON_VALUE(event_extra.delta) AS `delta`,
      JSON_VALUE(event_extra.descriptor_type) AS `descriptor_type`,
      JSON_VALUE(event_extra.destroyed) AS `destroyed`,
      JSON_VALUE(event_extra.detail) AS `detail`,
      JSON_VALUE(event_extra.detected_intent) AS `detected_intent`,
      JSON_VALUE(event_extra.detected_language) AS `detected_language`,
      JSON_VALUE(event_extra.device_count) AS `device_count`,
      JSON_VALUE(event_extra.device_name) AS `device_name`,
      JSON_VALUE(event_extra.device_pixel_ratio) AS `device_pixel_ratio`,
      JSON_VALUE(event_extra.device_type) AS `device_type`,
      JSON_VALUE(event_extra.didResetValue) AS `didResetValue`,
      JSON_VALUE(event_extra.direction) AS `direction`,
      JSON_VALUE(event_extra.directive) AS `directive`,
      JSON_VALUE(event_extra.disabled_engines) AS `disabled_engines`,
      JSON_VALUE(event_extra.display) AS `display`,
      JSON_VALUE(event_extra.document_language) AS `document_language`,
      JSON_VALUE(event_extra.domain) AS `domain`,
      JSON_VALUE(event_extra.download_time) AS `download_time`,
      JSON_VALUE(event_extra.dropped) AS `dropped`,
      JSON_VALUE(event_extra.duration) AS `duration`,
      JSON_VALUE(event_extra.editor_result) AS `editor_result`,
      JSON_VALUE(event_extra.effective) AS `effective`,
      JSON_VALUE(event_extra.element) AS `element`,
      JSON_VALUE(event_extra.email) AS `email`,
      JSON_VALUE(event_extra.enabled_engines) AS `enabled_engines`,
      JSON_VALUE(event_extra.end_reason) AS `end_reason`,
      JSON_VALUE(event_extra.end_state) AS `end_state`,
      JSON_VALUE(event_extra.engagement_type) AS `engagement_type`,
      JSON_VALUE(event_extra.engine_id) AS `engine_id`,
      JSON_VALUE(event_extra.engineId) AS `engineId`,
      JSON_VALUE(event_extra.enrollmentId) AS `enrollmentId`,
      JSON_VALUE(event_extra.enrollments) AS `enrollments`,
      JSON_VALUE(event_extra.enterprise) AS `enterprise`,
      JSON_VALUE(event_extra.entry_point) AS `entry_point`,
      JSON_VALUE(event_extra.entrypoint) AS `entrypoint`,
      JSON_VALUE(event_extra.error) AS `error`,
      JSON_VALUE(event_extra.error_code) AS `error_code`,
      JSON_VALUE(event_extra.error_kind) AS `error_kind`,
      JSON_VALUE(event_extra.error_message) AS `error_message`,
      JSON_VALUE(event_extra.error_name) AS `error_name`,
      JSON_VALUE(event_extra.error_reason) AS `error_reason`,
      JSON_VALUE(event_extra.error_string) AS `error_string`,
      JSON_VALUE(event_extra.error_type) AS `error_type`,
      JSON_VALUE(event_extra.errorName) AS `errorName`,
      JSON_VALUE(event_extra.errorResult) AS `errorResult`,
      JSON_VALUE(event_extra.etld) AS `etld`,
      JSON_VALUE(event_extra.evaluateReason) AS `evaluateReason`,
      JSON_VALUE(event_extra.exceptions) AS `exceptions`,
      JSON_VALUE(event_extra.existing_features) AS `existing_features`,
      JSON_VALUE(event_extra.experiment) AS `experiment`,
      JSON_VALUE(event_extra.experiment_type) AS `experiment_type`,
      JSON_VALUE(event_extra.experimentType) AS `experimentType`,
      JSON_VALUE(event_extra.expiry_date) AS `expiry_date`,
      JSON_VALUE(event_extra.expiry_date_day) AS `expiry_date_day`,
      JSON_VALUE(event_extra.expiry_date_month) AS `expiry_date_month`,
      JSON_VALUE(event_extra.expiry_date_year) AS `expiry_date_year`,
      JSON_VALUE(event_extra.extension_ids) AS `extension_ids`,
      JSON_VALUE(event_extra.extensions) AS `extensions`,
      JSON_VALUE(event_extra.failedLookups) AS `failedLookups`,
      JSON_VALUE(event_extra.family_name) AS `family_name`,
      JSON_VALUE(event_extra.fathom_infer_label) AS `fathom_infer_label`,
      JSON_VALUE(event_extra.fathom_infer_score) AS `fathom_infer_score`,
      JSON_VALUE(event_extra.feature) AS `feature`,
      JSON_VALUE(event_extra.feature_id) AS `feature_id`,
      JSON_VALUE(event_extra.feature_ids) AS `feature_ids`,
      JSON_VALUE(event_extra.featureId) AS `featureId`,
      JSON_VALUE(event_extra.field_kind) AS `field_kind`,
      JSON_VALUE(event_extra.field_name) AS `field_name`,
      JSON_VALUE(event_extra.fieldType) AS `fieldType`,
      JSON_VALUE(event_extra.file_name) AS `file_name`,
      JSON_VALUE(event_extra.filedetails) AS `filedetails`,
      JSON_VALUE(event_extra.fileinfo) AS `fileinfo`,
      JSON_VALUE(event_extra.filename) AS `filename`,
      JSON_VALUE(event_extra.filter) AS `filter`,
      JSON_VALUE(event_extra.filtering) AS `filtering`,
      JSON_VALUE(event_extra.first_panel) AS `first_panel`,
      JSON_VALUE(event_extra.flow) AS `flow`,
      JSON_VALUE(event_extra.flow_id) AS `flow_id`,
      JSON_VALUE(event_extra.form_action_origin_error) AS `form_action_origin_error`,
      JSON_VALUE(event_extra.formdata) AS `formdata`,
      JSON_VALUE(event_extra.frame_id) AS `frame_id`,
      JSON_VALUE(event_extra.from) AS `from`,
      JSON_VALUE(event_extra.from_container_id) AS `from_container_id`,
      JSON_VALUE(event_extra.from_language) AS `from_language`,
      JSON_VALUE(event_extra.from_msi) AS `from_msi`,
      JSON_VALUE(event_extra.fullpage) AS `fullpage`,
      JSON_VALUE(event_extra.function) AS `function`,
      JSON_VALUE(event_extra.functionality) AS `functionality`,
      JSON_VALUE(event_extra.fxa) AS `fxa`,
      JSON_VALUE(event_extra.fxa_avatar) AS `fxa_avatar`,
      JSON_VALUE(event_extra.fxa_status) AS `fxa_status`,
      JSON_VALUE(event_extra.fxa_sync_on) AS `fxa_sync_on`,
      JSON_VALUE(event_extra.generatedPasswo) AS `generatedPasswo`,
      JSON_VALUE(event_extra.given_name) AS `given_name`,
      JSON_VALUE(event_extra.grade) AS `grade`,
      JSON_VALUE(event_extra.group_type) AS `group_type`,
      JSON_VALUE(event_extra.groups) AS `groups`,
      JSON_VALUE(event_extra.hadPrevious) AS `hadPrevious`,
      JSON_VALUE(event_extra.handler) AS `handler`,
      JSON_VALUE(event_extra.hang_duration) AS `hang_duration`,
      JSON_VALUE(event_extra.has_jsonfile) AS `has_jsonfile`,
      JSON_VALUE(event_extra.has_olddata) AS `has_olddata`,
      JSON_VALUE(event_extra.hashed_addon_id) AS `hashed_addon_id`,
      JSON_VALUE(event_extra.history) AS `history`,
      JSON_VALUE(event_extra.host) AS `host`,
      JSON_VALUE(event_extra.host_type) AS `host_type`,
      JSON_VALUE(event_extra.hours_since) AS `hours_since`,
      JSON_VALUE(event_extra.html_lang_attribute) AS `html_lang_attribute`,
      JSON_VALUE(event_extra.ice_transport_policy) AS `ice_transport_policy`,
      JSON_VALUE(event_extra.icon_id) AS `icon_id`,
      JSON_VALUE(event_extra.id) AS `id`,
      JSON_VALUE(event_extra.identified_language) AS `identified_language`,
      JSON_VALUE(event_extra.iframe) AS `iframe`,
      JSON_VALUE(event_extra.image_type) AS `image_type`,
      JSON_VALUE(event_extra.importableLogin) AS `importableLogin`,
      JSON_VALUE(event_extra.impression_id) AS `impression_id`,
      JSON_VALUE(event_extra.inactive) AS `inactive`,
      JSON_VALUE(event_extra.infer_field_name) AS `infer_field_name`,
      JSON_VALUE(event_extra.infer_reason) AS `infer_reason`,
      JSON_VALUE(event_extra.input) AS `input`,
      JSON_VALUE(event_extra.input_type) AS `input_type`,
      JSON_VALUE(event_extra.inputType) AS `inputType`,
      JSON_VALUE(event_extra.insecureWarning) AS `insecureWarning`,
      JSON_VALUE(event_extra.install_existed) AS `install_existed`,
      JSON_VALUE(event_extra.install_id) AS `install_id`,
      JSON_VALUE(event_extra.install_origins) AS `install_origins`,
      JSON_VALUE(event_extra.installId) AS `installId`,
      JSON_VALUE(event_extra.intent) AS `intent`,
      JSON_VALUE(event_extra.interaction) AS `interaction`,
      JSON_VALUE(event_extra.interaction_type) AS `interaction_type`,
      JSON_VALUE(event_extra.issue) AS `issue`,
      JSON_VALUE(event_extra.issue_date) AS `issue_date`,
      JSON_VALUE(event_extra.issue_date_day) AS `issue_date_day`,
      JSON_VALUE(event_extra.issue_date_month) AS `issue_date_month`,
      JSON_VALUE(event_extra.issue_date_year) AS `issue_date_year`,
      JSON_VALUE(event_extra.key_system) AS `key_system`,
      JSON_VALUE(event_extra.l10n_ids) AS `l10n_ids`,
      JSON_VALUE(event_extra.label) AS `label`,
      JSON_VALUE(event_extra.label_reason) AS `label_reason`,
      JSON_VALUE(event_extra.lang) AS `lang`,
      JSON_VALUE(event_extra.lang_source) AS `lang_source`,
      JSON_VALUE(event_extra.language) AS `language`,
      JSON_VALUE(event_extra.last_line) AS `last_line`,
      JSON_VALUE(event_extra.layout) AS `layout`,
      JSON_VALUE(event_extra.lib_stacks) AS `lib_stacks`,
      JSON_VALUE(event_extra.lines) AS `lines`,
      JSON_VALUE(event_extra.link_type) AS `link_type`,
      JSON_VALUE(event_extra.loadfail_reason) AS `loadfail_reason`,
      JSON_VALUE(event_extra.locale) AS `locale`,
      JSON_VALUE(event_extra.location) AS `location`,
      JSON_VALUE(event_extra.login) AS `login`,
      JSON_VALUE(event_extra.loginsFooter) AS `loginsFooter`,
      JSON_VALUE(event_extra.made_changes) AS `made_changes`,
      JSON_VALUE(event_extra.main_frame) AS `main_frame`,
      JSON_VALUE(event_extra.media_content_type) AS `media_content_type`,
      JSON_VALUE(event_extra.media_extension) AS `media_extension`,
      JSON_VALUE(event_extra.memory_source) AS `memory_source`,
      JSON_VALUE(event_extra.menu_action) AS `menu_action`,
      JSON_VALUE(event_extra.message) AS `message`,
      JSON_VALUE(event_extra.message_count) AS `message_count`,
      JSON_VALUE(event_extra.message_id) AS `message_id`,
      JSON_VALUE(event_extra.method) AS `method`,
      JSON_VALUE(event_extra.metric_version) AS `metric_version`,
      JSON_VALUE(event_extra.migration) AS `migration`,
      JSON_VALUE(event_extra.migration_id) AS `migration_id`,
      JSON_VALUE(event_extra.migrator_key) AS `migrator_key`,
      JSON_VALUE(event_extra.mime_type) AS `mime_type`,
      JSON_VALUE(event_extra.ml) AS `ml`,
      JSON_VALUE(event_extra.ml_infer_label) AS `ml_infer_label`,
      JSON_VALUE(event_extra.ml_infer_score) AS `ml_infer_score`,
      JSON_VALUE(event_extra.ml_revision) AS `ml_revision`,
      JSON_VALUE(event_extra.mlbf_generation) AS `mlbf_generation`,
      JSON_VALUE(event_extra.mlbf_last_time) AS `mlbf_last_time`,
      JSON_VALUE(event_extra.mlbf_softblocks_generation) AS `mlbf_softblocks_generation`,
      JSON_VALUE(event_extra.mlbf_softblocks_source) AS `mlbf_softblocks_source`,
      JSON_VALUE(event_extra.mlbf_source) AS `mlbf_source`,
      JSON_VALUE(event_extra.mltime) AS `mltime`,
      JSON_VALUE(event_extra.mlversion) AS `mlversion`,
      JSON_VALUE(event_extra.mode) AS `mode`,
      JSON_VALUE(event_extra.model) AS `model`,
      JSON_VALUE(event_extra.model_id) AS `model_id`,
      JSON_VALUE(event_extra.model_locale) AS `model_locale`,
      JSON_VALUE(event_extra.model_revision) AS `model_revision`,
      JSON_VALUE(event_extra.modelDownloadId) AS `modelDownloadId`,
      JSON_VALUE(event_extra.modelId) AS `modelId`,
      JSON_VALUE(event_extra.modelRevision) AS `modelRevision`,
      JSON_VALUE(event_extra.modifiers) AS `modifiers`,
      JSON_VALUE(event_extra.move) AS `move`,
      JSON_VALUE(event_extra.name) AS `name`,
      JSON_VALUE(event_extra.networkID) AS `networkID`,
      JSON_VALUE(event_extra.networkUnstable) AS `networkUnstable`,
      JSON_VALUE(event_extra.new_display_name) AS `new_display_name`,
      JSON_VALUE(event_extra.new_engine_id) AS `new_engine_id`,
      JSON_VALUE(event_extra.new_load_path) AS `new_load_path`,
      JSON_VALUE(event_extra.new_model) AS `new_model`,
      JSON_VALUE(event_extra.new_region) AS `new_region`,
      JSON_VALUE(event_extra.new_submission_url) AS `new_submission_url`,
      JSON_VALUE(event_extra.new_tab_action) AS `new_tab_action`,
      JSON_VALUE(event_extra.newpanel) AS `newpanel`,
      JSON_VALUE(event_extra.next_panel) AS `next_panel`,
      JSON_VALUE(event_extra.notification_detail) AS `notification_detail`,
      JSON_VALUE(event_extra.notification_type) AS `notification_type`,
      JSON_VALUE(event_extra.num_strings) AS `num_strings`,
      JSON_VALUE(event_extra.number) AS `number`,
      JSON_VALUE(event_extra.object) AS `object`,
      JSON_VALUE(event_extra.old_region) AS `old_region`,
      JSON_VALUE(event_extra.oldpanel) AS `oldpanel`,
      JSON_VALUE(event_extra.opened_from) AS `opened_from`,
      JSON_VALUE(event_extra.opening_reason) AS `opening_reason`,
      JSON_VALUE(event_extra.openingReason) AS `openingReason`,
      JSON_VALUE(event_extra.operation) AS `operation`,
      JSON_VALUE(event_extra.option) AS `option`,
      JSON_VALUE(event_extra.option_clicked) AS `option_clicked`,
      JSON_VALUE(event_extra.option_name) AS `option_name`,
      JSON_VALUE(event_extra.organization) AS `organization`,
      JSON_VALUE(event_extra.orientation) AS `orientation`,
      JSON_VALUE(event_extra.origin) AS `origin`,
      JSON_VALUE(event_extra.origin_error) AS `origin_error`,
      JSON_VALUE(event_extra.originalReason) AS `originalReason`,
      JSON_VALUE(event_extra.os) AS `os`,
      JSON_VALUE(event_extra.other_inst) AS `other_inst`,
      JSON_VALUE(event_extra.other_msix_inst) AS `other_msix_inst`,
      JSON_VALUE(event_extra.outcome) AS `outcome`,
      JSON_VALUE(event_extra.output_hash) AS `output_hash`,
      JSON_VALUE(event_extra.packet_error) AS `packet_error`,
      JSON_VALUE(event_extra.packet_target) AS `packet_target`,
      JSON_VALUE(event_extra.packet_type) AS `packet_type`,
      JSON_VALUE(event_extra.page) AS `page`,
      JSON_VALUE(event_extra.page_type) AS `page_type`,
      JSON_VALUE(event_extra.pane) AS `pane`,
      JSON_VALUE(event_extra.panel) AS `panel`,
      JSON_VALUE(event_extra.panel_name) AS `panel_name`,
      JSON_VALUE(event_extra.paramChanged) AS `paramChanged`,
      JSON_VALUE(event_extra.partner_code) AS `partner_code`,
      JSON_VALUE(event_extra.passwords) AS `passwords`,
      JSON_VALUE(event_extra.patch_type) AS `patch_type`,
      JSON_VALUE(event_extra.path_key) AS `path_key`,
      JSON_VALUE(event_extra.payment_methods) AS `payment_methods`,
      JSON_VALUE(event_extra.pc_id) AS `pc_id`,
      JSON_VALUE(event_extra.permission_type) AS `permission_type`,
      JSON_VALUE(event_extra.phase) AS `phase`,
      JSON_VALUE(event_extra.platform) AS `platform`,
      JSON_VALUE(event_extra.playback_type) AS `playback_type`,
      JSON_VALUE(event_extra.position) AS `position`,
      JSON_VALUE(event_extra.postal_code) AS `postal_code`,
      JSON_VALUE(event_extra.pre_llm_field_kind) AS `pre_llm_field_kind`,
      JSON_VALUE(event_extra.pre_llm_source) AS `pre_llm_source`,
      JSON_VALUE(event_extra.pref) AS `pref`,
      JSON_VALUE(event_extra.pref_name) AS `pref_name`,
      JSON_VALUE(event_extra.pref_type) AS `pref_type`,
      JSON_VALUE(event_extra.prefBranch) AS `prefBranch`,
      JSON_VALUE(event_extra.preference) AS `preference`,
      JSON_VALUE(event_extra.preferenceName) AS `preferenceName`,
      JSON_VALUE(event_extra.preferred_recv_codec) AS `preferred_recv_codec`,
      JSON_VALUE(event_extra.preferred_send_codec) AS `preferred_send_codec`,
      JSON_VALUE(event_extra.prefName) AS `prefName`,
      JSON_VALUE(event_extra.prefType) AS `prefType`,
      JSON_VALUE(event_extra.previous) AS `previous`,
      JSON_VALUE(event_extra.previous_engine_id) AS `previous_engine_id`,
      JSON_VALUE(event_extra.previous_language) AS `previous_language`,
      JSON_VALUE(event_extra.previous_model) AS `previous_model`,
      JSON_VALUE(event_extra.previous_version) AS `previous_version`,
      JSON_VALUE(event_extra.previousState) AS `previousState`,
      JSON_VALUE(event_extra.primary) AS `primary`,
      JSON_VALUE(event_extra.principal_type) AS `principal_type`,
      JSON_VALUE(event_extra.principalType) AS `principalType`,
      JSON_VALUE(event_extra.process_id) AS `process_id`,
      JSON_VALUE(event_extra.profdir_existed) AS `profdir_existed`,
      JSON_VALUE(event_extra.profile_state) AS `profile_state`,
      JSON_VALUE(event_extra.prompt) AS `prompt`,
      JSON_VALUE(event_extra.prompt_enabled) AS `prompt_enabled`,
      JSON_VALUE(event_extra.prompt_type) AS `prompt_type`,
      JSON_VALUE(event_extra.prompt_version) AS `prompt_version`,
      JSON_VALUE(event_extra.property) AS `property`,
      JSON_VALUE(event_extra.protocol) AS `protocol`,
      JSON_VALUE(event_extra.provider) AS `provider`,
      JSON_VALUE(event_extra.provider_id) AS `provider_id`,
      JSON_VALUE(event_extra.provider_key) AS `provider_key`,
      JSON_VALUE(event_extra.provider_name) AS `provider_name`,
      JSON_VALUE(event_extra.psi_full_avg10) AS `psi_full_avg10`,
      JSON_VALUE(event_extra.psi_full_avg60) AS `psi_full_avg60`,
      JSON_VALUE(event_extra.psi_some_avg10) AS `psi_some_avg10`,
      JSON_VALUE(event_extra.psi_some_avg60) AS `psi_some_avg60`,
      JSON_VALUE(event_extra.rating) AS `rating`,
      JSON_VALUE(event_extra.re) AS `re`,
      JSON_VALUE(event_extra.reaction) AS `reaction`,
      JSON_VALUE(event_extra.reader_time) AS `reader_time`,
      JSON_VALUE(event_extra.readiness) AS `readiness`,
      JSON_VALUE(event_extra.reason) AS `reason`,
      JSON_VALUE(event_extra.record_type) AS `record_type`,
      JSON_VALUE(event_extra.redirects) AS `redirects`,
      JSON_VALUE(event_extra.region) AS `region`,
      JSON_VALUE(event_extra.remotetype) AS `remotetype`,
      JSON_VALUE(event_extra.removal_result) AS `removal_result`,
      JSON_VALUE(event_extra.request_id) AS `request_id`,
      JSON_VALUE(event_extra.request_target) AS `request_target`,
      JSON_VALUE(event_extra.requested_features) AS `requested_features`,
      JSON_VALUE(event_extra.require_signon) AS `require_signon`,
      JSON_VALUE(event_extra.reset_error_name) AS `reset_error_name`,
      JSON_VALUE(event_extra.resize) AS `resize`,
      JSON_VALUE(event_extra.resolution) AS `resolution`,
      JSON_VALUE(event_extra.response) AS `response`,
      JSON_VALUE(event_extra.result) AS `result`,
      JSON_VALUE(event_extra.results) AS `results`,
      JSON_VALUE(event_extra.retime) AS `retime`,
      JSON_VALUE(event_extra.retryCount) AS `retryCount`,
      JSON_VALUE(event_extra.run_id) AS `run_id`,
      JSON_VALUE(event_extra.runtime_id) AS `runtime_id`,
      JSON_VALUE(event_extra.runtime_name) AS `runtime_name`,
      JSON_VALUE(event_extra.runtime_os) AS `runtime_os`,
      JSON_VALUE(event_extra.runtime_type) AS `runtime_type`,
      JSON_VALUE(event_extra.runtime_version) AS `runtime_version`,
      JSON_VALUE(event_extra.sample) AS `sample`,
      JSON_VALUE(event_extra.sandboxed) AS `sandboxed`,
      JSON_VALUE(event_extra.sap) AS `sap`,
      JSON_VALUE(event_extra.scenario) AS `scenario`,
      JSON_VALUE(event_extra.schedule_type) AS `schedule_type`,
      JSON_VALUE(event_extra.scheme) AS `scheme`,
      JSON_VALUE(event_extra.scroll_position) AS `scroll_position`,
      JSON_VALUE(event_extra.search_domain) AS `search_domain`,
      JSON_VALUE(event_extra.search_engine_default_id) AS `search_engine_default_id`,
      JSON_VALUE(event_extra.search_mode) AS `search_mode`,
      JSON_VALUE(event_extra.search_start) AS `search_start`,
      JSON_VALUE(event_extra.section) AS `section`,
      JSON_VALUE(event_extra.selected_result) AS `selected_result`,
      JSON_VALUE(event_extra.selection) AS `selection`,
      JSON_VALUE(event_extra.selfdetails) AS `selfdetails`,
      JSON_VALUE(event_extra.selftype) AS `selftype`,
      JSON_VALUE(event_extra.server_content_process_stack) AS `server_content_process_stack`,
      JSON_VALUE(event_extra.server_stack) AS `server_stack`,
      JSON_VALUE(event_extra.session) AS `session`,
      JSON_VALUE(event_extra.session_id) AS `session_id`,
      JSON_VALUE(event_extra.session_start_time) AS `session_start_time`,
      JSON_VALUE(event_extra.setdefault_source) AS `setdefault_source`,
      JSON_VALUE(event_extra.setting) AS `setting`,
      JSON_VALUE(event_extra.severity) AS `severity`,
      JSON_VALUE(event_extra.share_type) AS `share_type`,
      JSON_VALUE(event_extra.shortcut) AS `shortcut`,
      JSON_VALUE(event_extra.shutdown_ok) AS `shutdown_ok`,
      JSON_VALUE(event_extra.shutdown_reason) AS `shutdown_reason`,
      JSON_VALUE(event_extra.side_bar_state) AS `side_bar_state`,
      JSON_VALUE(event_extra.sidebar) AS `sidebar`,
      JSON_VALUE(event_extra.signature) AS `signature`,
      JSON_VALUE(event_extra.signed_date) AS `signed_date`,
      JSON_VALUE(event_extra.silent) AS `silent`,
      JSON_VALUE(event_extra.site_category) AS `site_category`,
      JSON_VALUE(event_extra.site_permission) AS `site_permission`,
      JSON_VALUE(event_extra.skip_reason) AS `skip_reason`,
      JSON_VALUE(event_extra.slug) AS `slug`,
      JSON_VALUE(event_extra.sort_key) AS `sort_key`,
      JSON_VALUE(event_extra.sort_type) AS `sort_type`,
      JSON_VALUE(event_extra.source) AS `source`,
      JSON_VALUE(event_extra.source_file) AS `source_file`,
      JSON_VALUE(event_extra.source_method) AS `source_method`,
      JSON_VALUE(event_extra.sourcedetails) AS `sourcedetails`,
      JSON_VALUE(event_extra.sourcetype) AS `sourcetype`,
      JSON_VALUE(event_extra.splitconsole) AS `splitconsole`,
      JSON_VALUE(event_extra.stack) AS `stack`,
      JSON_VALUE(event_extra.stack_id) AS `stack_id`,
      JSON_VALUE(event_extra.start_state) AS `start_state`,
      JSON_VALUE(event_extra.starter_type) AS `starter_type`,
      JSON_VALUE(event_extra.state) AS `state`,
      JSON_VALUE(event_extra.status) AS `status`,
      JSON_VALUE(event_extra.steeredProvider) AS `steeredProvider`,
      JSON_VALUE(event_extra.step) AS `step`,
      JSON_VALUE(event_extra.street_address) AS `street_address`,
      JSON_VALUE(event_extra.stringLength) AS `stringLength`,
      JSON_VALUE(event_extra.subcategory) AS `subcategory`,
      JSON_VALUE(event_extra.submit_type) AS `submit_type`,
      JSON_VALUE(event_extra.surface) AS `surface`,
      JSON_VALUE(event_extra.suspicious_site) AS `suspicious_site`,
      JSON_VALUE(event_extra.sync) AS `sync`,
      JSON_VALUE(event_extra.taar_based) AS `taar_based`,
      JSON_VALUE(event_extra.tab) AS `tab`,
      JSON_VALUE(event_extra.tab_layout) AS `tab_layout`,
      JSON_VALUE(event_extra.tab_pos) AS `tab_pos`,
      JSON_VALUE(event_extra.tabgroup) AS `tabgroup`,
      JSON_VALUE(event_extra.table) AS `table`,
      JSON_VALUE(event_extra.table_name) AS `table_name`,
      JSON_VALUE(event_extra.target) AS `target`,
      JSON_VALUE(event_extra.target_type) AS `target_type`,
      JSON_VALUE(event_extra.target_version) AS `target_version`,
      JSON_VALUE(event_extra.taskName) AS `taskName`,
      JSON_VALUE(event_extra.tel) AS `tel`,
      JSON_VALUE(event_extra.telemetry_name) AS `telemetry_name`,
      JSON_VALUE(event_extra.telemetry_version) AS `telemetry_version`,
      JSON_VALUE(event_extra.terminal) AS `terminal`,
      JSON_VALUE(event_extra.text) AS `text`,
      JSON_VALUE(event_extra.text_source) AS `text_source`,
      JSON_VALUE(event_extra.theme_id) AS `theme_id`,
      JSON_VALUE(event_extra.threshold) AS `threshold`,
      JSON_VALUE(event_extra.time) AS `time`,
      JSON_VALUE(event_extra.time_created) AS `time_created`,
      JSON_VALUE(event_extra.time_last_used) AS `time_last_used`,
      JSON_VALUE(event_extra.time_open) AS `time_open`,
      JSON_VALUE(event_extra.time_span) AS `time_span`,
      JSON_VALUE(event_extra.timestamp) AS `timestamp`,
      JSON_VALUE(event_extra.to) AS `to`,
      JSON_VALUE(event_extra.to_container_id) AS `to_container_id`,
      JSON_VALUE(event_extra.to_language) AS `to_language`,
      JSON_VALUE(event_extra.token_kind) AS `token_kind`,
      JSON_VALUE(event_extra.tool_name) AS `tool_name`,
      JSON_VALUE(event_extra.top_preferred_language) AS `top_preferred_language`,
      JSON_VALUE(event_extra.topic) AS `topic`,
      JSON_VALUE(event_extra.touch_capable) AS `touch_capable`,
      JSON_VALUE(event_extra.trigger) AS `trigger`,
      JSON_VALUE(event_extra.triggers) AS `triggers`,
      JSON_VALUE(event_extra.trr) AS `trr`,
      JSON_VALUE(event_extra.type) AS `type`,
      JSON_VALUE(event_extra.typeWasPassword) AS `typeWasPassword`,
      JSON_VALUE(event_extra.updated_from) AS `updated_from`,
      JSON_VALUE(event_extra.uptime) AS `uptime`,
      JSON_VALUE(event_extra.uri_type) AS `uri_type`,
      JSON_VALUE(event_extra.utm_campaign) AS `utm_campaign`,
      JSON_VALUE(event_extra.utm_content) AS `utm_content`,
      JSON_VALUE(event_extra.utm_medium) AS `utm_medium`,
      JSON_VALUE(event_extra.utm_source) AS `utm_source`,
      JSON_VALUE(event_extra.value) AS `value`,
      JSON_VALUE(event_extra.variant_shown) AS `variant_shown`,
      JSON_VALUE(event_extra.version) AS `version`,
      JSON_VALUE(event_extra.video_codec) AS `video_codec`,
      JSON_VALUE(event_extra.view_name) AS `view_name`,
      JSON_VALUE(event_extra.visible) AS `visible`,
      JSON_VALUE(event_extra.webExtensionId) AS `webExtensionId`,
      JSON_VALUE(event_extra.where_local) AS `where_local`,
      JSON_VALUE(event_extra.where_remote) AS `where_remote`,
      JSON_VALUE(event_extra.why) AS `why`,
      JSON_VALUE(event_extra.why_local) AS `why_local`,
      JSON_VALUE(event_extra.why_remote) AS `why_remote`,
      JSON_VALUE(event_extra.width) AS `width`,
      JSON_VALUE(event_extra.window) AS `window`,
      JSON_VALUE(event_extra.window_mode) AS `window_mode`
    ) AS `string`
  ) AS extras
FROM
  `moz-fx-data-shared-prod.firefox_desktop_derived.events_stream_v1`
