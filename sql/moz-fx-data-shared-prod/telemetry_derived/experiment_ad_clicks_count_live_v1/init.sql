...
IF
  NOT EXISTS `moz-fx-data-shared-prod.telemetry_derived.experiment_ad_clicks_count_live_v1`
  OPTIONS
  ...
  WITH desktop AS (
    SELECT
      ...
      UNNEST(payload.processes.parent.keyed_scalars.browser_search_ad_clicks) AS browser_search_ad_clicks
  )
  SELECT
