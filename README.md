...
- Should be defined in files named as `<project>/<dataset>/<table>_<version>/query.sql` e.g.
  `moz-fx-data-shared-prod/telemetry_derived/clients_daily_v7/query.sql`
...
- May be generated using a python script that prints the query to stdout
  ...
  - Should be named as `<project>/query_type.sql.py` e.g. `moz-fx-data-shared-prod/clients_daily.sql.py`
  - May use options to generate queries for different destination tables e.g.
    ...
    `moz-fx-data-shared-prod/telemetry/core_clients_daily_v1/query.sql` and using `--source main_summary_v4` to
    generate `moz-fx-data-shared-prod/telemetry/clients_daily_v7/query.sql`
...
- Should be defined in files named as `<project>/<dataset>/<table>/view.sql` e.g.
  `moz-fx-data-shared-prod/telemetry/core/view.sql`
...
- May depend on the previous partition
  - If using previous partition, must include an `init.sql` query to initialize the
    table, e.g. `moz-fx-data-shared-prod/telemetry_derived/clients_last_seen_v1/init.sql`
  - Should be impacted by values from a finite number of preceding partitions
    ...
    - For example `moz-fx-data-shared-prod/clients_last_seen_v1.sql` can be run serially on any 28 day
...
When adding or modifying a query in this repository, make your changes in the project directory.
