...
SELECT
  ...
  assert_equals(CAST(NULL AS STRING), regexp_extract('.$*', event_analysis.escape_metachars('...'))),
  assert_equals('.$*', regexp_extract('.$*', CONCAT(event_analysis.escape_metachars('.'), '.', event_analysis.escape_metachars('*')))),
