...
_grouped AS (
  ...
  SELECT
    ...
    udf.mode_last(ARRAY_AGG(first_seen_date)) AS first_seen_date,
...
_current AS (
  SELECT
    * EXCEPT (days_since_first_seen),
    ...
    udf.bool_to_365_bits(active_hours_sum > 0 AND total_uri_count_sum > 0) AS days_dau_bytes,
    udf.int_to_365_bits(days_since_first_seen) AS days_first_seen_bytes
  FROM
    _grouped
),
_previous AS (
