...
1. Run `./bqetl query validate <dataset>.<table>` to dry run the query. For data scientists (and anyone without `jobs.create` permissions in `moz-fx-data-shared-prod`), instead run: (a) `gcloud auth login --update-adc  # to authenticate to GCP`  (b) `gcloud config set project mozdata    # to set the project`  (c) `./bqetl query validate --use-cloud-function=false --project-id=mozdata <full path to the query file>`
