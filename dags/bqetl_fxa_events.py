...
with DAG(
    ...
    firefox_accounts_derived__fxa_server_events__v1 = bigquery_etl_query(
        task_id="firefox_accounts_derived__fxa_server_events__v1",
        destination_table="fxa_server_events_v1",
        dataset_id="firefox_accounts_derived",
        project_id="moz-fx-data-shared-prod",
    ...
    with TaskGroup(
        "firefox_accounts_derived__fxa_server_events__v1_external"
    ) as firefox_accounts_derived__fxa_server_events__v1_external:
        ExternalTaskMarker(
            task_id="bqetl_subplat__wait_for_firefox_accounts_derived__fxa_server_events__v1",
            external_dag_id="bqetl_subplat",
            external_task_id="wait_for_firefox_accounts_derived__fxa_server_events__v1",
            execution_date="{{ (execution_date - macros.timedelta(days=-1, seconds=85500)).isoformat() }}",
        )

        ExternalTaskMarker(
            task_id="bqetl_event_rollup__wait_for_firefox_accounts_derived__fxa_server_events__v1",
            external_dag_id="bqetl_event_rollup",
            external_task_id="wait_for_firefox_accounts_derived__fxa_server_events__v1",
            execution_date="{{ (execution_date - macros.timedelta(days=-1, seconds=81000)).isoformat() }}",
        )

        firefox_accounts_derived__fxa_server_events__v1_external.set_upstream(
            firefox_accounts_derived__fxa_server_events__v1
        )
