...
Reads blog post JSON records written by docker-etl's release_scraping job
...
`gcs_blog_path_for` keys the blob name on `browser`+`publish_date`+a
URL slug (not `scraped_date`), and the scraper fetches existing GCS paths
up front and skips scraping anything already present -- so a given post is
scraped and uploaded at most once, ever, and its blob is never touched
again. That makes `blob_name` (already stored per row for traceability) a
safe load cursor: every run lists the full GCS prefix, diffs it against
the `blob_name`s already loaded, and downloads+appends only what's new.
The one edge case this doesn't cover: if release_scraping's own dedup
logic ever changed such that a blob got rewritten in place under the same
name, that update would be silently missed here, since a previously-seen
blob name is never re-fetched.
"""
...
if __name__ == "__main__":
    ...
    if not blobs:
        raise ValueError(
            ...
            f"{GCS_BLOGS_PREFIX}/ -- refusing to load {TABLE_ID}"
        )
