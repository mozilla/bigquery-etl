...
`policies_is_enterprise` is aggregated by a different rule on each side, and the `coalesce` hides which one produced the published value. The pipelines take `mozfun.stats.mode_last` over the day's events, matching v8. The legacy side publishes whatever the winning metrics ping carried, `null` included, because `legacy_with_client_info_cte` picks the row first and takes about thirty dimensions from it — reaching back to an earlier ping for this one column would make the row a mixture of two pings. The two rules diverge mainly where the latest ping reported nothing and an earlier one did: `mode_last` ignores nulls and returns the earlier value, where the legacy rule returns the null. A genuine disagreement, both rules returning a value and the values differing, needs the flag to flip within the day and is rarer still.

#### How they compare to v8 and to the SERP columns

Each counter has a v8 counterpart — `legacy_searches_tagged_non_follow_on_sum` against `tagged_sap`, `legacy_searches_organic_sum` against `organic`, `legacy_searches_with_ads_tagged_sum` against `search_with_ads`, `legacy_ad_clicks_tagged_sum` against `ad_click`, and the organic pairs — and tracks it closely. The names no longer mirror v8's, because they describe what is summed rather than which v8 column it matches; this list is the mapping. `legacy_searches_organic_sum` is the exception on agreement; see the known gap below.

Two of them differ from their SERP-derived neighbours for definitional reasons rather than coverage. `legacy_searches_with_ads_*_sum` counts a SERP with ads when the ad was **served**, observed on the network, where `serp_searches_with_ads_*_count` requires the ad to have been **visible**, so the SERP column is lower by design. `legacy_ad_clicks_*_sum` counts a click by matching the outgoing request URL against ad-server patterns, and runs materially above `serp_ad_clicks_tagged_sum`.

The suffixes differ across that pairing for a reason: the legacy columns sum a counter value, where one label can contribute many, and the SERP columns count impressions, where one contributes at most one. Follow-on is also cut on a different axis on each side — legacy splits it by `tag_type` on the label, so `legacy_searches_tagged_non_follow_on_sum` and `legacy_searches_tagged_follow_on_sum` are disjoint and sum to the tagged total, while SERP splits it by access point, so `serp_follow_on_searches_tagged_count` is a subset of `serp_searches_tagged_count`. The ads families make no such split at all and include follow-on.

**Known gap on `legacy_searches_organic_sum`.** It runs below v8's `organic` and the residual is not root-caused, so prefer v8 for organic levels until it is. The rest of the family is close. Part of the gap is structural: some v8 organic rows carry no access-point suffix, and a per-access-point counter has nowhere to put them. The remainder is spread proportionally across access points, which is the shape of a coverage difference rather than a definitional one.
...
Every one of those prefixes reaches the output rather than being stripped in `final_cte`, so a consumer can tell from the name alone which side a value can come from, and therefore which columns are zero or `null` on a row that side never reached. The SERP search and ad-click measures keep theirs too, in place of the v8-facing names they once published under. On the `legacy_` counters the prefix distinguishes each counter from the SERP-derived measure of the same events, which is published under its own name.
...
- `serp_searches_tagged_count` is the only tagged count. v8's `tagged_sap` is dropped: SAP has no `is_tagged`, so glean_v1 has no independent SAP-side measure to put there and both columns would have carried the same value.
...
- The ad measures name which half they count, rather than leaving the tagged half as the unmarked default the way v8 did. v8's `ad_click` is `serp_ad_clicks_sum` and `ad_clicks_tagged` is `serp_ad_clicks_tagged_sum`, beside `serp_ad_clicks_organic_sum`. v8's `search_with_ads`, which silently meant the tagged half, is `serp_searches_with_ads_tagged_count`, beside `serp_searches_with_ads_organic_count`. Note that `serp_ad_clicks_sum` is not guaranteed to equal `serp_ad_clicks_tagged_sum` plus `serp_ad_clicks_organic_sum`: the two halves are split on `is_tagged is true` and `is_tagged is false`, both of which exclude `null`, while the total sums every row. They agree only where `is_tagged` is never `null`.
