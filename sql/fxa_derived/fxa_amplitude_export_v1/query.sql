WITH
  ...
  SELECT
    ...
    TO_HEX(udf.hmac_sha256(CAST(jsonPayload.fields.user_id AS BYTES))) AS user_id,
    ...
    ARRAY_AGG(DISTINCT JSON_EXTRACT_SCALAR(jsonPayload.fields.event_properties,
        "$.oauth_client_id") IGNORE NULLS) AS oauth_client_ids,
    -- User properties, arrays
    ARRAY_AGG(DISTINCT JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
        "$['$append'].fxa_services_used") IGNORE NULLS) AS fxa_services_used,
    ARRAY_AGG(DISTINCT jsonPayload.fields.os_name IGNORE NULLS) AS os_used_day,
    -- User properties, scalars
    MAX(JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
        "$.sync_device_count")) AS sync_device_count,
    MAX(JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
        "$.sync_active_devices_day")) AS sync_active_devices_day,
    MAX(JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
        "$.sync_active_devices_week")) AS sync_active_devices_week,
    MAX(JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
        "$.sync_active_devices_month")) AS sync_active_devices_month,
    `moz-fx-data-shared-prod`.udf.mode_last(ARRAY_AGG(JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
          "$.ua_version") IGNORE NULLS)) AS ua_version,
    `moz-fx-data-shared-prod`.udf.mode_last(ARRAY_AGG(JSON_EXTRACT_SCALAR(jsonPayload.fields.user_properties,
          "$.ua_version") IGNORE NULLS)) AS ua_browser,
  ...
  SELECT
    ...
    timestamp,
    TO_JSON_STRING( STRUCT( services,
        oauth_client_ids ) ) AS event_properties,
    '' AS user_events
  FROM
    grouped_by_user),
  user_properties AS (
  SELECT
    ...
    REPLACE( TO_JSON_STRING( STRUCT( region,
          country,
          LANGUAGE,
          os_used_day,
          sync_device_count,
          ...
          ua_browser,
          app_version,
          STRUCT( fxa_services_used ) AS str_dollar_sign_postInsert ) ), 'str_dollar_sign_', '$')
  FROM
    grouped_by_user),
  all_events AS (
  SELECT
