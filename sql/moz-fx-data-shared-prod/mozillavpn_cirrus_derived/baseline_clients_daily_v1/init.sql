...
CREATE TABLE IF NOT EXISTS
  `mozillavpn_cirrus_derived.baseline_clients_daily_v1`
PARTITION BY
  submission_date
CLUSTER BY
  is_new_profile,
...
WITH base AS (
  ...
  FROM
    `mozillavpn_cirrus_stable.baseline_v1`
...
joined AS (
  ...
  FROM
    windowed AS cd
  LEFT JOIN
    `mozillavpn_cirrus_derived.baseline_clients_first_seen_v1` AS cfs
  USING
    (client_id)
  WHERE
    _n = 1
