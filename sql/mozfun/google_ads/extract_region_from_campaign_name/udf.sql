CREATE OR REPLACE FUNCTION google_ads.extract_region_from_campaign_name(campaign_name STRING) AS (
  ...
  CASE
    ...
    WHEN REGEXP_CONTAINS(campaign_name, r"^(Mozilla_Firefox_)")
      THEN (CASE WHEN REGEXP_CONTAINS(campaign_name, r"_(US|CA)_") THEN "NA" ELSE "EU" END)
    WHEN REGEXP_CONTAINS(campaign_name, r"^(Mozilla_[A-Z]{2}_FF)")
      THEN (REGEXP_EXTRACT(campaign_name, r'Mozilla_([A-Z]{2})_FF'))
    ELSE NULL
  END
);
