...
AS (
   ...
   CONCAT(
     CASE WHEN days_since_first_seen = 0 THEN 0
         WHEN (days_since_first_seen != 0) AND (((days_since_first_seen + 1) / 7) < max_weeks) THEN CEILING((days_since_first_seen + 1) / 7)
         ELSE max_weeks END,
     '_dow',
     ...
     CASE WHEN days_since_seen >= death_time THEN '00'
        WHEN pattern BETWEEN 1 AND 7 THEN '1'
        WHEN pattern BETWEEN 8 AND 14 THEN '2'
        WHEN pattern BETWEEN 15 AND 21 THEN '3'
        WHEN pattern > 21 THEN '4' ELSE CAST(pattern AS string) END,
     '_',
     active
   )
);
