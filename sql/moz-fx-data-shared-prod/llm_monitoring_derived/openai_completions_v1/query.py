"""Integration script to fetch OpenAI completions usage data and load it into BigQuery."""
...
class BigQueryAPI:
    ...
    def load_usage_data(self, destination_table: str, buckets: list[dict]):
        ...
        for bucket in buckets:
            ...
            # Flatten each result in the bucket
            for result in bucket.get("results", []):
                record = {
                    ...
                    "input_audio_tokens": result.get("input_audio_tokens"),
                    "output_audio_tokens": result.get("output_audio_tokens"),
        ...
        job_config = bigquery.LoadJobConfig(
            schema=[
                ...
                bigquery.SchemaField("input_cached_tokens", "INTEGER"),
                bigquery.SchemaField("input_audio_tokens", "INTEGER"),
                bigquery.SchemaField("output_audio_tokens", "INTEGER"),
                bigquery.SchemaField("num_model_requests", "INTEGER"),
                bigquery.SchemaField("project_id", "STRING"),
                bigquery.SchemaField("user_id", "STRING"),
                bigquery.SchemaField("api_key_id", "STRING"),
                bigquery.SchemaField("model", "STRING"),
                bigquery.SchemaField("batch", "BOOLEAN"),
...
class OpenAICompletionsAPI:
    """API client for fetching completions usage data from OpenAI."""

    BASE_URL = "https://api.openai.com/v1/organization/usage/completions"

    def __init__(self) -> None:
        """Initialize OpenAICompletionsAPI with API key from environment."""
        self.logger = logging.getLogger(self.__class__.__name__)
...
class OpenAIBigQueryIntegration:
    """Integration orchestrator for fetching OpenAI completions data and loading to BigQuery."""
    ...
    def run(self, args):
        """Execute the integration workflow to fetch and load OpenAI completions data."""
        ...
        self.logger.info("End of OpenAI Completions BigQuery Integration")


def main():
    """Run the OpenAI completions usage data integration script."""
