...
CREATE OR REPLACE FUNCTION
  udf.add_monthly_searches(prev ARRAY<STRUCT<key STRING, value STRUCT<total_searches ARRAY<INT64>, tagged_searches ARRAY<INT64>, search_with_ads ARRAY<INT64>, ad_click ARRAY<INT64>>>>,
...
WITH previous_examples AS (
        ...
        [
          STRUCT(
            ...
            STRUCT(generate_array(11, 0, -1) AS total_searches,
                   ...
                   udf.zeroed_array(12) AS ad_click) AS value
        )] AS prev, "google" as type
    UNION ALL
    SELECT [ STRUCT(
            CAST(NULL AS STRING) AS key,
            udf.new_monthly_engine_searches_struct() AS value
    ...
    SELECT udf.add_monthly_searches(p.prev, c.curr, d.date) AS res,
...
SELECT
  ...
  CASE WHEN res_type = "total_searches" THEN udf.get_key_with_null(res, key).total_searches
       WHEN res_type = "tagged_searches" THEN udf.get_key_with_null(res, key).tagged_searches
       WHEN res_type = "search_with_ads" THEN udf.get_key_with_null(res, key).search_with_ads
       WHEN res_type = "ad_click" THEN udf.get_key_with_null(res, key).ad_click
  END),
FROM
  results
INNER JOIN
