...
new_conversion_events AS (
  ...
  FROM
    `moz-fx-data-shared-prod.google_ads_derived.conversion_event_categorization_v1` a --replace this
  JOIN
...
telemetry_id_to_activity_staging AS (
  ...
  FROM
    `moz-fx-data-shared-prod.google_ads_derived.conversion_event_categorization_v1` --replace this
  WHERE
