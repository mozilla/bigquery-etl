...
{% if is_init() %}
  ...
  -- the two tables to validate.
    *
  FROM
    `moz-fx-data-shared-prod.thunderbird_desktop_derived.baseline_clients_daily_v1`
  WHERE
...
{% else %}
  ...
  _previous AS (
    ...
    FROM
      `moz-fx-data-shared-prod.thunderbird_desktop_derived.baseline_clients_last_seen_v1`
    WHERE
