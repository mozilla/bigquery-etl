...
- `product` is the raw Zendesk `custom_product` value on this underlying table; normalization via `static.cx_product_mappings_v1` (source = 'Zendesk') is applied at read time by the `customer_experience.zendesk_retrieval_index` view, falling back to the raw value when no mapping exists.
