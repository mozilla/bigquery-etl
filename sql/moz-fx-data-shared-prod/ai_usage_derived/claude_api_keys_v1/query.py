"""Integration script to fetch Claude API keys data and load it into BigQuery."""
...
class BigQueryAPI:
    ...
    def load_api_keys_data(self, destination_table: str, records: list[dict]):
        ...
        job = client.load_table_from_json(
            records, destination_table, job_config=job_config
        )
        job.result()
...
class AnthropicAdminAPI:
    """API client for fetching organizational data from Anthropic Admin API."""

    USERS_URL = "https://api.anthropic.com/v1/organizations/users"
    API_KEYS_URL = "https://api.anthropic.com/v1/organizations/api_keys"

    def __init__(self) -> None:
        """Initialize AnthropicAdminAPI with API key from environment."""
        self.logger = logging.getLogger(self.__class__.__name__)
...
class AnthropicAPIKeysBigQueryIntegration:
    """Integration orchestrator for fetching Claude API keys and loading to BigQuery."""
    ...
    def run(self, args):
        """Execute the integration workflow to fetch and load Claude API keys data."""
...
def main():
    """Run the Claude API keys integration script."""
    parser = ArgumentParser()
    parser.add_argument(
        "--destination",
        dest="destination",
        default="moz-fx-data-shared-prod.ai_usage_derived.claude_api_keys_v1",
        required=False,
    )
