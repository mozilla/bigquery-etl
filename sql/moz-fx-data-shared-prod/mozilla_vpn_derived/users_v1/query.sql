  ...
  SELECT
    *,
    CASE
      ...
      WHEN mozfun.map.get_key(attribution, 'utm_campaign') IN (
          'private-browsing-vpn-link'
        ) THEN mozfun.map.get_key(attribution, 'utm_campaign')
      WHEN mozfun.map.get_key(attribution, 'referrer') IN (
          'app-store-ios',
          'app-store-windows',
          'app-store-android'
        ) THEN mozfun.map.get_key(attribution, 'referrer')
      WHEN mozfun.map.get_key(attribution, 'utm_medium') IN (
          'snippet',
          'email',
          'secure-proxy',
          'devtools_whatsnew'
        ) THEN mozfun.map.get_key(attribution, 'utm_medium')
      WHEN mozfun.map.get_key(attribution, 'utm_source') IN (
          'leanplum-push-notification'
        ) THEN mozfun.map.get_key(attribution, 'utm_source')

      -- Clean up some unambiguous mess
      WHEN mozfun.map.get_key(attribution, 'referrer') = 'app-store-macos'
        THEN 'app-store-ios'
      WHEN mozfun.map.get_key(attribution, 'utm_source') = 'google-play'
        THEN 'app-store-android'
      WHEN mozfun.map.get_key(attribution, 'referrer') = 'android-app://org.mozilla.firefox.vpn/'
        THEN 'android vpn app'
      WHEN mozfun.map.get_key(attribution, 'utm_source') = 'monitor.firefox.com'
        THEN 'monitor'
      WHEN attribution IS NULL
        THEN 'no attribution'

      -- Deal with the somewhat ambiguous, potentially-overlapping categories
      WHEN mozfun.map.get_key(attribution, 'referrer') = 'https://monitor.firefox.com/'
        -- Unclear whether this is a real web referral, or an in-product referral?
        -- Especially since the tracking codes are missing.
        THEN 'monitor'
      WHEN STARTS_WITH(mozfun.map.get_key(attribution, 'utm_source'), 'mozilla.org-whatsnew')
        THEN 'whatsnew page'
      WHEN mozfun.map.get_key(attribution, 'utm_source') = 'firefox-browser'
          AND STARTS_WITH(mozfun.map.get_key(attribution, 'utm_campaign'), 'about-protections')
        THEN 'about:protections'
      WHEN mozfun.map.get_key(attribution, 'utm_campaign') IS NULL
      ...
      WHEN ENDS_WITH(mozfun.map.get_key(attribution, 'referrer'), 'mozilla.org/')
        ...
        -- referral??
        THEN 'mozilla website referral'
      WHEN mozfun.map.get_key(attribution, 'utm_source') = 'blog.mozilla.org'
...
SELECT
  *,
  CASE
    WHEN attribution_category IN (
        'app-store-android',
        'app-store-ios',
        'android vpn app'
      )
      THEN 'mobile apps/stores'
    WHEN attribution_category IN (
        'untracked link',
        'blog.m.o organic'
      )
      THEN 'non-mozilla web referral'
    WHEN attribution_category IN (
        'devtools_whatsnew',
        'about:protections',
        'private-browsing-vpn-link',
    ...
    WHEN attribution_category IN (
      ...
      THEN 'no attribution'
    ELSE attribution_category
  END AS coarse_attribution_category
FROM intermediate