    ...
    (
      SELECT AS STRUCT
        application.* REPLACE (
          (
            SELECT AS STRUCT
              application.addons.* REPLACE (
                application.addons.active_addons.key_value AS active_addons,
                application.addons.active_gmplugins.key_value AS active_gmplugins
              )
          ) AS addons
        )
    ) AS application,
    (
      SELECT AS STRUCT
        payload.* REPLACE (
          (
            SELECT AS STRUCT
              payload.data.* REPLACE (payload.data.attributes.key_value AS attributes)
          ) AS data
        )
    ) AS payload
  )
