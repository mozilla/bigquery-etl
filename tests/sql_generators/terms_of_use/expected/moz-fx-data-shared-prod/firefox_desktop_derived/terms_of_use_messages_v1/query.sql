...
SELECT
  ...
  CASE
    ...
    WHEN metrics.string.messaging_system_event IN (
        "IMPRESSION",
        "CLICK_PRIMARY_BUTTON",
        "DISMISSED",
        "CLICK_SECONDARY_BUTTON"
      )
      THEN metrics.string.messaging_system_event
    -- for the modal the modal DISMISS == DISMISSED
    WHEN metrics.string.messaging_system_event = "DISMISS"
      THEN "DISMISSED"
    WHEN metrics.string.messaging_system_event_source = "primary_button"
      THEN "CLICK_PRIMARY_BUTTON"
    WHEN metrics.string.messaging_system_event_source = "secondary_button"
      THEN "CLICK_SECONDARY_BUTTON"
  ...
  CASE
    WHEN metrics.text2.messaging_system_message_id LIKE "%infobar%"
      THEN "infobar"
    WHEN LOWER(metrics.text2.messaging_system_message_id) LIKE "%onboarding%"
      THEN "onboarding"
    ELSE "modal"
  END
  messaging_surface,
