...
FROM
  `moz-fx-data-shared-prod.net_thunderbird_android.events` AS e
CROSS JOIN
...
SELECT
  ...
  event.extra AS event_extra
FROM
  `moz-fx-data-shared-prod.net_thunderbird_android_beta.events` AS e
CROSS JOIN
  UNNEST(e.events) AS event
  WITH OFFSET AS event_offset
UNION ALL
SELECT
  ...
  event.extra AS event_extra
FROM
  `moz-fx-data-shared-prod.net_thunderbird_android_daily.events` AS e
CROSS JOIN
  UNNEST(e.events) AS event
  WITH OFFSET AS event_offset
