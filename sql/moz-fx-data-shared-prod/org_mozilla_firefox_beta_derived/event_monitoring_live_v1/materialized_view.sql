...
WITH base_adjust_attribution_v1 AS (
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_firefox_beta_live.adjust_attribution_v1`
  CROSS JOIN
...
base_home_v1 AS (
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_firefox_beta_live.home_v1`
  CROSS JOIN
    UNNEST(events) AS event
  CROSS JOIN
          -- Iterator for accessing experiments.
...
base_onboarding_v1 AS (
  ...
  FROM
    `moz-fx-data-shared-prod.org_mozilla_firefox_beta_live.onboarding_v1`
  CROSS JOIN
    UNNEST(events) AS event
  CROSS JOIN
          -- Iterator for accessing experiments.
...
FROM
  combined
WHERE
  DATE(submission_timestamp) >= "2026-05-13"
GROUP BY
  submission_date,
  window_start,
  window_end,
