...
WITH todays_metrics AS (
  SELECT
    ...
    os,
    COALESCE(
      `mozfun.norm.windows_version_info`(os, last_seen.normalized_os_version, windows_build_number),
      last_seen.normalized_os_version
    ) AS os_version,
    COALESCE(
      CAST(NULLIF(SPLIT(last_seen.normalized_os_version, ".")[SAFE_OFFSET(0)], "") AS INTEGER),
      0
    ) AS os_version_major,
    COALESCE(
      CAST(NULLIF(SPLIT(last_seen.normalized_os_version, ".")[SAFE_OFFSET(1)], "") AS INTEGER),
      0
    ) AS os_version_minor,
    submission_date,
    COALESCE(
