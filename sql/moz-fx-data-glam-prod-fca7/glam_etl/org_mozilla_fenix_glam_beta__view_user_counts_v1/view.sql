-- view for org_mozilla_fenix_glam_beta__view_user_counts_v1;
CREATE OR REPLACE VIEW
  `moz-fx-data-glam-prod-fca7.glam_etl.org_mozilla_fenix_glam_beta__view_user_counts_v1`
AS
WITH all_clients AS (
  SELECT
    client_id,
    ...
    channel
  FROM
    `moz-fx-data-glam-prod-fca7`.glam_etl.org_mozilla_fenix_glam_beta__clients_scalar_aggregates_v1
  UNION ALL
  SELECT
    client_id,
    ping_type,
    ...
    channel
  FROM
    `moz-fx-data-glam-prod-fca7`.glam_etl.org_mozilla_fenix_glam_beta__clients_histogram_aggregates_v1
),
