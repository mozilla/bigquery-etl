...
SELECT
  ...
  (
    CASE
      WHEN _previous.client_id IS NULL THEN _current.channel
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.channel, _current.channel)
      ELSE _previous.channel
    END AS channel,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.app_name
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.app_name, _current.app_name)
      ELSE _previous.app_name
    END AS app_name,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.app_version
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.app_version, _current.app_version)
      ELSE _previous.app_version
    END AS app_version,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.city
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.city, _current.city)
      ELSE _previous.city
    END AS city,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.normalized_channel
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
    ...
    CASE
      WHEN _previous.client_id IS NULL THEN _current.country
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.country, _current.country)
      ELSE _previous.country
    END AS country,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.normalized_os
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.normalized_os, _current.normalized_os)
      ELSE _previous.normalized_os
    END AS normalized_os,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.normalized_os_version
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
    ...
    CASE
      WHEN _previous.client_id IS NULL THEN _current.startup_profile_selection_reason
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.startup_profile_selection_reason, _current.startup_profile_selection_reason)
      ELSE _previous.startup_profile_selection_reason
    END AS startup_profile_selection_reason,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.download_token
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.download_token, _current.download_token)
      ELSE _previous.download_token
    END AS download_token,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.download_source
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
        THEN COALESCE(_previous.download_source, _current.download_source)
      ELSE _previous.download_source
    END AS download_source,
    CASE
      WHEN _previous.client_id IS NULL THEN _current.attribution_campaign
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
    ...
    CASE
      WHEN _previous.client_id IS NULL THEN _current.attribution_content
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
    ...
    CASE
      WHEN _previous.client_id IS NULL THEN _current.attribution_experiment
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
    ...
    CASE
      WHEN _previous.client_id IS NULL THEN _current.attribution_medium
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
    ...
    CASE
      WHEN _previous.client_id IS NULL THEN _current.attribution_source
      WHEN _previous.client_id IS NOT NULL AND _previous.first_seen_date >= DATE_SUB(_current.first_seen_date, INTERVAL 7 DAY)
