WITH _previous AS (
  ...
  FROM
    `moz-fx-data-shared-prod.firefox_enterprise_desktop_derived.metrics_clients_last_seen_v1`
  WHERE
...
_current AS (
  SELECT
    *
  FROM
    `moz-fx-data-shared-prod.firefox_enterprise_desktop.metrics_clients_daily` AS m
  WHERE
