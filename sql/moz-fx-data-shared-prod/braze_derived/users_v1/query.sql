WITH ctms_emails AS (
  SELECT
    ...
    CASE
      WHEN emails.has_opted_out_of_email = TRUE THEN "unsubscribed"
      WHEN emails.double_opt_in = TRUE THEN 'opted_in'
      WHEN emails.double_opt_in = FALSE AND emails.has_opted_out_of_email = FALSE THEN 'subscribed'
      ELSE 'unsubscribed'
    ...
    CASE
      WHEN fxa.fxa_id IS NOT NULL AND fxa.account_deleted = FALSE THEN TRUE
    END AS has_fxa,
...
acoustic_events AS (
  SELECT
    contacts.email_id,
    MAX(PARSE_TIMESTAMP("%m/%d/%Y %H:%M:%S", recipients.event_timestamp)) AS acoustic_last_engaged_at
  FROM
  ...
  WHERE
    ...
    (
      NOT EXISTS (
        ...
        WHERE mofo_newsletters.email_id = emails.external_id
        AND mofo_newsletters.subscribed = TRUE
        AND mofo_newsletters.name = 'mozilla-foundation'
      )
      OR EXISTS (
        SELECT 1
        FROM `moz-fx-data-shared-prod.ctms_braze.ctms_newsletters` AS other_newsletters
        WHERE other_newsletters.email_id = emails.external_id
      ...
      OR EXISTS (
        ...
        WHERE waitlists.email_id = emails.external_id
        AND waitlists.subscribed = TRUE
      )
      OR EXISTS (
        SELECT 1
        FROM `moz-fx-data-shared-prod.subscription_platform.logical_subscriptions` AS subscriptions
        WHERE subscriptions.mozilla_account_id_sha256 = emails.fxa_id_sha256
      )
    )
    )
