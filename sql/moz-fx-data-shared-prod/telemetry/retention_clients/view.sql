...
WITH clients_last_seen AS (
  ...
  mozfun.bits28.retention( cls.days_active_bits &  cls.days_seen_bits,  cls.submission_date) AS retention_active,
  cls.days_seen_bits,
...
SELECT
  ...
  (
    np.is_new_profile
    AND cd.retention_active_in_week_3
  ) AS retained_week_4_new_profile,
  (
    np.is_new_profile
  AND BIT_COUNT(cd.days_active_bits) > 1
  )AS repeat_profile,
