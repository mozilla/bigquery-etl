...
CREATE OR REPLACE FUNCTION
  udf.evicting_queue_append(queue ANY TYPE, size INT64, item ANY TYPE) AS (
    ARRAY(
      SELECT
        val
      FROM
...
SELECT
  ...
  assert_array_equals(CAST([] AS ARRAY<STRING>), udf.evicting_queue_append(CAST([] AS ARRAY<STRING>), 0, 'c')),
  assert_array_equals([1, 2, 3], udf.evicting_queue_append([1, 2], 3, 3)),
  assert_nested_array_equals([STRUCT([1, 2] AS arr), STRUCT([2, 3] AS arr)], udf.evicting_queue_append([STRUCT([1, 2] AS arr)], 3, STRUCT([2, 3] AS arr))),
  assert_nested_array_equals([STRUCT([2, 3] AS arr)], udf.evicting_queue_append([STRUCT([1, 2] AS arr)], 1, STRUCT([2, 3] AS arr)));
