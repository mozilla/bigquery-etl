...
SELECT
  ...
  exposure_count
FROM
  `moz-fx-data-shared-prod.org_mozilla_fennec_aurora_derived.experiment_events_live_v1`
WHERE
  window_start > TIMESTAMP(DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY))
UNION ALL
SELECT
...
SELECT
  ...
  exposure_count
FROM
  `moz-fx-data-shared-prod.org_mozilla_fenix_nightly_derived.experiment_events_live_v1`
WHERE
  window_start > TIMESTAMP(DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY))
UNION ALL
SELECT
