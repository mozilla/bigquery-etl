...
SELECT
  ...
  CASE
  WHEN
    cfs.first_seen_date IS NULL OR cfs.first_seen_date > @submission_date
  THEN
    NULL
  WHEN
  ...
  IF(cfs.client_id IS NULL OR cfs.first_seen_date > @submission_date, today, cfs).* EXCEPT (first_seen_date, second_seen_date)
FROM
