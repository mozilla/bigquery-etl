...
SELECT
  combined.submission_date,
  combined.app_version,
  combined.channel,
  combined.country,
  combined.newtab_content_surface_id,
  combined.corpus_item_id,
  combined.position,
  combined.format,
  combined.is_sponsored,
  combined.is_section_followed,
  combined.matches_selected_topic,
  combined.received_rank,
  combined.section,
  combined.section_position,
  combined.topic,
  combined.content_redacted,
  combined.newtab_content_ping_version,
  combined.impression_count,
  combined.click_count,
  combined.dismiss_count,
  ...
  CASE
    WHEN combined.section_position IS NOT NULL
      THEN SAFE_CAST(
          combined.impression_count / COALESCE(
            propensity.weight,
            default_propensity.default_weight
          ) AS INT64
        )
    ELSE combined.impression_count
...
LEFT JOIN
  ...
  ON propensity.position = combined.position
  AND propensity.tile_format = combined.format
